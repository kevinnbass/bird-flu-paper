...
# New separate raw xlsx for graphing
...
def compute_article_level_scores(df, sentiment_categories):
    # For each sentiment, _quotation_mean_article is calculated by:
    # 1. Identifying all quotation columns for that sentiment (e.g. sentiment_d1, sentiment_d2, etc.)
    # 2. Clipping their values at lower=0
    # 3. Computing the mean of these clipped values across all such columns for each article.
    # Similarly, _fulltext_article is just the clipped fulltext score for that sentiment.
...
def calculate_mean_median(aggregated_df):
    # No median calculations as requested.
    stats = []
...
def compile_results_into_multiple_workbooks(aggregated_df, stats_df, raw_df, gee_results, plots_dir,
    ...
    # Sort raw data by media_category in specified order and then by outlet
    raw_df = raw_df.copy()
...
def create_analysis_for_graphing_file(df, sentiments, category_order, output_file):
    # We want a separate raw xlsx that has, for each sentiment and each measure (_quotation_mean_article and _fulltext_article),
    # a table with columns: Scientific, Left, Lean Left, Center, Lean Right, Right
    # and rows are each sentiment/emotion.
    #
    # We'll aggregate df by taking the mean per category for each sentiment and measure.
    # That is, for each sentiment, we compute the mean of _quotation_mean_article and _fulltext_article by category.
    #
    # Then we produce two sheets: one for quotation_mean_article, one for fulltext_article.

    # Ensure proper category ordering
    temp_df = df.copy()
    temp_df['media_category'] = pd.Categorical(temp_df['media_category'], categories=category_order + ['Other'], ordered=True)
    ...
    for sentiment in sentiments:
        ...
        # Compute means by category if columns exist
        if q_col in temp_df.columns:
            q_means = temp_df.groupby('media_category')[q_col].mean()
        else:
            q_means = pd.Series(index=category_order, dtype=float)

        if f_col in temp_df.columns:
            f_means = temp_df.groupby('media_category')[f_col].mean()
        else:
            f_means = pd.Series(index=category_order, dtype=float)

        # Reindex to ensure all categories are present
        q_means = q_means.reindex(category_order)
        f_means = f_means.reindex(category_order)

        # Append to lists
        quotation_data.append([sentiment] + q_means.tolist())
        fulltext_data.append([sentiment] + f_means.tolist())

    quotation_df = pd.DataFrame(quotation_data, columns=["Sentiment/Emotion"] + category_order)
    fulltext_df = pd.DataFrame(fulltext_data, columns=["Sentiment/Emotion"] + category_order)

    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        quotation_df.to_excel(writer, sheet_name='Quotation_Mean_Article', index=False)
        fulltext_df.to_excel(writer, sheet_name='Fulltext_Article', index=False)
...
def main():
    ...
    # Create separate raw xlsx for graphing _fulltext_article and _quotation_mean_article
