...
Now duplicates the entire pipeline for a subset df_yes, i.e. only rows
where "high_rate_2" == "yes".
"""
...
# We'll define these as template names, then suffix them if needed
BASE_OUTPUT_DIR = "graphs_analysis"
BASE_CSV_OUTPUT_DIR = "csv_raw_scores"

# For the "all data" run:
...
# For the "high_rate_2=yes" subset:
...
def chunk_and_save(df, chunk_size=20000, csv_output_dir=BASE_CSV_OUTPUT_DIR, prefix=""):
    logging.info(f"Chunking DataFrame => len={len(df)}, chunk_size={chunk_size}, prefix={prefix}")
    for i in range(0,len(df),chunk_size):
        part=df.iloc[i:i+chunk_size]
        fn_suffix = f"_{prefix}" if prefix else ""
        out_csv=os.path.join(csv_output_dir,f"raw_data_part_{(i//chunk_size)+1}{fn_suffix}.csv")
...
###############################################################################
def analyze_quotation_fulltext_correlation(df, outdir=BASE_OUTPUT_DIR, 
    ...
    plt.ylim(-1,1)
    plt.tight_layout()
    cbar=os.path.join(outdir,"correlation_quotation_fulltext_bar.png")
    try:
        plt.savefig(cbar)
    except:
        pass
...
def plot_statistics(agg_df, out_dir=BASE_OUTPUT_DIR, prefix=""):
    ...
    for s in CATEGORIES:
        ...
        plt.title(f"Mean Quotation-Based '{s.capitalize()}' Scores {suffix}")
        plt.xticks(rotation=45,ha="right")
        plt.tight_layout()
        ...
        plt.title(f"Mean Fulltext-Based '{s.capitalize()}' Scores {suffix}")
        plt.xticks(rotation=45,ha="right")
        plt.tight_layout()
...
def run_gee_for_sentiment_measure_best_qic(df, sentiment, measure):
    # Quick check: attempt "no scale" with Poisson/Independence to see if we can create _score_col
...
###############################################################################
# 10) Function to run entire pipeline
###############################################################################
def run_full_pipeline_for_df(
    df,
    prefix="", 
    # By default, write to the base sets of files unless user overrides:
    ...
    """
    Runs the entire pipeline (the same logic as 'main()' used to).
    We'll produce analysis for the given df, saving files with a suffix if prefix is not empty.
    """
    if prefix:
        # Adjust output paths
    ...
    # Just chunk for demonstration if needed:
    chunk_and_save(df,20000,csv_output_dir=csv_output_dir,prefix=prefix)
    print_basic_stats(df)

    # correlation => Quotation vs Fulltext
    ...
    save_aggregated_scores_to_csv(agg_df, out_dir=csv_output_dir, prefix=prefix)
    plot_statistics(agg_df, out_dir=output_dir, prefix=prefix)

    # GEE analyses
    df_best, df_allcombos=run_gee_analyses_best_qic(df)
    print("Best QIC approach completed for prefix=", prefix)
    print(df_best)
...
def main():
    setup_logging()
    logging.info("Starting best QIC GEE approach + advanced checks, plus duplication for high_rate_2=yes subset")

    print("Loading full data & mapping categories...")
    df=load_jsonl(INPUT_JSONL_FILE)
    print(f"Total articles loaded: {len(df)}")

    df=map_media_outlet_to_category(df)

    print("Running pipeline on ALL data now...")
    run_full_pipeline_for_df(
        df, 
        prefix=""  # no suffix for the main analysis
    )

    # Now subset for "high_rate_2" == "yes"
    print("\nCreating subset for high_rate_2=yes ...\n")
    df_yes = df[df["high_rate_2"] == "yes"].copy()
    print(f"Subset with high_rate_2=yes => {len(df_yes)} articles")

    # Run pipeline for subset
    if len(df_yes) > 0:
        print("Running pipeline on SUBSET (high_rate_2=yes) now...")
        run_full_pipeline_for_df(
            df_yes, 
            prefix="Yes"  # This will produce e.g. analysis_main_Yes.xlsx, etc.
        )
    else:
        print("No articles found with high_rate_2=yes. Skipping subset analysis.")

    logging.info("All analyses completed. Exiting main().")
