...
def pairwise_and_diagnostics(df, sentiment, measure, fam_name, struct, scale_name,
                             prefix="", isSens=False):
    """
    A comprehensive version that:
      1) Refits model with best scale
      2) Checks residuals, pseudo-likelihood, cross-validation
      3) Performs cluster bootstrap
      4) Performs jackknife
      5) Calculates pairwise differences with correct Z- and p-values
      6) Adds DiffIDs and Trans_DiffIDs columns
      7) Returns an 8-tuple:
         (summary_txt, pair_df, diffIDs_df, diag_dict, pseudo_dict, cv_val, sens_df, boot_info)
    """
    ...
    param_stds_boot = boot_params_df.std().round(4)

    # ------------------------------------------------------
    # 3) Perform jackknife as well (jackknife_gee_md must be defined)
    # ------------------------------------------------------
    ...
    param_stds_jk = jk_params_df.std().round(4)

    # ------------------------------------------------------
    # 4) Fill COMPARE_IMBALANCE with single-run, bootstrap, & jackknife SE
    # ------------------------------------------------------
    for i, p in enumerate(param_names):
        sr_se  = robust_se[i]
        sr_val = param_values[i]

        bs_std = param_stds_boot.get(p, np.nan)
        jk_std = param_stds_jk.get(p, np.nan)

        ratio_b = np.nan
        ratio_j = np.nan
        if sr_se > 0 and not np.isnan(bs_std):
            ratio_b = bs_std / sr_se
        if sr_se > 0 and not np.isnan(jk_std):
            ratio_j = jk_std / sr_se
        ...
        COMPARE_IMBALANCE.append({
            ...
            "IsSens":       isSens,
            "SingleRun_SE": sr_se,
            "MD_BootStd":   bs_std,   # cluster bootstrap
            "JK_Std":       jk_std,   # jackknife
            "Ratio_Boot":   ratio_b,
            "Ratio_JK":     ratio_j,
            "ParamEst":     sr_val
        })

    # Build a dict with param mean/std from cluster bootstrap
    param_mean_dict = boot_params_df.mean().round(4).to_dict()
    param_std_dict  = param_stds_boot.to_dict()
    boot_info = {
    ...
    # ------------------------------------------------------
    # 5) Pairwise differences => fix Z and p-values
    # ------------------------------------------------------
    ...
    # ------------------------------------------------------
    # 6) Re-add Trans_DiffIDs logic if you want it
    # ------------------------------------------------------
    mapping = {1:3, 2:1, 3:5, 4:2, 5:6, 6:4}

    def transform_diff_ids(cell_value):
        if pd.isna(cell_value) or cell_value.strip() == "":
            return ""
        parts = [x.strip() for x in cell_value.split(",") if x.strip()]
        mapped = []
        for p in parts:
            if p.isdigit():
                old_num = int(p)
    ...
    # Done
    return (
        summary_txt,       # textual summary from final_fit
        pair_df,           # DataFrame with (CategoryA, CategoryB, Difference, SE, Z, p_value, p_value_adj, reject_H0)
        ...
        boot_info          # param means/std from cluster bootstrap
    )
...
def run_gee_for_sentiment_measure_best_qic(df, sentiment, measure):
    ...
    # 5) Build combos_df from 'all_results' with the extra column for NaN_Count
    combos_df = pd.DataFrame(
        all_results,
        columns=["Family", "CovStruct", "Scale", "QIC", "NaN_Count"]
    )
