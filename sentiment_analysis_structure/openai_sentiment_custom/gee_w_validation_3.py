...
Fixes the "KeyError: 2" by converting 'pearson_res' to a NumPy array so numeric indexing is consistent.
"""
...
def check_residuals_and_correlation(final_fit):
    """
    Convert 'pearson_res' to a NumPy array for consistent numeric indexing.
    """
    y=final_fit.model.endog
    mu=final_fit.fittedvalues
    # create numeric array for consistent indexing
    pearson_res_arr = ((y - mu) / np.sqrt(mu+1e-9)).to_numpy()
    ...
    for i, cid in enumerate(clusters):
        cluster_map.setdefault(cid,[]).append(pearson_res_arr[i])  # numeric index works
    ...
    for cid, arr in cluster_map.items():
        ...
        if len(arr)>1:
            if len(arr)>2:
                ...
                # if arr is length=2 => corr is cmat[0,1]
                cmat=np.corrcoef(arr)
                r=cmat[0,1]
                wcorr.append(r)
