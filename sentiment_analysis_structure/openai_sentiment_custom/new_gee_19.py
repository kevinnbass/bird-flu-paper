...
def fit_gee_and_pairwise(df, sentiment, measure_type='Quotation', cov_struct_type=Exchangeable):
    ...
    # Set grid=True for Unstructured and Autoregressive only (as they support that)
    if cov_struct_type in [Unstructured, Autoregressive]:
...
def compile_results_into_multiple_workbooks(aggregated_df, stats_df, raw_df, gee_results, qic_df, plots_dir,
    ...
    # Replace applymap with apply to avoid future warning:
    # We'll apply column-wise:
