...
def transform_date_huffpost(date_str):
    ...
    """
    # Attempt to parse using strptime
    # Note: '%m/%d/%Y' can parse without leading zeros on month/day in Python
    dt = datetime.strptime(date_str, '%m/%d/%Y')
    return dt.strftime('%Y/%m/%d')

def transform_date_forbes(date_str):
    """
    Convert a date in D(D) Month YY to YYYY/MM/DD (zero-padded).
    Example: '26 June 20' -> '2020/06/26'
    """
    # Attempt to parse using strptime
    # '%d %B %y' => day (1-31), full month name (January, February, ...), two-digit year
