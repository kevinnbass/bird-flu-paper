...
# ------------------------------ #
# We keep single braces around {fulltext}, but escape the braces in the JSON snippet.
...
def main():
    ...
    3) We process them with the two prompts approach, printing each coded spread_risk & feedback to terminal.
    ...
    for article in tqdm(new_articles, desc="Processing new articles"):
        ...
        # *** NEW: Print each entry's spread_risk & spread_feedback to terminal ***
        print(f"\n[NEW ENTRY CODED]")
        print(f"  ID: {article['id']}")
        print(f"  spread_risk: {article['spread_risk']}")
        print(f"  spread_feedback: {article['spread_feedback']}")
    ...
    print(f"\nWrote {len(processed_articles)} new articles to {new_file}.")

    # Then append the 20 old articles from spread_subset_test_3.jsonl
    appended_count = 0
    if os.path.isfile(OLD_FILE):
