#!/usr/bin/env python3
# gee_final_best_qic_with_pairwise_cld_rpy2.py
"""
Script that:
  ...
  6) For each best QIC model:
     ...
     - Build a CLD table with 4 columns => [MediaCategory, Mean, UserCLD (blank), AutoCLD]
       by constructing an adjacency matrix => pass to R via rpy2 => call multcompView::multcompLetters(...)
         * We interpret "reject_H0=False" => not differ => adjacency=1
         * "reject_H0=True" => differ => adjacency=0
         * retrieve letters => final table
...
import rpy2.robjects.numpy2ri  # so we can do np->R array conversion

# We assume "multcompView" is installed in R
...
###############################################################################
# 6) Pairwise + R-based CLD with rpy2 => using adjacency matrix
###############################################################################
def build_cld_with_rpy2(final_fit, pair_df):
    """
    1) We'll gather the categories in final_fit.
    2) Construct an adjacency matrix M => M[i,j] = 1 if NOT significantly different,
                                           0 if significantly different
       using "reject_H0" from pair_df
    3) We'll pass M to R => multcompView::multcompLetters(M, compare=FALSE, threshold=0.5)
    4) Retrieve letters => build final table => [MediaCategory, Mean, UserCLD="", AutoCLD]
    5) Optionally do a final check to ensure no adjacency conflicts
    """
    ...
    mean_map={}
    for c in cats:
        val=intercept+cat_effect[c]
        mean_map[c]=math.exp(val)

    # Build adjacency => "not differ" => 1, "differ" => 0
    ...
    # Build dictionary => (A,B) => "not differ"? from pair_df
    # pair_df => each row => CategoryA, CategoryB, reject_H0
    # If reject_H0=True => "they differ" => adjacency=0
    # else => adjacency=1
    ...
    # Now pass adjacency to R => 0 or 1
    from rpy2.robjects import default_converter
    from rpy2.robjects.conversion import localconverter
    import rpy2.robjects.numpy2ri as np2ri

    with localconverter(default_converter + np2ri.converter):
        M_r=ro.conversion.py2rpy(adjacency)  # Convert np array -> R matrix
    # set row/col names in R
    ro.r.colnames(M_r)=ro.StrVector(group_names)
    ro.r.rownames(M_r)=ro.StrVector(group_names)

    # Call multcompLetters(M_r, compare=FALSE, threshold=0.5)
    # => returns an R object with a "Letters" named vector
    res=multcompview.multcompLetters(M_r, compare=False, threshold=0.5)
    # retrieve => Letters
    ...
    # letters_py is typically a dict-like structure: { "Center": "a", "Left": "ab", ... }

    # optional final check => ensure adjacency is correct
    # If adjacency[i,j]==1 => they must share >=1 letter
    # If adjacency[i,j]==0 => no letter overlap
    # We'll do this check => if fail => raise
    for i in range(n):
        for j in range(i+1,n):
            A=group_names[i]
            B=group_names[j]
            not_diff=(adjacency[i,j]==1)
            A_letters=letters_py[A]
            B_letters=letters_py[B]
            overlap=set(A_letters).intersection(set(B_letters))
            if not_diff:
                # must share at least 1 letter
                if len(overlap)==0:
                    raise RuntimeError(
                        f"R-based CLD conflict: {A} vs {B} are 'not differ' but share no letter. "
                    )
            else:
                # differ => must share 0 letters
                if len(overlap)>0:
                    raise RuntimeError(
                        f"R-based CLD conflict: {A} vs {B} differ but share letter(s) {overlap}. "
                    )

    # build final DataFrame => [MediaCategory, Mean, UserCLD="", AutoCLD=letters_py[c]]
    rows=[]
    for c in CLD_ORDER:
        if c in group_names:
            meanval=mean_map[c]
            rows.append((c, meanval, "", letters_py[c]))
        else:
            rows.append((c, None, "", ""))
...
def pairwise_and_cld(df, sentiment, measure, struct, scale_name):
    ...
    2) Pairwise => BH => build adjacency => pass to R => get letters
    3) Build final table
    """
    from statsmodels.genmod.generalized_estimating_equations import GEE
    d2=df.copy()
    ...
    # Now we do the adjacency approach => rpy2 => build_cld_with_rpy2
...
def compile_results_into_multiple_workbooks(
    ...
    with pd.ExcelWriter(gee_excel, engine="openpyxl") as writer:
        ...
        for i, row in df_best_qic.iterrows():
            ...
            # highlight rows where reject_H0=True in red
...
def main():
    setup_logging()
    logging.info("Starting best QIC GEE approach with BH post-hoc & rpy2-based CLD using multcompView::multcompLetters")
    ...
    # 5) best QIC approach
    print("Fitting best QIC approach for each sentiment–measure (with BH pairwise + rpy2-based letter assignment).")
