...
FIX for KeyError when indexing pearson_res_arr => convert to np.array
"""
...
def check_residuals_and_correlation(final_fit):
    ...
    for cid, arr in cluster_map.items():
        ...
        if len(arr)>1:
            ...
            # if 'arr' is 1D, cmat is a single scalar or shape (1,1)? 
            # In that case, skip. 
...
def main():
    setup_logging()
    logging.info("Starting best QIC GEE approach + advanced checks + fix for index KeyError in residuals")
    ...
    print("Analysis completed successfully with KeyError fix (residual indexing).")
