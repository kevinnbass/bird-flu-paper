...
def build_cld(fit_result, pair_df):
    ...
    Removed the "import CLD_ORDER" line—now we just use the global CLD_ORDER.
    """
    ...
    # differ[A][B] = True if "they differ"
    ...
    # Initialize letter sets to empty
    ...
    # Convert sets to sorted letter strings, reorder by CLD_ORDER
    rows = []
    for c in CLD_ORDER:
        if c in cats:
