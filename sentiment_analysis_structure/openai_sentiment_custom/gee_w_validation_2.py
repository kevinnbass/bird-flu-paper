...
def analyze_quotation_fulltext_correlation(df):
    """
    Also store the combined Z-scatter data in a second sheet of 'quotation_fulltext_correlation.xlsx'
    so it can be manually re-plotted if needed.
    """
    ...
    # combined z-scatter data
    # regroup data by sentiment => store in scatter_map as well
    for s in CATEGORIES:
        sub=agg_df[(agg_df["Sentiment"]==s)].dropna(subset=["Quotation_Average","Fulltext_Average"])
    ...
    # create "quotation_fulltext_correlation.xlsx" => two tabs => "CorrelationSummary", "CombinedZScatterData"
    with pd.ExcelWriter("quotation_fulltext_correlation.xlsx", engine="openpyxl") as w:
        cor_df.to_excel(excel_writer=w, sheet_name="CorrelationSummary", index=False)
        ...
        if combo:
            ...
            v.to_excel(excel_writer=w, sheet_name="CombinedZScatterData", index=False)
...
###############################################################################
# 7) Compile => produce summary, pairwise & diffIDs, plus new Diagnostics, etc.
###############################################################################
def compile_results_into_multiple_workbooks(
    ...
    with pd.ExcelWriter(main_excel, engine="openpyxl") as w:
        aggregated_df.to_excel(excel_writer=w, sheet_name="Aggregated_Scores", index=False)
        stats_df.to_excel(excel_writer=w, sheet_name="Mean_Median_Statistics", index=False)

    # analysis_raw
    with pd.ExcelWriter(raw_excel, engine="openpyxl") as w:
        raw_df.to_excel(excel_writer=w, sheet_name="Raw_Data", index=False)
        for s in CATEGORIES:
            ...
            s_df.to_excel(excel_writer=w, sheet_name=sheet_name, index=False)
    ...
    with pd.ExcelWriter(gee_excel, engine="openpyxl") as writer:
        ...
        for i, row in df_best_qic.iterrows():
            ...
            if summary_txt is None:
                ...
                tmp_df.to_excel(excel_writer=writer, sheet_name=sh_name, index=False)
                continue

            # Summaries
            lines=summary_txt.split("\n")
            sumdf=pd.DataFrame({"GEE_Summary":lines})
            sumdf.to_excel(excel_writer=writer, sheet_name=sh_name, index=False)

            sr=len(sumdf)+2
            pair_df.to_excel(excel_writer=writer, sheet_name=sh_name, index=False, startrow=sr)
            ...
            if diffIDs_df is not None and not diffIDs_df.empty:
                diffIDs_df.to_excel(excel_writer=writer, sheet_name=sh_name, index=False, startrow=sr2)

            # separate re-fit for finalFit => gather checks
            from statsmodels.genmod.generalized_estimating_equations import GEE
            d2=df_full.copy()
            if meas=="Quotation":
                pat=rf"^{re.escape(s)}_\d+$"
        ...
        idxdf.to_excel(excel_writer=writer, sheet_name="BestQIC_Index", index=False)

        if not df_all_combos.empty:
            df_all_combos.to_excel(excel_writer=writer, sheet_name="All_Combos", index=False)

        diag_df=pd.DataFrame(diag_records)
        diag_df.to_excel(excel_writer=writer, sheet_name="Diagnostics", index=False)

        cv_df=pd.DataFrame(cv_records)
        cv_df.to_excel(excel_writer=writer, sheet_name="CrossValidation_Res", index=False)

        if len(sens_records)>0:
            sens_all=pd.concat(sens_records,ignore_index=True)
            sens_all.to_excel(excel_writer=writer, sheet_name="Sensitivity_Analysis", index=False)

        if len(boot_records)>0:
            boot_all=pd.concat(boot_records,ignore_index=True)
            boot_all.to_excel(excel_writer=writer, sheet_name="Bootstrap_Res", index=False)
...
def main():
    ...
    # 3) correlation => also writes xlsx with combinedZ data
    ...
    # 6) compile => summary, pairwise, diffIDs, plus new diagnostics
    compile_results_into_multiple_workbooks(
        ...
        main_excel="analysis_main.xlsx",
        raw_excel="analysis_raw.xlsx",
        gee_excel="analysis_gee.xlsx",
        plots_excel="analysis_plots.xlsx",
        combined_excel="analysis_combined.xlsx",
        df_full=df
    )
