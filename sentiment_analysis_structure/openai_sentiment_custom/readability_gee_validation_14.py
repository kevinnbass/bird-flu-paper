...
Script that:
  ...
  3) Finds best QIC combination (multi-family) for each measure
     - Now includes a grid search for NegativeBinomial alpha in [0..30] by 1
  ...
  6) Also saves analysis_readability_main, analysis_readability_raw,
     analysis_readability_plots, analysis_readability_combined (Renamed for clarity).
"""
...
# Also ignore the 'log link alias is deprecated' warning
...
###############################################################################
# 5) Coverage Simulation (model.groups-based)
###############################################################################
def coverage_simulation_gee(final_fit, n_sims=1000, alpha=0.05):
    ...
    Using model.groups to assign cluster_id to each row to avoid KeyError.
    """
...
###############################################################################
# 6) Multi-Family enumerations with NB alpha search
###############################################################################
def try_all_families_and_scales(df, formula, group_col):
    """
    We brute-force across:
      - Families (Poisson, Gaussian, Gamma, InverseGaussian) with no alpha search
      - NegativeBinomial with alpha in [0..30]
      - CovStruct (Independence, Exchangeable)
      - Scale (none, pearson, deviance, ub, bc)

    Return best_tuple -> (fam_name, cov_name, scale_opt, alpha_val, qic_val)
    and a list of all (fam, cov, scale, alpha, QIC).
    """
    from statsmodels.genmod.families.family import NegativeBinomial
    ...
    # 2) Handle NegativeBinomial => alpha in [0..30, step=1]
        ...
        for sc_opt in scale_opts:
            for alpha_candidate in alpha_range:
                try:
                    ...
                    model=GEE.from_formula(formula, groups=group_col,
                                           data=df, family=nb_fam, cov_struct=cov_obj)
                    base_res=model.fit(scale=None)
                    y=base_res.model.endog
                    mu=base_res.fittedvalues
                    n=len(y)
...
def run_gee_for_measure_best_qic(df, measure):
    ...
    Returns {
      ...
      Best_Structure,
      Best_Scale,
      Best_Alpha,   <-- might be a float if NB
      Best_QIC_main,
      AllCombos
    }
    """
    ...
    return {
        ...
        "Best_Scale": scOpt,
        "Best_Alpha": alphaVal,  # might be float (NB) or None
...
def refit_best_gee_with_scale(df, measure, fam_name, struct, scale_name, alpha_val=None):
    """
    Using best alpha if NegativeBinomial. 
    """
...
def compile_results_into_multiple_workbooks(
    ...
    with pd.ExcelWriter(gee_excel, engine="openpyxl") as writer:
        ...
        for i,row in df_best_qic.iterrows():
            ...
            # CrossValidation => store minimal columns
            cv_records.append({
                "Measure": m,
                "Family": fam,
                "Structure": st,
...
def main():
    setup_logging()
    logging.info("Starting GEE approach with NB alpha grid search, coverage, cluster bootstrap, etc.")
    ...
    print("Analysis completed successfully with best alpha integrated, coverage simulation, cluster bootstrap, etc.")
    logging.info("Analysis completed successfully with best alpha integrated, coverage simulation, cluster bootstrap, etc.")
