...
OUTPUT_EXCEL_LMM = 'analysis_gee.xlsx'  # Now GEE
...
def fit_gee_and_pairwise(df, sentiment, measure_type='Quotation'):
    ...
    # Fit GEE model with Poisson family and Exchangeable covariance
    family = Poisson()
    ...
    # Compute QIC (quasi-likelihood under independence model criterion)
    # QIC is not built-in. Approximate QIC:
    # QIC = -2 * quasi-likelihood under the independence model + 2 * number of parameters
    # This is a simplified approach; you may need a custom function for QIC.
    # For demonstration, we skip detailed QIC calculation due to complexity.
    # results.qic() does not exist by default.

    # Residual analysis:
    # Extract Pearson residuals
    pearson_resid = results.resid_pearson
    plt.figure()
    sns.histplot(pearson_resid, kde=True)
    plt.title(f"Pearson Residuals - {sentiment} {measure_type}")
    plt.xlabel("Residual")
    plt.ylabel("Frequency")
    plt.savefig(os.path.join(OUTPUT_DIR, f"residuals_{sentiment}_{measure_type}.png"))
    plt.close()
    ...
    for i in range(len(cats)):
        for j in range(i+1, len(cats)):
            ...
            z = diff_est / diff_se
            p_value = 2 * (1 - sm.distributions.norm.cdf(abs(z)))
