...
Script that:
  ...
  6) For each best QIC model:
     ...
     - Build a CLD table with 4 columns => [MediaCategory, Mean, UserCLD (blank), AutoCLD]
       using the new snippet from compactletterdisplay (with index-range checks)
...
###############################################################################
# 6) The iterative approach + fix for IndexError
###############################################################################
...
def compile_results_into_multiple_workbooks(
    ...
    # Avoid "At least one sheet must be visible" => only remove "Sheet" if it is not the only one
    wb_plots=Workbook()
...
def main():
    setup_logging()
    logging.info("Starting best QIC GEE approach with BH post-hoc & iterative CLD, with fixes for IndexError & sheet removal.")
