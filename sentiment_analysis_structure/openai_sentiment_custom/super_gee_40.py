...
# We will store parameter-level comparisons here
COMPARE_IMBALANCE = []
...
def analyze_all_custom_correlations(df, prefix=""):
    """
    We just call analyze_2fields_correlation with different
    pairs of fields to see how Quotation, Fulltext, etc. align.
    """
...
###############################################################################
# try_all_families_and_scales => No cluster bootstrap here
###############################################################################
            ...
            for sc_opt in scale_opts:
                try:
                    ...
                    if sc_opt!="none" and dfresid>0:
                        # For brevity, we are skipping the part where we actually
                        # compute 'val' from compute_pearson_scale or others.
...
###############################################################################
# cluster_bootstrap_gee => only used on final best model
###############################################################################
...
###############################################################################
# pairwise_and_diagnostics => final step => DO cluster bootstrap on final model
###############################################################################
def pairwise_and_diagnostics(df, sentiment, measure, fam_name, struct, scale_name,
    ...
    # 2) Perform cluster bootstrap for the final model only
    fam_obj = final_fit.model.family
    cov_obj = final_fit.model.cov_struct
    formula = final_fit.model.formula
    means_boot, sds_boot = cluster_bootstrap_gee(d2, formula, "media_outlet_clean",
                                                 B=200, family=fam_obj, cov_struct=cov_obj)

    means_dict = means_boot.to_dict()
    sds_dict   = sds_boot.to_dict()

    # 3) Store param-level info
    for i, p in enumerate(param_names):
        sr_se  = robust_se[i]
        sr_val = param_values[i]
        bs_sd  = sds_dict.get(p, np.nan)
        ratio  = np.nan
        if sr_se>0:
            ratio = bs_sd/sr_se
    ...
    # 4) Pairwise differences across categories
    ...
    # 5) Return boot info to store in Excel
    boot_info={
        "ParamMean": means_dict,
        "ParamStd":  sds_dict
    }
    ...
    with pd.ExcelWriter(gee_excel, engine="openpyxl") as writer:
        ...
        for i,row in df_best_qic.iterrows():
            ...
            boot_records.append({
                ...
                "Structure": st,
                "Scale": sc,
                "BootstrapParamMean": pm_str,
                "BootstrapParamStd":  ps_str
            })
...
def run_gee_analyses_best_qic(df):
    """
    For each sentiment & measure, we call run_gee_for_sentiment_measure_best_qic,
    collecting the best QIC combos. Then we combine them all into DataFrames.
    """
    measure_list=[
        "Quotation",
        "Fulltext",
...
def run_gee_for_sentiment_measure_best_qic(df, sentiment, measure):
    """
    Just picks best QIC by calling try_all_families_and_scales. 
    No cluster bootstrap here. 
    """
    d2=df.copy()
    if measure=="Quotation":
        pat=rf"^{re.escape(sentiment)}_\d+$"
