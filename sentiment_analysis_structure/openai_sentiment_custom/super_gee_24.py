#!/usr/bin/env python3
# super_gee_all_validations_dual_cats_sens_in_same_xlsx.py
"""
...
Small clusters are merged in a "sens" run, and appended to the same
Excel files in new SENS tabs (rather than separate files).
"""
...
# Will store data for "Compare_Imbalance"
COMPARE_IMBALANCE = []
...
def pairwise_and_diagnostics(df, sentiment, measure, fam_name, struct, scale_name,
    ...
    cv_val=cross_validation_gee(d2,"_score_col ~ media_category","media_outlet_clean",
                                final_fit.model.family, final_fit.model.cov_struct,3)
...
def append_sens_to_excel(
    ...
    # 4) combined_excel => add SENS tabs
...
def run_pipeline_for_df(
    ...
    # 2) if prefix=="Yes", collapse each category => single cluster
    if prefix=="Yes":
    ...
    # 8) SENS => merges small clusters => appended
