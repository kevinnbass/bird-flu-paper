...
def try_fit_model(df, score_col):
    ...
    # Since we reset index, df and X should align
    # df and X have the same number of rows now, so no indexing out of bounds
    df_aligned = df.loc[X.index]  # X.index should match df now that df is reset
