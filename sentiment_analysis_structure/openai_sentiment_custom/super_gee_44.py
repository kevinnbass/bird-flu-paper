        ...
        for cov_obj in cor_structs:
            ...
            for sc_opt in scale_opts:
                ...
                try:
                    ...
                    qic_val = base_res.qic()
                    if isinstance(qic_val, tuple):
                        qic_val = qic_val[0]
                except Exception as e:
                    # We'll mark this as a "fail" => nan_count=1
                    nan_count = 1
                    # QIC remains NaN

                # Store the result (including the new NaN_Count column)
                all_results.append((fam_name, cov_name, sc_opt, qic_val, nan_count))

                # If QIC is not NaN and it's better than our current best, update
