...
def plot_median_scores(stats_df, sentiment_categories, output_dir):
    ...
    unique_combined_categories = stats_df['Combined Category'].unique()
    palette = sns.color_palette("viridis", len(unique_combined_categories))
    ...
    for sentiment in sentiment_categories:
        ...
        # Assign 'Combined Category' to hue to utilize the palette correctly
        sns.barplot(x='Combined Category', y='Median', data=subset, palette=palette, hue='Combined Category')
        ...
        plt.legend(title='Combined Category', bbox_to_anchor=(1.05, 1), loc='upper left')  # Move legend outside
        plt.tight_layout()
...
def plot_mean_scores(stats_df, sentiment_categories, output_dir):
    ...
    sns.set_style("whitegrid")
    unique_combined_categories = stats_df['Combined Category'].unique()
    palette = sns.color_palette("magma", len(unique_combined_categories))
    ...
    for sentiment in sentiment_categories:
        ...
        plt.figure(figsize=(12, 8))
        # Assign 'Combined Category' to hue to utilize the palette correctly
        sns.barplot(x='Combined Category', y='Mean', data=subset, palette=palette, hue='Combined Category')
