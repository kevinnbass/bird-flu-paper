...
def exclude_small_clusters_within_category(
    ...
    # Track how many merges happen, for debugging
    total_merged = 0
    ...
    for cat in categories:
        ...
        # Outlets < min_size for *this* category
        small_outlets = counts[counts < min_size].index
        if len(small_outlets) > 0:
            # Update the DataFrame in-place for those outlets
            mask_small = (df2[group_col].isin(small_outlets)) & mask_cat
            df2.loc[mask_small, group_col] = merge_into
            total_merged += mask_small.sum()

    # Convert to categorical after merging => ensures that "OtherSmall" is recognized as a new category
    df2[group_col] = df2[group_col].astype("category")

    print(f"[DEBUG] exclude_small_clusters_within_category => Merged {total_merged} rows into '{merge_into}' for < {min_size} articles.")
    print(f"[DEBUG] Unique outlets after merging => {df2[group_col].nunique()} (including 'OtherSmall' if used).")
    return df2
...
def run_pipeline_for_df(
    ...
    # 6) Compile results => multiple Excel workbooks
    ...
    # 7) The SENS run => merges small outlets per category => "OtherSmall"
    ...
    # Check final number of clusters in df_sens for debugging
    print(f"[SENS] => prefix={prefix}, after merging => #unique outlets: {df_sens['media_outlet_clean'].nunique()}")

    if len(df_sens) < 2:
        print(f"[Sensitivity] => merging <{min_size} => not enough data => skip SENS for prefix={prefix}.")
        return

    # 8) Re-do aggregator on SENS
