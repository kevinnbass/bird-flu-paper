...
def run_gee_and_qic(df, sentiment, measure_type, structures, min_date=None):
    ...
    # If AR(1) is tested, we require a valid date => create int_date
    ...
    for struct_name, struct_obj in structures.items():
        time_arr = None
        # If AR1, we only do so if int_date is present
        if struct_name == 'AR1':
            if 'int_date' not in mdf.columns:
                # no date => skip
                continue
            # ensure no missing int_date
            mdf.dropna(subset=['int_date'], inplace=True)
            if len(mdf) < 2:
                continue
