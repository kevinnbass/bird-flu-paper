...
def build_cld_with_rpy2(final_fit, pair_df):
    ...
    Then pass it to R => set row/col names inside R by assignment,
    then call multcompView::multcompLetters(...).
    Retrieve letter assignment => build final table.
    """
    ...
    np.fill_diagonal(adjacency,1)  # each group "not different" from itself
    ...
    # Instead of ro.r.colnames(M_r) = ro.StrVector(group_names),
    ...
    res=multcompview.multcompLetters(ro.r("M_r"), compare=False, threshold=0.5)
    ...
    # Optional check => if adjacency[i,j]==1 => they share >=1 letter, else share none
    for i in range(n):
...
def main():
    setup_logging()
    logging.info("Starting best QIC GEE approach with BH post-hoc & rpy2-based CLD (syntax fix for colnames/rownames).")
