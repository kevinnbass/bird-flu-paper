...
# Input file changed to processed_all_articles_merged.jsonl
INPUT_JSONL_FILE = 'processed_all_articles_merged.jsonl'

# We will create new output directories and files for the subset run
OUTPUT_DIR = 'graphs_analysis_subset'
os.makedirs(OUTPUT_DIR, exist_ok=True)

CSV_OUTPUT_DIR = 'csv_raw_scores_subset'
os.makedirs(CSV_OUTPUT_DIR, exist_ok=True)

OUTPUT_EXCEL_MAIN = 'analysis_main_subset.xlsx'
OUTPUT_EXCEL_RAW = 'analysis_raw_subset.xlsx'
OUTPUT_EXCEL_LMM = 'analysis_gee_subset.xlsx'  # Using GEE now
OUTPUT_EXCEL_PLOTS = 'analysis_plots_subset.xlsx'
OUTPUT_EXCEL_COMBINED = 'analysis_combined_subset.xlsx'
...
def setup_logging(log_file='analysis_subset.log'):
    logging.basicConfig(
        filename=log_file,
        level=logging.DEBUG,
...
def save_aggregated_scores_to_csv(aggregated_df, csv_output_dir, prefix='aggregated_sentiment_emotion_scores_subset.csv'):
...
def plot_statistics(aggregated_df, output_dir):
    ...
    for sentiment in CATEGORIES:
        ...
        plt.tight_layout()
        quote_plot_filename = f"quote_{sentiment}_subset.png"
        ...
        plt.xticks(rotation=45, ha='right')
        plt.tight_layout()
        fulltext_plot_filename = f"fulltext_{sentiment}_subset.png"
...
def compile_results_into_multiple_workbooks(aggregated_df, stats_df, raw_df, gee_results, plots_dir,
    ...
    for sentiment in CATEGORIES:
        quote_plot_path = os.path.join(plots_dir, f"quote_{sentiment}_subset.png")
        ...
        fulltext_plot_path = os.path.join(plots_dir, f"fulltext_{sentiment}_subset.png")
...
def main():
    setup_logging()
    print("Running analysis on a subset of data where high_rate_2 == 'yes' from processed_all_articles_merged.jsonl")

    df = load_jsonl(INPUT_JSONL_FILE)
    print(f"Total articles loaded initially: {len(df)}")

    # Filter for subset: only articles where high_rate_2 is "yes"
    if 'high_rate_2' not in df.columns:
        print("The 'high_rate_2' column is not found in the dataset.")
        logging.error("The 'high_rate_2' column is not found in the dataset.")
        sys.exit(1)

    df = df[df['high_rate_2'] == 'yes']
    print(f"Filtered subset size: {len(df)} articles where high_rate_2 == 'yes'")

    df = map_media_outlet_to_category(df, MEDIA_CATEGORIES)

    # Chunking the raw data to CSV for record
    chunk_size = 20000
    for i in range(0, len(df), chunk_size):
        chunk = df.iloc[i:i+chunk_size]
        chunk_csv_path = os.path.join(CSV_OUTPUT_DIR, f"raw_data_part_{i//chunk_size+1}_subset.csv")
        chunk.to_csv(chunk_csv_path, index=False)
        print(f"Saved chunk {i//chunk_size+1} (subset) to {chunk_csv_path}")

    print("\nSummary Statistics (Subset):")
    print("Total subset articles:", len(df))
    outlet_counts = df['media_outlet_clean'].value_counts()
    print("\nNumber of subset articles per outlet:")
    print(outlet_counts)
    category_counts = df['media_category'].value_counts()
    print("\nNumber of subset articles per category:")
    print(category_counts)
    print()

    print("Aggregating sentiment/emotion scores per media category (Subset)...")
    ...
    save_aggregated_scores_to_csv(aggregated_df, CSV_OUTPUT_DIR, prefix='aggregated_sentiment_emotion_scores_subset.csv')
    plot_statistics(aggregated_df, OUTPUT_DIR)

    print("Fitting GEE models and performing pairwise comparisons on subset...")
    gee_results = run_gee_analyses(df)
    print("GEE analysis and pairwise tests (subset) completed successfully.\n")
    ...
    print("Subset analysis completed successfully.")
