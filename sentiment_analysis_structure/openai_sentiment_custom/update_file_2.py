...
def main():
    ...
    # ------------------------------------------------------------------
    # 1) We'll use a counter to assign IDs if missing
    # ------------------------------------------------------------------
    global_id_counter = 1
    ...
    with open(file_5_jsonl, "r", encoding="utf-8") as f5:
        for line in f5:
            ...
            # ------------------------------------------------------
            # Ensure this entry has an 'id'; if not, assign one
            # ------------------------------------------------------
            nonlocal_id = obj_5.get("id")
            if not nonlocal_id:
                obj_5["id"] = str(global_id_counter)
                global_id_counter += 1
    ...
    # Overwrite processed_all_articles_fixed_5.jsonl with updated entries (including assigned IDs)
