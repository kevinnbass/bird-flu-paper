...
Now includes:
  - For "Quotation_Intensity", we match columns like "anger_1_intensity", "anger_2_intensity", etc.
  - Additional scale options: "none","pearson","deviance","ub","bc".
"""
...
def analyze_quotation_fulltext_correlation(df):
    """
    Original correlation analysis between Quotation (averaged ^<s>_\\d+$) and <s>_fulltext
    across media categories. This remains unchanged.
    """
...
def try_all_families_and_scales(df, formula, group_col):
    """
    Here we now try *all* scale options: ["none","pearson","deviance","ub","bc"] 
    for each family, with each correlation structure.
    """
    families = [
        Poisson(),
    ...
    # ADDED: additional scale options
...
def refit_best_gee_with_scale(df, sentiment, measure, fam_name, struct, scale_name):
    """
    We also change "Quotation_Intensity" to match columns like:
       ^<sentiment>_\\d+_intensity$
    """
    ...
    if measure=="Quotation":
        ...
        # Now we match all columns like ^<sentiment>_\\d+_intensity$
        pat=rf"^{re.escape(sentiment)}_\d+_intensity$"
        matched=[c for c in d2.columns if re.match(pat,c)]
        if not matched:
            return None,None
        # If you want to keep negative intensities, remove .clip(lower=0).
...
def run_gee_for_sentiment_measure_best_qic(df, sentiment, measure):
    ...
    We do a quick test to ensure columns exist before the big brute force.
    """
    ...
    # Now run the multi-family, multi-scale approach
...
def main():
    setup_logging()
    logging.info("Starting best QIC GEE approach + advanced checks, with Quotation_Intensity pattern matching & additional scale options")
    ...
    print("Fitting best QIC approach for each sentiment–measure (incl. new Quotation_Intensity).")
    ...
    print("Analysis completed successfully with expanded scaling + Quotation_Intensity pattern matching.")
