...
def bootstrap_gee_md(df, formula, group_col,
    ...
    for _ in range(B):
        ...
        try:
            # If you have maxiter=300, keep it:
            res = mod.fit(maxiter=300)
        except:
            # If GEE fails completely, skip replicate
            continue

        # Check parameter names
        param_names = res.params.index
        if param_index_full is None:
            # First success => define
            param_index_full = param_names
        else:
            # If dimension or order differs, skip replicate
    ...
    # If we got zero valid replicates:
