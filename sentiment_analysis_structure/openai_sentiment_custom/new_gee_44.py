...
def run_gee_and_qic(df, sentiment, measure_type, structures, min_date=None):
    ...
    # Remove single-obs groups
    mdf= remove_single_obs_groups(mdf, score_col)
    # Now remove huge groups (>500)
...
def main():
    setup_logging()
    print("Multi-structure GEE + QIC + optional fix (remove single-obs) + remove huge groups (>500) + deeper debugging.")
    ...
    print("Fitting GEE models with multiple correlation structures, scale=1, optional single-obs removal, removing >500-observation groups, deeper debug.")
