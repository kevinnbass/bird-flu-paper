...
def try_all_families_and_scales(df, formula, group_col):
    ...
    Return:
      best_tuple -> (fam_name, cov_name, scale_opt, alpha_val, qic_val)
      all_results -> list of (fam, cov, scale, alphaVal or None, QIC)
    """
    families = [
        Poisson(),
                ...
                if fam_name != "NegativeBinomial":
                    ...
                    try:
                        ...
                        if isinstance(qic_val, tuple):
                            qic_val=qic_val[0]
                        # alpha_val = None
                        all_results.append((fam_name, cov_name, sc_opt, "alpha=None", qic_val))
                    ...
                    # NegativeBinomial => do alpha search
                    alpha_vals = np.arange(0.0, 10.1, 0.1)
                    for alpha_candidate in alpha_vals:
                        ...
                        try:
                            ...
                            if qic_val<best_qic:
                                best_qic=qic_val
                                best_tuple=(fam_name,cov_name,sc_opt,alpha_candidate,qic_val)
                        except Exception as e:
...
def run_gee_for_measure_best_qic(df, measure):
    ...
    for row in combos:
        ...
        recs.append({
            ...
            "CovStruct": cov_,
            "Scale": sc_,
            "Alpha": alpha_str,  # e.g. 'alpha=1.2' or 'alpha=None'
            "QIC": q_
        })
    ...
    return {
        ...
        "Best_Scale": scOpt,
        "Best_Alpha": alphaVal,  # <--- store the numeric alpha or None
    ...
    for m in MEASURES:
        ...
        if info is not None:
            best_list.append({
                ...
                "Best_Alpha": info["Best_Alpha"],   # new
...
def refit_best_gee_with_scale(df, measure, fam_name, struct, scale_name, alpha_val=None):
    """
    Now includes alpha_val to handle NB's best alpha.
    """
...
def pairwise_and_diagnostics(df, measure, fam_name, struct, scale_name, alpha_val=None):
    """
    Refit model with best QIC approach, now passing alpha_val for NB.
    """
...
def compile_results_into_multiple_workbooks(
    ...
    with pd.ExcelWriter(gee_excel, engine="openpyxl") as writer:
        ...
        for i,row in df_best_qic.iterrows():
            ...
            sc=row["Best_Scale"]
            alpha_val=row.get("Best_Alpha", None)   # <--- we retrieve alpha
            best_qic=row["Best_QIC_main"]

            sh_name=f"BestQIC_{m[:15]}"

            # Now we pass alpha_val so we actually use the best alpha
            ...
            # We could optionally add a line about alpha if NB
            if fam=="NegativeBinomial":
                sumdf.loc[len(sumdf)] = [f"Alpha used: {alpha_val}"]
            ...
            # Add alpha to diag_records, etc.
            diag_records.append({
                "Measure": m,
                "Family": fam,
                "Structure": st,
                "Scale": sc,
                "Alpha": alpha_val,  # <--- record alpha
...
def main():
    ...
    save_aggregated_scores_to_csv(agg_df,"csv_raw_scores")
    ...
    print("Analysis completed successfully with best alpha integrated.")
    logging.info("Analysis completed successfully with best alpha integrated.")
