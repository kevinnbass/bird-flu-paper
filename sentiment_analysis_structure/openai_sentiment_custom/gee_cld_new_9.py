...
Script that:
  ...
  6) For each best QIC model:
     ...
     - Instead of a CLD, build a [category, diffIDs] table => if category i is significantly
       different from j, then j's index is in that row. E.g. "2,4,5"
  7) Writes summary, pairwise, & this "DiffIDs" table into analysis_gee.xlsx (one sheet per sentiment×measure)
  8) Also saves analysis_main, analysis_raw, analysis_plots, analysis_combined

No adjacency or letter grouping is used here, so we avoid all triangulation conflicts.
"""
...
# The final output order for some plots:
...
###############################################################################
# 6) Pairwise + "DiffIDs" table (no CLD)
###############################################################################
def pairwise_and_diffIDs(df, sentiment, measure, struct, scale_name):
    """
    Refit best GEE => final_fit
    1) GEE summary
    2) Pairwise table => BH
    3) Build 2-column table => [Category, DiffIDs]
       - For each cat => gather which other cats are significantly different => list them as '2,4,5'
    """
    from statsmodels.genmod.generalized_estimating_equations import GEE
    d2=df.copy()
    if measure=="Quotation":
    ...
    # Now build the "DiffIDs" table
    # We'll define cat i => index i+1
    cat_list=list(cats)
    cat_index_map={cat_list[i]: i+1 for i in range(len(cat_list))}

    # We'll create a dict => cat => setOfDiff
    diff_map={c: set() for c in cat_list}

    for i,row in pair_df.iterrows():
        A=row["CategoryA"]
        B=row["CategoryB"]
        different=row["reject_H0"]  # True => significantly different
        if different:
            # add each other's index
            diff_map[A].add(cat_index_map[B])
            diff_map[B].add(cat_index_map[A])

    # convert to DataFrame => 2 columns => [Category, "2,4,5"]
    rows=[]
    for c in cat_list:
...
###############################################################################
# 7) Compile => produce summary, pairwise, & DiffIDs table
###############################################################################
def compile_results_into_multiple_workbooks(
    ...
    # 3) analysis_gee => best QIC => each (s, meas)
    ...
    # 4) analysis_plots => at least one sheet
    wb_plots=Workbook()
    ...
    # 5) analysis_combined => ensure at least one sheet
...
def main():
    setup_logging()
    logging.info("Starting best QIC GEE approach, no CLD => replacing with 'DiffIDs' notation (2,4,5).")
    ...
    # 6) compile => summary, pairwise, & DiffIDs for each tab
    ...
    print("Analysis completed successfully. 'DiffIDs' column replaces CLD with e.g. '2,4,5'.")
