...
def fit_lmm_and_posthoc(df, sentiment, measure_type='Quotation'):
    ...
    if "Random effects covariance is singular" in summ_text:
        ...
        print("\n[INFO] The 'Random effects covariance is singular' warning often occurs when there isn't enough variation in the random effect levels (e.g., some outlets may have identical values or insufficient data).")
        print("       This can cause the random effects structure to collapse, resulting in a singular covariance matrix.")
        print("       In other words, the model can't reliably estimate the random effects variance because some groups provide no new information.\n")

        # Check random effects covariance matrix if available:
        if hasattr(mdf, 'cov_re') and mdf.cov_re is not None:
            print("[DEBUG] Random Effects Covariance Matrix (cov_re):")
            print(mdf.cov_re)
            # Identify if any diagonal elements are zero or extremely small:
            diag_vals = np.diag(mdf.cov_re)
            small_threshold = 1e-10
            small_diag_indices = np.where(diag_vals < small_threshold)[0]
            if len(small_diag_indices) > 0:
                print("[DEBUG] Some random effect variances are near zero (indices):", small_diag_indices)
                print("       This implies the model can't estimate variation for these random effects.")
            else:
                print("[DEBUG] cov_re is singular, but no diagonal element is exactly zero. Possibly correlation issues among effects.")

        # Re-check outlet variations with std:
        group_std = model_df.groupby("media_outlet")[score_col].std().fillna(0)
        zero_std_outlets = group_std[group_std == 0].index.tolist()
        if zero_std_outlets:
            print("[INFO] Outlets with zero standard deviation in scores (likely contributing to singularity):")
            for po in zero_std_outlets:
                print(f" - {po}")
        else:
            # If no zero-std, check very low std:
            low_std_threshold = 1e-6
            very_low_std = group_std[group_std < low_std_threshold]
            if not very_low_std.empty:
                print("[INFO] Outlets with extremely low standard deviation in scores (likely contributing to singularity):")
                for vo in very_low_std.index:
                    print(f" - {vo}: std={very_low_std[vo]}")
            else:
                print("[INFO] No outlets with zero or extremely low std found. Still, insufficient variation or data configuration may cause singularity.")

        print("\n[INFO] Consider removing outlets with no variation or combining categories. Also ensure enough data per outlet.")
        print("       Another approach is to simplify the random effects structure if certain random effects are not estimable.\n")
