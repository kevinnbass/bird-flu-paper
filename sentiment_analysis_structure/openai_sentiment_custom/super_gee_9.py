...
Additionally:
  - We produce two sets of outputs (Yes & All) by adding prefix to filenames.
  - We use a new cluster bootstrap function (bootstrap_gee_md) that applies
    the Mancl & DeRouen correction inside each replicate.
"""
...
def mancl_derouen_correction(gee_result):
    """
    Multiply the robust covariance by M/(M-1).
    """
...
def bootstrap_gee_md(df, formula, group_col,
    ...
    """
    Cluster-level bootstrap that applies M&D to the robust covariance 
    for each replicate. We store the parameter estimates + M&D-corrected SEs.
    """
...
###############################################################################
# pairwise_and_diagnostics => use M&D-corrected bootstrap
###############################################################################
def pairwise_and_diagnostics(df, sentiment, measure, fam_name, struct, scale_name):
    ...
    # New: use the M&D bootstrap approach
    ...
    # Summaries of the bootstrap param & M&D-based SE distributions
    param_means = boot_params_df.mean().round(4).to_dict()
    param_stds  = boot_params_df.std().round(4).to_dict()
    se_means = boot_se_df.mean().round(4).to_dict()
    se_stds  = boot_se_df.std().round(4).to_dict()

    # We'll store them in a dict for passing upward
    boot_info = {
    ...
    # pairwise code with original final_fit
    ...
    # Return: we pass boot_info in place of old boot_df
...
def compile_results_into_multiple_workbooks(
    ...
    with pd.ExcelWriter(gee_excel, engine="openpyxl") as writer:
        ...
        for i,row in df_best_qic.iterrows():
            ...
            paramMeanStr = str(boot_info["ParamMean"])
            paramStdStr  = str(boot_info["ParamStd"])
            seMeanStr    = str(boot_info["MD_SEMean"])
            seStdStr     = str(boot_info["MD_SEStd"])
            ...
            boot_records.append({
                ...
                "Structure": st,
                "Scale": sc,
                "MD_BootParamMean": paramMeanStr,
                "MD_BootParamStd":  paramStdStr,
                "MD_BootSEMean":    seMeanStr,
                "MD_BootSEStd":     seStdStr
            })
...
###############################################################################
# 6) run_pipeline_for_df => gather validation records, prefix-based filenames
###############################################################################
def run_pipeline_for_df(
    ...
    We also rename the output files if prefix is non-empty
    to ensure two separate sets of outputs.
    """
    if prefix:
    ...
    # coverage simulation => single coverage value
