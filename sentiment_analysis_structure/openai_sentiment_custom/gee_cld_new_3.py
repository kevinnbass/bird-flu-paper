#!/usr/bin/env python3
# gee_final_best_qic_with_pairwise_cld_rpy2_fixed.py
"""
Script that:
...
def build_cld_with_rpy2(final_fit, pair_df):
    ...
    Retrieve letter assignment => final table.
    NOTE: The fix is: compare=">" instead of compare=False, threshold=0.5
    """
    ...
    # Assign matrix + names in R
    ro.r.assign("M_r", M_r)
    ro.r.assign("cat_names", ro.StrVector(group_names))
    ro.r("colnames(M_r) <- cat_names")
    ro.r("rownames(M_r) <- cat_names")

    # The crucial fix: compare=">", threshold=0.5
    # If adjacency=1 => 1 is '>' 0.5 => same group
    # adjacency=0 => 0 is not '>' 0.5 => differ
    ...
    # optional check => if adjacency[i,j]==1 => share >=1 letter
    for i in range(n):
        for j in range(i+1,n):
            not_diff=(adjacency[i,j]==1)
            A=group_names[i]
            B=group_names[j]
            overlap=set(letters_py[A]).intersection(set(letters_py[B]))
            if not_diff:
                if len(overlap)==0:
                    raise RuntimeError(
    ...
    for c in CLD_ORDER:
        if c in group_names:
            rows.append((c, mean_map[c], "", letters_py[c]))
        else:
            rows.append((c, None, "", ""))
...
def pairwise_and_cld(df, sentiment, measure, struct, scale_name):
    ...
    # Now call build_cld_with_rpy2 => with compare=">"
...
def compile_results_into_multiple_workbooks(
    ...
    # We'll add sheets for each category's plot if they exist
    any_sheets_added=False
    for s in CATEGORIES:
    ...
    # If we haven't created any sheet, add a dummy sheet so openpyxl won't complain
    if len(wb_plots.sheetnames)==0:
        wb_plots.create_sheet("DummySheet")

    wb_plots.save(plots_excel)

    # For the "combined" workbook
    ...
    # If we somehow ended with no sheets (very unlikely here), add a dummy
...
def main():
    setup_logging()
    logging.info("Starting best QIC GEE approach with BH post-hoc & rpy2-based CLD (compare='>' fix + dummy sheets).")
