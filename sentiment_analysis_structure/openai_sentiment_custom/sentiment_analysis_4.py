...
def map_outlets_to_categories(df, categories_dict):
    ...
    for category, outlets in categories_dict.items():
        for outlet in outlets:
            outlet_to_category[outlet.lower()] = category  # Convert to lowercase for consistent mapping
...
def print_category_sums(df, group_by, column, label):
    """
    Prints the sum of categories for a given group and column.
...
def main():
    ...
    # Print Counts Before Proportion Calculation
    print_category_sums(df, group_by='source', column='sentiment', label='Sentiment')
    print_category_sums(df, group_by='category', column='sentiment', label='Sentiment by Category')
    print_category_sums(df, group_by='source', column='fear', label='Fear')
    print_category_sums(df, group_by='category', column='fear', label='Fear by Category')
