...
NOW FIXED so that if prefix=="Yes", files like "analysis_main_Yes.xlsx" are generated.
"""
...
def pairwise_and_diagnostics(df, sentiment, measure, fam_name, struct, scale_name):
    ...
    # if you wanted the M&D standard errors
...
def run_pipeline_for_df(
    ...
    FIX: we append prefix to the output file names if prefix is non-empty,
    so that we get separate outputs for 'Yes' subset and 'All' full set.
    """
    # ------------------- NEW: Suffix filenames with prefix ------------------- #
    if prefix:
