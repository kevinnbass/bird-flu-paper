#!/usr/bin/env python3
# gee_no_cld_diffids_extended.py
"""
Script that:
  1) Loads data from JSONL
  2) Chunks & saves CSV
  3) Performs correlation analyses & scatterplots for Quotation vs Fulltext
     => Also saves combined Z-scatter data into "quotation_fulltext_correlation.xlsx"
  ...
  7) Writes summary, pairwise, & "DiffIDs" table into analysis_gee.xlsx (one sheet per sentiment×measure)
  8) Also saves analysis_main, analysis_raw, analysis_plots, analysis_combined
  9) Additional checks: residual checks, negative binomial test, cross-validation,
     sensitivity analysis across correlation structures, bootstrap resampling
     => all saved in new sheets in analysis_gee.xlsx

No adjacency or letter grouping is used, so we avoid triangulation conflicts.
"""
...
# Additional new file for correlation details
OUTPUT_EXCEL_CORRELATION = "quotation_fulltext_correlation.xlsx"
...
def analyze_quotation_fulltext_correlation(df):
    """
    Also store the combined Z-scatter data in another tab of 'quotation_fulltext_correlation.xlsx'
    so it can be manually re-plotted.
    """
    ...
    # We'll also store the separate data for the combined Z-scatter
    combo=[]
    # re-walk the data to group it by sentiment
    scatter_map={}
    for s in CATEGORIES:
        sub=agg_df[(agg_df["Sentiment"]==s)].dropna(subset=["Quotation_Average","Fulltext_Average"])
    ...
    # We'll store: 
    #  1) cor_df in "CorrelationSummary"
    #  2) if combo is non-empty => the final combined z-scatter data in "CombinedZScatterData"
    writer_xlsx = pd.ExcelWriter("quotation_fulltext_correlation.xlsx", engine="openpyxl")

    cor_df.to_excel(writer_xlsx,"CorrelationSummary",index=False)

    # Do the bar + scatter plots
    # bar
    ...
    # now do the combined z-scatter if combo has data
    if combo:
        ...
        # store this v in a separate tab in the xlsx
        v.to_excel(writer_xlsx,"CombinedZScatterData",index=False)
    ...
    # Also store correlation data in CSV as before
...
def check_residuals_and_correlation(final_fit):
    """
    Pearson residuals, within-cluster correlation, deviance => short text assessment
    """
    y=final_fit.model.endog
    mu=final_fit.fittedvalues
    pearson_res=(y-mu)/np.sqrt(mu+1e-9)
    mean_res=np.mean(pearson_res)
    std_res=np.std(pearson_res)
    clusters=final_fit.model.groups
    cluster_map={}
    for i, cid in enumerate(clusters):
        cluster_map.setdefault(cid,[]).append(pearson_res[i])
    wcorr=[]
    ...
    if std_res>2:
        assess+="Residual std>2 => potential outliers/overdisp. "
    if abs(avg_corr)>0.3:
        assess+=f"Within-cluster corr={avg_corr:.2f} => check structure. "
    if (not np.isnan(overdisp)) and overdisp>2:
        assess+=f"Overdisp={overdisp:.2f} => NB or fix. "
    if assess=="":
...
def pseudo_likelihood_check(final_fit):
    ...
    if not isinstance(fam,Poisson):
        return {"NB_QIC":None,"Poisson_QIC":None,"diff_QIC":None,"conclusion":"Non-Poisson => skip NB check"}
    ...
    nb_model=GEE.from_formula(formula, groups=groups, data=data, family=NegativeBinomial(), cov_struct=cov_struct)
    nb_res=nb_model.fit()
    nb_qic=nb_res.qic()
    old_qic=final_fit.qic()
    diff_qic=old_qic-nb_qic
    concl="NegBin better" if diff_qic>0 else "No NB improvement"
    return {
        "NB_QIC": nb_qic,
        "Poisson_QIC": old_qic,
        "diff_QIC": diff_qic,
    ...
    for fam in families:
        for covs in structures:
            ...
            results.append({
                ...
                "QIC":qic if isinstance(qic,(int,float)) else qic[0],
...
###############################################################################
# 7) Compile results => we add new sheets for Diagnostics, CrossVal, ...
###############################################################################
    ...
    with pd.ExcelWriter(gee_excel, engine="openpyxl") as writer:
        ...
        for i, row in df_best_qic.iterrows():
            ...
            if summary_txt is None:
                tmp_df=pd.DataFrame({"Summary":[f"No valid model or not enough data => {s}-{meas}"]})
            ...
            # We'll do a separate re-fit to gather final_fit for diagnostics, etc.
            from statsmodels.genmod.generalized_estimating_equations import GEE
            d2=df_full.copy()
            if meas=="Quotation":
                pat=rf"^{re.escape(s)}_\d+$"
...
def main():
    setup_logging()
    logging.info("Starting best QIC GEE approach + 'DiffIDs' + extended checks + CombinedZScatter data in new tab.")
    ...
    # 3) correlation => also writes an xlsx with combinedZ data
    ...
    # 6) compile => summary, pairwise, diffIDs, plus new diagnostics sheets
    ...
    print("Analysis completed successfully with extended methods + data for CombinedZScatter in 'quotation_fulltext_correlation.xlsx'.")
