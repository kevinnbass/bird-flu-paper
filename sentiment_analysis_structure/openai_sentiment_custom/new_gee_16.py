...
OUTPUT_EXCEL_QIC = 'analysis_gee_qic.xlsx'  # New file for QIC comparison
...
def fit_gee_and_pairwise(df, sentiment, measure_type='Quotation', cov_struct_type=Exchangeable):
    # Prepare data depending on measure_type
    ...
    model_df = temp_df.dropna(subset=[score_col, 'media_category', 'media_outlet', 'date']).copy()
    ...
    qic_value = getattr(results, 'qic', np.nan)
...
def try_all_cov_structures(df, sentiment, measure_type):
    # Include AR(1) in the list of structures
    ...
    for sentiment in CATEGORIES:
        ...
        for measure_type in ['Quotation', 'Fulltext']:
            ...
            if len(results_list) == 0:
                continue
            # Pick the best by QIC
            results_list = sorted(results_list, key=lambda x: x[1])  # sort by QIC ascending
            ...
            # Sensitivity: The user can inspect QIC_Results to see how much QIC differs
...
def main():
    ...
    # Ensure 'date' is present and usable
    # If not present or not parseable, this may fail. We assume date field exists.
    if 'date' not in df.columns:
        print("Warning: 'date' field not found in the data. AR(1) structure may not work properly.")
    else:
    ...
    # Compute correlation between Quotation_Average and Fulltext_Average for each sentiment
    correlation_results = []
    ...
    # Combine all sentiments for normalized scatter
    combined_list = []
    for sentiment, sdata in correlation_scatter_data.items():
        df_temp = sdata.copy()
    ...
    combined_all['Quotation_Z'] = combined_all.groupby('Sentiment/Emotion')['Quotation_Average'].transform(lambda x: (x - x.mean())/x.std() if x.std() != 0 else x - x.mean())
    combined_all['Fulltext_Z'] = combined_all.groupby('Sentiment/Emotion')['Fulltext_Average'].transform(lambda x: (x - x.mean())/x.std() if x.std() != 0 else x - x.mean())
    ...
    compile_results_into_multiple_workbooks(aggregated_df, stats_df, df, gee_results, qic_df, OUTPUT_DIR,
