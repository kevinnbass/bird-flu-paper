...
Script that:
  ...
  7) Now replicates the entire pipeline for a subset df_yes
     where "high_rate_2" == "yes".

Additionally:
  - We incorporate all the validation and utility functions from your snippet,
    including sensitivity_analysis_correlation, bootstrap_gee, cross_validation_gee,
    pseudo_likelihood_check, check_residuals_and_correlation, etc.

Important:
  - For the subset run, we produce separate plots & XLSX files with a suffix "_Yes"
    so they do not overwrite the full-data results.
"""
...
INPUT_JSONL_FILE = "processed_all_articles_fixed_4.jsonl"  # or whatever your file is
...
def sensitivity_analysis_correlation(df, formula, group_col):
    ...
    gather their QIC & sample parameters. This is a helpful check.
    """
...
def pairwise_and_diagnostics(df, sentiment, measure, fam_name, struct, scale_name):
    ...
    cats=mdf["media_category"].unique()  # or cat.categories if it's categorical
    ...
    for i in range(len(cats)):
        for j in range(i+1,len(cats)):
            ...
            z=diff_est/diff_se if diff_se>0 else np.nan
...
def main():
    setup_logging()
    logging.info("Starting best QIC GEE approach + advanced checks, plus subset for high_rate_2=yes")
