...
def fit_gee_and_pairwise(df, sentiment, measure_type='Quotation', cov_struct_type=Exchangeable):
    ...
    if cov_struct_type == Autoregressive:
        ...
        # If still there's a cluster with only 1 obs, skip AR(1)
        counts = model_df.groupby('media_outlet').size()
        if (counts < 2).any():
            # Still invalid for AR(1)
            return None

        time = model_df['time_index'].astype(np.int32).values
...
def try_all_cov_structures(df, sentiment, measure_type):
    # If AR(1) fails because of single obs clusters, code will skip
