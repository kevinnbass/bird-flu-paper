...
def fit_gee_and_pairwise(df, sentiment, measure_type='Quotation', cov_struct_type=Exchangeable):
    ...
    if cov_struct_type == Autoregressive:
        ...
        model_df['time_index'] = model_df.groupby('media_outlet').cumcount().astype(np.int32)

        counts = model_df.groupby('media_outlet').size()
        if (counts < 2).any():
            return None

        time = model_df['time_index'].values
...
def main():
    ...
    # Ensure plot_statistics is defined and called here
