...
# Global set to track problematic outlets
...
def fit_lmm_and_posthoc(df, sentiment, measure_type='Quotation'):
    ...
    for outlet in outlet_counts.index:
        ...
        if outlet_data.nunique() < 2:
            outlets_with_variation_issues.append((outlet, outlet_data.nunique(), outlet_data.unique()))
    ...
    if outlets_with_variation_issues:
        for outlet, uniq_count, uniq_vals in outlets_with_variation_issues:
            print(f"Outlet '{outlet}' may have insufficient variation (unique values: {uniq_count}).")
            print(f"[DEBUG] Outlet '{outlet}' unique values: {uniq_vals}")
            logging.info(f"Outlet '{outlet}' may have insufficient variation (unique values: {uniq_count}, values={uniq_vals}).")
            # Add to global set of problematic outlets
...
def main():
    ...
    # Now, if we found problematic outlets due to insufficient variation, let's filter and re-run analysis
    if problematic_outlets:
        print("\nOutlets identified with insufficient variation:", problematic_outlets)

        # Create a filtered version of your data
        df_filtered = df[~df['media_outlet'].isin(problematic_outlets)].copy()

        print("\n--- Running analysis on filtered dataset ---")

        # Redefine output directories for filtered analysis
        ...
        # Define new Excel outputs for filtered analysis
        ...
        # Temporarily store original names to restore later
        ...
        # Override globals for filtered run
        ...
        # Re-run pipeline on filtered data
        ...
        # Restore original output file names if needed
