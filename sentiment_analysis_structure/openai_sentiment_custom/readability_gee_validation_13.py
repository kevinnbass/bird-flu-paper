#!/usr/bin/env python3
# readability_gee_validation_12.py
"""
Script that:
  1) Loads data, does correlation analyses
  2) Aggregates stats for two measures (flesch_kincaid_grade_global, gunning_fog_global)
  ...
  4) For each best QIC model:
     ...
     - Pairwise => "DiffIDs"
     - Cluster bootstrap, coverage simulation
  5) Writes results into an Excel output with multiple tabs
  6) This version fixes the coverage simulation KeyError by using model.groups properly.
"""
...
# The above warning is about 'Gamma(link=ln.log())'; we can ignore it or switch to 'links.Log()' if desired.
...
###############################################################################
# 5) Coverage Simulation (FIXED cluster indexing)
###############################################################################
def coverage_simulation_gee(final_fit, n_sims=1000, alpha=0.05):
    """
    Parametric approach to estimate coverage of the final GEE model's CIs.
    We now fix the KeyError by assigning the cluster array directly.
    ...
    # Instead of using group_col as a column name, we take the actual group array:
    clusters = final_fit.model.groups
    ...
    for _ in range(n_sims):
        ...
        # Simulate outcomes param. by family
...
def main():
    setup_logging()
    logging.info("Starting GEE approach with coverage fix (no KeyError).")
    ...
    print("Fitting best QIC approach for each measure...")
    ...
    print("Analysis completed successfully with coverage simulation fix.")
    logging.info("Analysis completed successfully with coverage simulation fix.")
