...
from openpyxl.styles import PatternFill  # <-- For red fill color
...
def pairwise_and_cld(df, sentiment, measure, struct, scale_name):
    ...
    # 3) Build CLD => BLANK second column
    # We'll produce a table with 2 columns: "MediaCategory" + "CLD"
    # but the "CLD" column is always empty.
    cld_df=build_cld_blank(cats)

    return summary_txt, pair_df, cld_df

def build_cld_blank(category_index):
    """
    Just returns a two-column DataFrame:
      "MediaCategory" + "CLD"
    but the CLD column is always blank.
    """
    rows=[]
    for c in CLD_ORDER:
        if c in category_index:
            rows.append((c, ""))
        else:
            rows.append((c, ""))
...
def compile_results_into_multiple_workbooks(
    ...
    # 3) analysis_gee => best QIC w/ pairwise + blank CLD
    with pd.ExcelWriter(gee_excel, engine="openpyxl") as writer:
        ...
        for i, row in df_best_qic.iterrows():
            ...
            # the header is at row sr, data starts at row sr+1
            # pair_df has len(pair_df) data rows
            for row_idx in range(len(pair_df)):
                if pair_df.loc[row_idx,"reject_H0"] == True:
                    excel_row = sr+1 + row_idx  # 1-based row
                    # fill ENTIRE row with red
...
def main():
    setup_logging()
    logging.info("Starting best QIC GEE approach with BH pairwise + BLANK CLD + reject_H0 in red.")
    ...
    # 5) best QIC approach
    print("Fitting best QIC approach for each sentiment–measure (with pairwise + blank CLD).")
