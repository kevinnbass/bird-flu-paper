...
def fit_lmm_and_posthoc(df, sentiment, measure_type='Quotation'):
    ...
    if not mdf.converged:
        ...
        print(f"LMM did not converge for {sentiment}-{measure_type}. Inspect outlets and variation above.")
    ...
    if "Random effects covariance is singular" in summ_text:
        ...
        print(f"Warning: Random effects covariance is singular for {sentiment}-{measure_type}. Check for outlets with no variation and consider removing them.")
...
def compile_into_single_combined_workbook(aggregated_df, stats_df, raw_df, lmm_results, plots_dir):
    ...
    # If any fields are lists, convert them to strings to avoid errors:
