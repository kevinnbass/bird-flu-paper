...
Hence, we produce twice the analyses: original vs. collapsed categories.

All functions are shown in full, including:
- chunk & stats
- correlation
- GEE & Mancl & DeRouen bootstrap
- sensitivity run merging small clusters
- 'Compare_Imbalance' tab with single-run vs. cluster-bootstrapped SE
"""
...
def map_media_outlet_to_category(df, cat_dict):
    """
    Map outlets to 'media_category' using the dictionary cat_dict 
    (either the original 6-cat scheme or the 4-cat collapsed scheme).
    """
    cat_map={}
...
def aggregate_sentiment_scores(df, sentiments):
    recs=[]
    # We'll just iterate over df["media_category"].unique() to gather 
...
###############################################################################
# 5) Mancl & DeRouen Correction + bootstrap
###############################################################################
...
###############################################################################
# residual checks, pairwise diffs
###############################################################################
...
###############################################################################
# exclude_small_clusters => merges tiny clusters
###############################################################################
...
###############################################################################
# pairwise_and_diagnostics => compare single-run robust SE & M&D bootstrap
###############################################################################
...
###############################################################################
# compile => produce Compare_Imbalance
###############################################################################
def compile_results_into_multiple_workbooks(
    ...
    # We now open GEE => produce "All_Combos", "Diagnostics", etc.
    with pd.ExcelWriter(gee_excel, engine="openpyxl") as writer:
        ...
        # We'll re-fit or we can just do a final loop:
    ...
    if not ci_df.empty:
        ...
        for r_i, row_val in enumerate(ci_rows, start=1):
            for c_i, cell_val in enumerate(row_val, start=1):
...
def run_pipeline_for_df(
    ...
    # sensitivity => exclude_small_clusters => prefix_sens
    if prefix:
    ...
    if len(df_sens)<2:
        print(f"[Sensitivity] => Merging <{min_size} rows => not enough data => skip.")
        return
...
###############################################################################
# MAIN => does original cats => yes/all, then collapsed => yes_collapsed/all_collapsed
###############################################################################
def main():
    setup_logging()
    ...
    if "high_rate_2" in df_collapsed.columns:
        ...
        print("No 'high_rate_2' => skip yes subset (collapsed)")
