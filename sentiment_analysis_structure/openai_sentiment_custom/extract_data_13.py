#!/usr/bin/env python3
# extract_data_10_fixed_no_forceblank.py
"""
...
# --------------------------------------------------------------------------------
# 3) unify_rows_across_measures
#    => gather data for each measure, grouped by media category, with NO forced blanking
# --------------------------------------------------------------------------------
...
def unify_rows_across_measures(df, sentiment, measure_group):
    ...
    or ["Title"].  
    We gather values across these measures, for each media category, with NO forced blanking.
    """
    cat_names = ["Scientific","Left","Lean Left","Center","Lean Right","Right"]
    measure_data={}
    ...
    # fill the measure_data structure
    for meas in measure_group:
...
def create_50tabs_scatter(df, wb):
    ...
    grouped as follows:
      ...
      [Quotation_Intensity, Fulltext_Intensity, Title_Intensity],
      [Title]  (standalone).
    We do NOT forcibly blank anything.
    """
    ...
    measure_alignment_groups = [
        ["Quotation","Fulltext","Title_Intensity"],
        ["Quotation_Intensity","Fulltext_Intensity","Title_Intensity"],
        ["Title"],  # single measure group for raw Title
    ]
...
def create_20tabs_scatter(df, wb_20):
    ...
    for sent in SENTIMENTS:
        ...
        row_dataA = [out_colsA]  # header row
        ...
        row_dataB = [out_colsB]  # header row
