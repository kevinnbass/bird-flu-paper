...
A complete Python script that:
  ...
  3) Runs the entire pipeline FOUR times:
       (A) Original categories => "Yes" subset
       (B) Original categories => "All" (full dataset)
       (C) Collapsed categories => "Yes_collapsed"
       (D) Collapsed categories => "All_collapsed"

Hence:
 - 6-category runs => prefixes: Yes, All
 - 4-category runs => prefixes: Yes_collapsed, All_collapsed

Each run outputs multiple Excel files, e.g.:
    analysis_main_Yes.xlsx,
    analysis_main_All.xlsx,
    analysis_main_Yes_collapsed.xlsx,
    analysis_main_All_collapsed.xlsx
...plus raw/gee/plots/combined variants.
"""
...
def bootstrap_gee_md(df, formula, group_col,
    ...
    for each replicate. We store parameter estimates + M&D-corrected SEs.
    If a replicate yields a different number of parameters, we skip it
    (to avoid the “6 columns passed...” error).
    """
    ...
    param_index_full = None  # from first successful replicate
...
def refit_best_gee_with_scale(df, sentiment, measure, fam_name, struct, scale_name):
    ...
    # 1) define _score_col from the chosen measure
    ...
    # 4) re-fit if scale is specified
...
def run_pipeline_for_df(
    ...
    """
    If prefix=="Yes", we collapse each category => single cluster for that subset.
    If prefix=="All", keep the original cluster => media_outlet_clean.
    If prefix ends with _collapsed, we are using the 4-cat approach, but the code is the same.
    Outputs multiple Excel files appended with `prefix`.
    Also merges small clusters for a sensitivity run => <prefix>_Sens
    """
    validation_records=[]
    ...
    # 8) sensitivity => merges small clusters => <prefix>_Sens
    if prefix:
