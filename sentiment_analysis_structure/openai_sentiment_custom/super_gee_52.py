...
def try_all_families_and_scales(df, formula, group_col):
    ...
    - If any step fails => QIC=NaN for that combo
    ...
    for fam in families:
        ...
        for cov_obj in cor_structs:
            ...
            for sc_opt in scale_opts:
                qic_val = float("nan")
                status_flag = "ok"  # or "fail" if any error
                try:
                    ...
                    # 3) If scale_opt != "none", compute scale & re-fit
                    ...
                    if sc_opt != "none" and dfresid > 0:
                        ...
                        # If scale is valid => second pass
                        if (val is not None) and (not np.isnan(val)):
                            final_res = model.fit(maxiter=300, scale=val)
                        else:
                            # If scale is invalid => treat as fail => skip
                            raise ValueError("Invalid or NaN scale factor => fail")

                    # If we get here without error, we have a final_res
                    qic_val = final_res.qic()
                    if isinstance(qic_val, tuple):
                        qic_val = qic_val[0]

                    # 4) If QIC is valid & better than current best => update
    ...
    # best_tuple => (Family, CovStruct, Scale, bestQICVal)
...
def pairwise_and_diagnostics(
    ...
    A comprehensive function that:
      ...
      5) Applies BH correction to p-values => p_value_adj.
      6) Builds 'DiffIDs' for significant pairs and 'Trans_DiffIDs' with the mapping 1->3,2->1,3->5,4->2,5->6,6->4.
      7) Returns an 8-tuple:

         (
             summary_txt,        # textual GEE summary
             pair_df,            # pairwise table with p-values, BH-corrected p_value_adj
             diffIDs_df,         # includes "DiffIDs" and "Trans_DiffIDs"
             diag_dict,          # from check_residuals_and_correlation
             pseudo_dict,        # from pseudo_likelihood_check
             cv_val,             # MSE from cross_validation_gee
             sens_df,            # from sensitivity_analysis_correlation
             boot_info           # combined info from cluster bootstrap + jackknife
         )
    """
    ...
    if final_fit is None:
        # If refitting fails, return None so we skip further steps
        return (None, None, None, None, None, None, None, None)

    summary_txt = final_fit.summary().as_text()

    # -------------------------------------------------------------------
    # 2) Checks: residuals, pseudo-likelihood, cross-validation, sens
    # -------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------
    # These calls each return two Pandas Series: (means, sds).
    ...
    # Build dictionary => param means/std from each approach
    # Note: 'boot_params_df' and 'jk_params_df' are actually Series of parameter means.
    if not boot_params_df.empty:
        param_mean_boot = boot_params_df.round(4).to_dict()
        param_std_boot  = boot_se_df.round(4).to_dict()
    else:
        param_mean_boot = {}
        param_std_boot  = {}

    if not jk_params_df.empty:
        param_mean_jk = jk_params_df.round(4).to_dict()
        param_std_jk  = jk_se_df.round(4).to_dict()
    else:
        param_mean_jk = {}
        param_std_jk  = {}
    ...
    #    for *all* categories (including reference).
    # -------------------------------------------------------------------
    ...
    def get_index(cat):
        # Returns the parameter index if found; else None => param=0
        return idx_map.get(cat, None)
    ...
    # -------------------------------------------------------------------
    return (
        summary_txt,       # 1) textual summary (GEE summary)
        pair_df,           # 2) pairwise table (with p_value, p_value_adj, reject_H0)
        diffIDs_df,        # 3) includes DiffIDs + Trans_DiffIDs
        diag_dict,         # 4) residual checks
        pseudo_dict,       # 5) pseudo-likelihood check
        cv_val,            # 6) cross-validation MSE
        sens_df,           # 7) sensitivity analysis
        boot_info          # 8) dict with cluster + jackknife param means/std
    )
...
def compile_results_into_multiple_workbooks(
    ...
    with pd.ExcelWriter(gee_excel, engine="openpyxl") as writer:
        ...
        for i, row in df_best_qic.iterrows():
            ...
            # -- FIXED: Properly reference param means/stds from boot_info
