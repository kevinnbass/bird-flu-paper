...
Script that:
  ...
  6) For each best QIC model:
     ...
     - Build CLD based on "start with sorted means" letter assignment
...
# ------------------------------
# 6) Pairwise + CLD (BH + new letter assignment)
# ------------------------------
def refit_best_gee_with_scale(df, sentiment, measure, struct, scale_name):
    ...
    if scv is None or np.isnan(scv):
        return bres
    return base_model.fit(scale=scv)
...
def pairwise_and_cld(df, sentiment, measure, struct, scale_name):
    ...
    # 3) Build CLD
    cld_df = build_cld(final_fit, pair_df)
    return summary_txt, pair_df, cld_df


# ------------------------------
# 7) The new letter assignment function
# ------------------------------
def build_cld(fit_result, pair_df):
    """
    Build letter groupings using a multi-letter iterative approach:
      - Sort categories by ascending mean
      - If two categories are NOT different (reject_H0=False), they share >=1 letter
      - If two categories ARE different (reject_H0=True), they share 0 letters
      - Repeatedly enforce constraints until stable

    pair_df must have columns ["CategoryA","CategoryB","reject_H0"] at minimum
    (plus any others). We'll extract 'reject_H0' to see if they differ or not.
    """

    # -- 1) Sort categories by ascending mean
    ...
    sorted_cats = sorted(cats, key=lambda c: mean_map[c])

    # -- 2) Build differ dict => differ[A][B] = True if "they differ"
    differ = {c: {} for c in sorted_cats}
    for row in pair_df.itertuples(index=False):
        A, B = row.CategoryA, row.CategoryB
        # Just read 'reject_H0'
        they_differ = row.reject_H0
        differ[A][B] = they_differ
        differ[B][A] = they_differ

    # -- 3) Initialize letter sets to empty
    ...
    def ensure_share_letter(cA, cB):
        """If cA and cB have no letter in common, add or share one."""
        nonlocal used_letters
        common = cat_letters[cA].intersection(cat_letters[cB])
        if not common:
            # They must share >=1 letter => we can try to share any letter from cA or cB
            possible_share = cat_letters[cA].union(cat_letters[cB])
            if possible_share:
                letter_to_add = next(iter(possible_share))
    ...
    def ensure_no_shared_letter(cA, cB):
        """If cA and cB differ, they share 0 letters => remove intersection."""
    ...
    # -- 4) Iterative constraint enforcement
    changed = True
    iteration = 0
    max_iter = 100
    ...
    while changed and iteration < max_iter:
        ...
        for i in range(len(sorted_cats)):
            for j in range(i+1, len(sorted_cats)):
                cA = sorted_cats[i]
                cB = sorted_cats[j]
                they_differ = differ[cA].get(cB, False)
    ...
    # -- 5) Convert sets to sorted letter strings, reorder by CLD_ORDER
    from gee_final_best_qic_with_pairwise_cld import CLD_ORDER  # or just use the global

    # (But we already have CLD_ORDER in the global namespace, so we can directly reference it)
    rows = []
    for c in CLD_ORDER:
        if c in cats:
...
def compile_results_into_multiple_workbooks(
    ...
    # 3) analysis_gee => best QIC w/ pairwise + CLD
...
def main():
    setup_logging()
    logging.info("Starting best QIC GEE approach with BH post-hoc & new CLD system (final).")
    ...
    # 5) best QIC approach
    print("Fitting best QIC approach for each sentiment–measure (with BH pairwise + new CLD).")
