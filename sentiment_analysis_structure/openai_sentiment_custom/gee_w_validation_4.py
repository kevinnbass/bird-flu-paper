...
Fixes the "IndexError: invalid index to scalar variable" by checking length=1 arrays
in the correlation loop and assigning np.nan or skipping that step.
"""
...
def analyze_quotation_fulltext_correlation(df):
    ...
    # create "quotation_fulltext_correlation.xlsx"
...
def check_residuals_and_correlation(final_fit):
    """
    If arr has length=1, cmat is a scalar => skip or assign np.nan.
    If arr has length=2, cmat => 2x2 => cmat[0,1].
    If arr has length>2, we average off-diagonal.
    """
    y=final_fit.model.endog
    mu=final_fit.fittedvalues
    # to numpy array to avoid indexing issues
    ...
    for cid, arr in cluster_map.items():
        arr=np.array(arr)
        if len(arr)==1:
            # only one data point => can't do correlation => skip or do np.nan
            continue
        elif len(arr)==2:
            ...
            cmat=np.corrcoef(arr)
            n2=len(arr)
            # sum of all minus diagonal => sum_offdiag
            # cmat is n2 x n2 => diagonal=1's
...
def compile_results_into_multiple_workbooks(
    ...
    # 3) analysis_gee => best QIC => pairwise => diffIDs => new checks
