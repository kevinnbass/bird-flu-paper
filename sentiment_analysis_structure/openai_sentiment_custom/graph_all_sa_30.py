...
def fit_lmm_and_posthoc(df, sentiment, measure_type='Quotation'):
    ...
    if "Random effects covariance is singular" in summ_text:
        logger.warning("Random effects covariance singular message found.")
...
def debug_singular_matrix(model_df, formula, sentiment, measure_type):
    ...
    # We'll just do a quick check using patsy dmatrices
...
def run_lmm_analyses(df):
    all_results = {}
    for sentiment in CATEGORIES:
        # Debug step: Examine random effects variability
        print(f"[DEBUG] Examining random effects variability for Quotation measure of '{sentiment}'")
...
def compile_results_into_multiple_workbooks(aggregated_df, stats_df, raw_df, lmm_results, plots_dir,
    ...
    # COMBINED EXCEL
    # We'll create a combined workbook with aggregated, stats, raw (cleaned), LMM summaries, and plots
    # For simplicity, just replicate the logic from before
    from openpyxl.utils.dataframe import dataframe_to_rows

    # Clean raw data for embedding
...
def remove_homogeneous_groups(df, threshold=0.0001):
    # Determine variation per outlet in the response measure
    # We'll pick 'joy_quotation_mean' as a sample measure or we can pick a generic approach
    # Actually, let's compute a representative measure:
    # We'll consider all sentiments and fulltext/quotation means. If no measure computed yet, compute a simple one.
    # For simplicity, let's just pick one measure to assess variation: e.g., 'joy_quotation_mean'.
    # If it doesn't exist, create it.
    # In a real scenario, you'd pick the outcome of interest or do multiple checks.

    print("Checking variance by outlet...")
    # Let's pick 'joy_quotation_mean' measure:
    sentiment = 'joy'
    pattern = f"^{re.escape(sentiment)}_\\d+$"
    matched_cols = [col for col in df.columns if re.match(pattern, col)]
    if matched_cols:
        df['joy_quotation_mean'] = df[matched_cols].clip(lower=0).mean(axis=1)
        score_col = 'joy_quotation_mean'
    else:
        # If no matched cols for joy, pick another sentiment that exists:
        # or fallback to first sentiment that exists:
        fallback_score_col = None
        for s in CATEGORIES:
            pattern = f"^{re.escape(s)}_\\d+$"
            mc = [c for c in df.columns if re.match(pattern, c)]
            if mc:
                df[f'{s}_quotation_mean'] = df[mc].clip(lower=0).mean(axis=1)
                fallback_score_col = f'{s}_quotation_mean'
                break
        if fallback_score_col is None:
            # No matched cols at all? Just return df as is
            print("No sentiment quotation columns found for variance check. Skipping variance-based filtering.")
            return df
        else:
    ...
    if low_var_outlets:
        print(f"Removing {len(low_var_outlets)} outlets due to near-zero variance in the response measure (threshold={threshold}):")
        for o in low_var_outlets:
            print(f" - {o}")
        df = df[~df['media_outlet'].isin(low_var_outlets)]
    else:
        print("No outlets removed based on near-zero variance criteria.")
    return df
...
def main():
    ...
    # Remove homogeneous groups based on a chosen threshold
    # Criteria: standard deviation of less than 0.0001
    df = remove_homogeneous_groups(df, threshold=0.0001)
    ...
    # Identify previously found insufficient variation outlets if any:
    # Let's suppose we previously identified 'DailyWire' and 'Mother Jones'
    # But we already removed them in homogeneous filtering step above.

    # For demonstration, assume we have recorded them somewhere. If not, we just proceed.
    # We'll run a second pass after removing these identified outlets again (though we already did).
    print("\n--- Running analysis on filtered dataset ---")
    # Just use the df that already had low variance outlets removed (df).
    # In a real scenario, if we wanted a second pass, we might have saved df before and do more filtering now.
    # We have already done that step above, so let's just do the same steps on the df again for demonstration.

    df_filtered = df.copy()  # It's already filtered
    ...
    save_aggregated_scores_to_csv(aggregated_df_filtered, CSV_OUTPUT_DIR_FILTERED, prefix='aggregated_sentiment_emotion_scores.csv')

    plot_statistics(aggregated_df_filtered, OUTPUT_DIR_FILTERED)

    lmm_results_2 = run_lmm_analyses(df_filtered)

    compile_results_into_multiple_workbooks(aggregated_df_filtered, calculate_mean_median(aggregated_df_filtered), df_filtered, lmm_results_2, OUTPUT_DIR_FILTERED,
                                            OUTPUT_EXCEL_MAIN_FILTERED, OUTPUT_EXCEL_RAW_FILTERED,
                                            OUTPUT_EXCEL_LMM_FILTERED, OUTPUT_EXCEL_PLOTS_FILTERED, OUTPUT_EXCEL_COMBINED_FILTERED)
