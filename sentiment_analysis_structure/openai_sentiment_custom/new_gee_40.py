...
#########################################
# Configuration and file definitions
#########################################

INPUT_JSONL_FILE = 'processed_all_articles_fixed_2.jsonl'  # or whichever input
...
def setup_logging(log_file='analysis.log'):
    """Sets up file+console logging and suppresses font debug logs."""
...
def map_media_outlet_to_category(df, media_categories):
    if 'media_outlet' not in df.columns:
        raise KeyError("'media_outlet' column not found in df!")
    out_map={}
...
def aggregate_sentiment_scores(df, sentiment_categories):
    """Sum + count for Quotation & Fulltext in each category."""
    rows=[]
...
##############################
#   GEE + QIC with scale=1
##############################

def remove_zero_var_groups(df, score_col):
    """Remove any groups with zero variance in the outcome to reduce numeric issues."""
    gvar= df.groupby('media_outlet')[score_col].var()
    zero_var= gvar[gvar==0].index
    if len(zero_var)>0:
        logging.debug(f"Removing {len(zero_var)} zero-variance groups for {score_col}: {list(zero_var)}")
        df= df[~df['media_outlet'].isin(zero_var)].copy()
    return df


def fit_gee_model(df, formula, groups, cov_struct, family=Poisson(), time=None):
    """
    Attempt GEE with scale=1 explicitly. This may yield warnings if statsmodels
    doesn't accept scale=1, but we do it anyway as requested.
    """
    logging.debug(f"fit_gee_model => cov_struct={cov_struct.__class__.__name__}, scale=1, time={'YES' if time is not None else 'NO'}")
    try:
        # The key difference: pass scale=1
        model= GEE.from_formula(formula, groups=groups, data=df, cov_struct=cov_struct,
                                family=family, time=time, scale=1)
    except Exception as e:
...
def run_gee_and_qic(df, sentiment, measure_type, structures, min_date=None):
    """Fit multiple structures => compare QIC => {struct_name: (res, qic_val)}."""
    ...
    for nm, stobj in structures.items():
        ...
        logging.debug(f"Fitting structure={nm}, scale=1, #rows={len(mdf)}, time={'YES' if used_time is not None else 'NO'}")
        res= fit_gee_model(mdf, formula, groups, stobj, time=used_time)
        if res is None:
            logging.debug(f"Fit => None => skip {nm}")
            continue
        # QIC might be tuple => pick first
        try:
            qic_ = res.qic()
        ...
        logging.debug(f"{nm} => QIC={qic_val}")
        result_map[nm]= (res,qic_val)
...
def fit_and_pick_best_structure(df, sentiment, measure_type, min_date=None):
    ...
    best_q= np.inf
    all_info= []
    for nm,(res,qv) in rmap.items():
        all_info.append((nm,qv))
...
def compile_results_into_multiple_workbooks(aggregated_df, stats_df, raw_df, gee_results,
    ...
    with pd.ExcelWriter(lmm_excel, engine='openpyxl') as w:
        ...
        for s in gee_results:
            for measure_type in gee_results[s]:
                ...
                qic_lst= gee_results[s][measure_type]['QIC_All']
                summ_txt= gee_results[s][measure_type]['GEE_Summary']
                pwdf= gee_results[s][measure_type]['Pairwise']

                summ_df= pd.DataFrame({'GEE_Summary': summ_txt.split('\n')})
                summ_df.to_excel(w, sheet_name=sht_nm, index=False)
                rstart= len(summ_df)+2
                info_df= pd.DataFrame({
                    'Info':[f"BestStructure={best_st}", f"AllStructuresAndQIC={qic_lst}"]
                })
    ...
    for s in CATEGORIES:
        ...
        if os.path.exists(qp):
            ...
            try:
                ...
                logging.error(f"Error embedding quote {qp}: {e}")
        ...
        if os.path.exists(fp):
            ...
            try:
                ...
                logging.error(f"Error embedding fulltext {fp}: {e}")

    wb.save(plots_excel)

    # combined
    raw_df_clean= raw_df2.copy()
    for c in raw_df_clean.columns:
        if raw_df_clean[c].dtype==object:
            raw_df_clean[c]= raw_df_clean[c].apply(lambda x: ", ".join(x) if isinstance(x,list) else x)
    ...
    for r in dataframe_to_rows(aggregated_df, index=False, header=True):
        ws_agg.append(r)
    ws_stat= wb2.create_sheet("Mean_Median_Statistics")
    ...
    for s in gee_results:
        for m in gee_results[s]:
            best_st= gee_results[s][m]['BestStructure']
            summ= gee_results[s][m]['GEE_Summary'].split('\n')
            snippet= "\n".join(summ[:6])
            ws_gee.append([s,m,best_st,snippet])
...
def main():
    setup_logging()
    print("Single run with GEE + QIC, Poisson family, scale=1, plus removing zero-variance groups.")
    ...
    print("Fitting GEE models with multiple correlation structures (QIC) => best structure selection, scale=1.")
    ...
    for s in CATEGORIES:
        if s in gee_results:
            for m in gee_results[s]:
                best_str= gee_results[s][m]['BestStructure']
                print(f"For sentiment='{s}', measure='{m}', best structure => {best_str}")
