...
def aggregate_measures(df, measures):
    ...
    for cat in MEDIA_CATEGORIES.keys():
        ...
        for m in measures:
            if m in sub.columns:
                # Force values to numeric, setting invalid to NaN, then drop them
...
def run_gee_for_measure_best_qic(df, measure):
    ...
    # Drop missing for critical columns
    d2 = d2.dropna(subset=["media_outlet_clean","media_category", measure])
    if len(d2) < 2 or d2["media_category"].nunique() < 2:
        return None

    # Force measure to numeric before clipping
    d2[measure] = pd.to_numeric(d2[measure], errors="coerce")
    d2 = d2.dropna(subset=[measure])  # remove newly formed NaNs
    if len(d2) < 2 or d2["media_category"].nunique() < 2:
        return None

    # Now clip below 0 if desired
...
def refit_best_gee_with_scale(df, measure, fam_name, struct, scale_name):
    ...
    # Also force numeric here, just in case
    d2[measure] = pd.to_numeric(d2[measure], errors="coerce")
    d2 = d2.dropna(subset=["media_outlet_clean","media_category", measure])
    if len(d2)<2 or d2["media_category"].nunique()<2:
        return None, None

    # We can clip or not, as desired
    d2["_score_col"] = d2[measure]  # or d2[measure].clip(lower=0)
