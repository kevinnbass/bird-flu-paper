...
OUTPUT_EXCEL_CORRELATION = 'analysis_correlation_data.xlsx'  # correlation data
OUTPUT_EXCEL_COMBINED_ALL = 'analysis_combined_all.xlsx'  # new file for combined normalized data
...
def main():
    ...
    # Combine all sentiments into one DataFrame for a single scatter plot with normalization
    combined_list = []
    for sentiment, sdata in correlation_scatter_data.items():
        df_temp = sdata.copy()
    ...
    # Normalize each sentiment's scores (z-score)
    ...
    # Save combined_all data to an xlsx
    combined_all.to_excel(OUTPUT_EXCEL_COMBINED_ALL, index=False)

    # Compute overall Pearson correlation for combined_all (Quotation_Z vs Fulltext_Z)
    # across all sentiments combined.
    ...
    # Create scatter plot (all points black, no legend, with trendline, display R)
    plt.figure(figsize=(10, 8))
    # Use regplot for trendline and single color
    sns.regplot(x='Quotation_Z', y='Fulltext_Z', data=combined_nonan, scatter_kws={'color':'black'}, line_kws={'color':'red'})
    # Remove legend if any (no hue so no legend by default)
    ...
    # No legend call needed as no hue is used
