...
Fixes:
  ...
  - Gamma(link=ln.log()) instead of Gamma(link=ln.log)
"""
...
# We import families + NB alpha=1, etc.
from statsmodels.genmod.families import (
    Poisson, NegativeBinomial,
    Gaussian, Gamma, InverseGaussian
)
# Now we import links as ln, so we can do Gamma(link=ln.log())
import statsmodels.genmod.families.links as ln
...
# NEW: needed to highlight rows in red
from openpyxl import Workbook
from openpyxl.drawing.image import Image as ExcelImage
from openpyxl.utils.dataframe import dataframe_to_rows
from openpyxl.styles import PatternFill   # <-- crucial import to fix NameError
...
def try_all_families_and_scales(df, formula, group_col):
    families = [
        ...
        Gaussian(),
        Gamma(link=ln.log()),           # <-- log() instance
        InverseGaussian()
    ]
    cor_structs = [Independence(), Exchangeable()]
    scale_opts = ["none","pearson"]  # you can expand if needed
...
def run_gee_for_sentiment_measure_best_qic(df, sentiment, measure):
    ...
    best_tuple=try_all_families_and_scales(d2, "_score_col ~ media_category","media_outlet_clean")
...
def refit_best_gee_with_scale(df, sentiment, measure, fam_name, struct, scale_name):
    ...
    if fam_name=="Poisson":
        ...
        fam_obj=Gamma(link=ln.log())  # ensure log link
...
def pairwise_and_cld(df, sentiment, measure, fam_name, struct, scale_name):
            ...
            for row_idx in range(len(pair_df)):
                if pair_df.loc[row_idx,"reject_H0"]:
                    ...
                    for col_idx in range(1, pair_df.shape[1] + 1):
                        ...
                        # Fix => import PatternFill from openpyxl.styles
...
def main():
    setup_logging()
    logging.info("Starting best QIC GEE approach with multiple families + log link for Gamma + PatternFill fix.")
    ...
    print("Fitting best QIC approach for each sentiment–measure (multi-families + BH pairwise).")
    ...
    print("Analysis completed successfully (with Gamma(link=log()) and PatternFill fix).")
