...
#         Single Prompt          #
# ------------------------------ #
# Note: Double braces for literal JSON braces so Python's format() won't treat them as placeholders.
PROMPT_SPREAD_RISK = """
...
- Evolve become more transmissible,
- Cause a pandemic,
- Become more infectious or deadly to humans,
...
1) Which criterion or criteria above that the article fulfills, if any; and
...
def get_next_output_file(base_prefix="spread_subset_test"):
    ...
    If 2 exists, try 3, if 3 exists, try 4, etc.
    """
    n = 2
    while True:
...
def main():
    """
    1) Find the latest 'spread_subset_test_{n}.jsonl' file if it exists.
       If none exist, we can't proceed - user presumably wants to start from v2, etc.
    2) Extract all 'id' fields from that file.
    3) Create the next version file (e.g. if _2 exists, we create _3).
    4) For each ID, find the matching article in distribution_mortality_subset.jsonl, 
       run the API call using the updated prompt, and write results to the new file
       while showing a progress bar.
    5) Compare the new file with all older ones, listing matches & mismatches.
    """

    # Step 1: Find the latest existing version file
    latest_file = find_latest_output_file(base_prefix="spread_subset_test")
    if not latest_file:
        print("No existing 'spread_subset_test_{n}.jsonl' found. Please create one first.")
        return
    print(f"Latest existing file: {latest_file}")

    # Step 2: Collect IDs from that file
    last_ids = []
    try:
    ...
    # Step 3: Get the next version file name
    new_file = get_next_output_file(base_prefix="spread_subset_test")
    print(f"Creating new file: {new_file}")

    # Step 4: For each ID from the latest file, find matching article in distribution_mortality_subset.jsonl
    # We'll do a dictionary lookup to store them first
    ...
    # Identify if any IDs are missing from distribution
    missing_ids = last_ids_set - set(articles_dict.keys())
    if missing_ids:
        print("WARNING: The following IDs from the latest file were not found in distribution_mortality_subset.jsonl:")
    ...
    # We'll do API calls for each found ID, in the same order as in latest_file
    processed_articles = []
    for_missing_count = 0

    # Using tqdm progress bar
    print("Processing articles from distribution_mortality_subset.jsonl...")
    for aid in tqdm(last_ids, desc="Processing articles"):
        if aid not in articles_dict:
            # we skip if not found
            for_missing_count += 1
            continue
    ...
    with open(new_file, 'a', encoding='utf-8') as nf:
        for record in processed_articles:
            nf.write(json.dumps(record, ensure_ascii=False) + "\n")

    print(f"\nCreated {new_file} with {len(processed_articles)} articles processed.")
    if for_missing_count > 0:
        print(f"Note: {for_missing_count} articles were missing in {INPUT_FILE} and were not processed.")

    # Step 5: Compare new file to all older versions (besides itself)
