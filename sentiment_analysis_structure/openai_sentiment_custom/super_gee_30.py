...
Any run with "_collapsed" in its prefix is labeled
"analysis_XXX_collapsed_YYY.xlsx".
...
**CHANGES**:
 - In the Aggregated_Scores tab, SD and SEM are now computed and included, for both
   Quotation and Fulltext.  
"""
...
def aggregate_sentiment_scores(df, sentiments):
    """
    For each media_category x sentiment, we compute:
      - Quotation_Sum, Quotation_Count
      - Fulltext_Sum, Fulltext_Count
      - Quotation_SD, Quotation_SEM
      - Fulltext_SD, Fulltext_SEM
    We do this by collecting the raw (per-article) values in that category
    so that standard deviation (SD) and standard error (SEM) can be computed.
    """
    recs=[]
    ...
    for cat in cats_in_df:
        ...
        for s in sentiments:
            # All Quotation columns for e.g. "joy_0", "joy_1", etc.
            pat = rf"^{re.escape(s)}_\d+$"
            matched = [c for c in sub.columns if re.match(pat,c)]

            if matched:
                # Flatten all matched columns into one array, after clipping
            ...
            # Single Fulltext column (e.g. "joy_fulltext")
            fcol = f"{s}_fulltext"
...
def check_residuals_and_correlation(final_fit):
    ...
    for cid, arr in cluster_map.items():
        ...
        if len(arr) > 1:
            ...
            # 1) Check that cmat is truly 2D (ndim == 2)
            # 2) Check that cmat.shape[0] > 1, ensuring at least 2 rows
