        ...
        for sentiment in sentiment_categories:
            ...
            if matched_cols:
                ...
                if negative_quot_count > 0:
                    logging.warning(f"{negative_quot_count} negative quotation-based scores found for '{sentiment}' in '{media_category}'. Setting them to 0.")
            ...
            if fulltext_col in df.columns:
                ...
                if negative_full_count > 0:
                    logging.warning(f"{negative_full_count} negative fulltext-based scores found for '{sentiment}' in '{media_category}'. Setting them to 0.")
...
def fit_lmm_and_posthoc(df, sentiment, measure_type='Quotation'):
    ...
    if outlets_with_variation_issues:
        for outlet, uniq_count in outlets_with_variation_issues:
            ...
            logger.debug(f"Outlet '{outlet}' unique values: {model_df.loc[model_df['media_outlet'] == outlet, score_col].unique()}")
            logging.info(f"Outlet '{outlet}' may have insufficient variation.")
    ...
    # Check random effects from the model result to see if variance is zero
    # This is a heuristic: If random effects variance is zero or near zero, that might cause singular warnings.
    if hasattr(mdf, 'random_effects'):
        # If any random effect is constant across outlets, we might remove those outlets.
        pass  # We already track problematic outlets above.
    ...
    if "Random effects covariance is singular" in summ_text:
        ...
        # If singular, we can't directly identify which outlet. We rely on previously identified outlets.
...
def main():
    ...
    try:
        ...
        logging.info("Mean/Median stats calculated.")
    except Exception as e:
    ...
    try:
        ...
        logging.info("Scores saved to CSV.")
    except Exception as e:
        print(f"Error saving aggregated scores to CSV: {e}")
        logging.error(f"Error saving aggregated scores to CSV: {e}")
        return
    ...
    try:
        ...
        logging.info("LMM done.")
    except Exception as e:
        print(f"Error in LMM analysis: {e}")
        logging.error(f"LMM error: {e}")
        return
    ...
    try:
        ...
        logging.info("Combined workbook done.")
    except Exception as e:
        print(f"Error compiling single combined Excel file: {e}")
        logging.error(f"Combined file error: {e}")
    ...
    if problematic_outlets:
        ...
        # Filter these problematic outlets and re-run analysis
        ...
        compile_results_into_multiple_workbooks(filtered_aggregated_df, filtered_stats_df, df_filtered,
                                                filtered_lmm_results, filtered_output_dir,
        ...
        compile_into_single_combined_workbook(filtered_aggregated_df, filtered_stats_df, df_filtered,
                                              filtered_lmm_results, filtered_output_dir,
