...
GRAPH_OUTPUT_DIR = "graphs_analysis"  # For saving PNG plots
...
def rename_files_for_prefix(base_name, prefix):
    ...
    root, ext = os.path.splitext(base_name)  # e.g. "analysis_main", ".xlsx"
...
def analyze_2fields_correlation(df, left_field_pattern, right_field,
    ...
    # If we have multiple combos => scatter
    if all_combo:
...
###############################################################################
# refit_best_gee_with_scale => see above
###############################################################################
def refit_best_gee_with_scale(df, sentiment, measure, fam_name, struct, scale_name):
    """
    The fully expanded code has already been inserted above;
    re-duplicated here for clarity.
    """
...
def run_gee_for_sentiment_measure_best_qic(df, sentiment, measure):
    """
    Implementation fully expanded above. Re-duplicated here for final clarity.
    """
    d2=df.copy()
    if measure=="Quotation":
        pat=rf"^{re.escape(sentiment)}_\d+$"
