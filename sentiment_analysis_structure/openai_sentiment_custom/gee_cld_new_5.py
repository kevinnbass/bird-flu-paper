...
Script that:
  ...
  6) For each best QIC model:
     ...
     - Build a CLD table with 4 columns => [MediaCategory, Mean, UserCLD, AutoCLD]
       using rpy2-based adjacency + multcompView::multcompLetters
...
multcompview = importr('multcompView')  # Must be installed in R
...
###############################################################################
# 6) Pairwise + CLD (via rpy2). We'll raise error if conflicts. We'll catch it.
###############################################################################
def build_cld_with_rpy2(final_fit, pair_df):
    """
    Builds adjacency => calls multcompView::multcompLetters(...)
    If a pair is 'not differ' but shares no letter, we raise RuntimeError => 'R-based CLD conflict: X vs Y...'
    """
    # (See previous code snippets for the full function logic.)
    # We'll assume you've already replaced final_fit.params[0] with .iloc
    # and convert letters result to a dict. 
    # ...
    # For brevity here, let's say the function is the same as the 'fixed' version in prior steps.
    # but you keep the relevant logic. We'll place a simplified version:

    # [ ... Your adjacency building logic ... ]
    # [ ... R code call with compare=">" ... ]
    # [ ... Convert letters to a dict ... ]
    # [ ... Final check => if adjacency=1 => must share >=1 letter => else => raise RuntimeError ... ]
    # [ ... Build DataFrame and return ... ]

    raise NotImplementedError("Use your prior fixed version of build_cld_with_rpy2 here.")

def pairwise_and_cld(df, sentiment, measure, struct, scale_name):
    """
    Refit best GEE, compute pairwise => adjacency => build_cld_with_rpy2 => might raise RuntimeError
    """
    from statsmodels.genmod.generalized_estimating_equations import GEE
    # [ Similar code as your final snippet, just referencing build_cld_with_rpy2... ]
    # ...
    # if conflict => RuntimeError => "R-based CLD conflict..."
    raise NotImplementedError("Use your final snippet referencing build_cld_with_rpy2 here.")


###############################################################################
# 7) Compile results => gracefully handle RuntimeError from CLD
###############################################################################
def compile_results_into_multiple_workbooks(
    ...
    # -------------- Handling RuntimeError in GEE sheets --------------
        ...
        for i, row in df_best_qic.iterrows():
            ...
            try:
                ...
                if summary_txt is None:
                    tmp_df=pd.DataFrame({"Summary":[f"No valid model or not enough data for {s}-{meas}."]})
                ...
                # We caught the "R-based CLD conflict" or something else
                # We'll create a new sheet with the error message
                error_sheet = f"Conflict_{s[:10]}_{meas[:8]}"
                conflict_df = pd.DataFrame({"Error":[str(e)]})
                conflict_df.to_excel(writer, error_sheet, index=False)
                # also log
                logging.error(f"CLD conflict => {e}")
                # skip building the normal output (continue to next iteration)
                continue
...
def main():
    setup_logging()
    logging.info("Starting best QIC GEE approach with BH post-hoc & rpy2-based CLD + conflict handling (no crash).")
