...
Script that:
  ...
  6) For each best QIC model:
     ...
     - Build a CLD table with 4 columns => [MediaCategory, Mean, UserCLD (blank), AutoCLD]
       using the iterative approach in 'build_iterative_cld' below
         * If not differ => share >=1 letter
         * If differ => share 0 letters
         * If any category ends up with 0 letters, force at least 1
         * Then final check => if differ => can't share letter => else raise error
...
###############################################################################
# 6) Our refined build_iterative_cld with final check & forced letter assignment
###############################################################################
def build_iterative_cld(final_fit, pair_df):
    """
    Iterative approach to assign letters so that:
      - If two categories do NOT differ => share >=1 letter
      - If they DO differ => share 0 letters
    We do repeated passes until stable or max_iter=200.
    If any category ends up empty, we assign it a letter.
    Then we do a final check => if differ => can't share letter => else raise error.
    """

    # 1) Gather categories & predicted means
    ...
    # 2) differ[A][B] = True => share 0, else share >=1
    differ = {cat: {} for cat in cats}
    for row in pair_df.itertuples(index=False):
        A, B = row.CategoryA, row.CategoryB
        differ[A][B] = row.reject_H0
        differ[B][A] = row.reject_H0

    # 3) Prepare letter sets
    cat_letters = {c: set() for c in cats}
    letter_pool = [chr(i) for i in range(ord('a'), ord('z')+1)]
    used_letters=0

    def ensure_share_letter(A,B):
        nonlocal used_letters
        common = cat_letters[A].intersection(cat_letters[B])
        if not common:
            union_ = cat_letters[A].union(cat_letters[B])
            if union_:
                letter_to_add = next(iter(union_))
                cat_letters[A].add(letter_to_add)
                cat_letters[B].add(letter_to_add)
            else:
                # both empty => new letter
                if used_letters<len(letter_pool):
                    new_letter=letter_pool[used_letters]
                    used_letters+=1
                    cat_letters[A].add(new_letter)
                    cat_letters[B].add(new_letter)

    def ensure_no_shared_letter(A,B):
        inter = cat_letters[A].intersection(cat_letters[B])
        if inter:
            cat_letters[A] -= inter
            cat_letters[B] -= inter
    ...
    while changed and iteration<max_iter:
        ...
        for i in range(len(cats)):
            for j in range(i+1,len(cats)):
                A,B=cats[i],cats[j]
                beforeA = set(cat_letters[A])
                beforeB = set(cat_letters[B])
                if differ[A].get(B,False):
                    # they differ => share 0
                    ensure_no_shared_letter(A,B)
                else:
                    # share >=1
                    ensure_share_letter(A,B)
                if cat_letters[A]!=beforeA or cat_letters[B]!=beforeB:
                    changed=True

    # 5) Force at least one letter
    for c in cats:
        if len(cat_letters[c])==0:
            if used_letters<len(letter_pool):
                new_letter=letter_pool[used_letters]
                used_letters+=1
    ...
    # 6) Final check => if they differ => can't share letter => else raise error
    #    We'll look in pair_df for stats
    for i in range(len(cats)):
        for j in range(i+1,len(cats)):
            A,B = cats[i], cats[j]
            if differ[A].get(B,False):
                # they differ => must share 0
                conflict=cat_letters[A].intersection(cat_letters[B])
                if conflict:
                    # find row in pair_df => difference, p_value, p_value_adj
                    rowmask = (
                        ((pair_df["CategoryA"]==A)&(pair_df["CategoryB"]==B)) |
                        ((pair_df["CategoryA"]==B)&(pair_df["CategoryB"]==A))
                    )
                    dfrow = pair_df[rowmask].iloc[0]
                    diff_val = dfrow["Difference"]
                    p_val = dfrow["p_value"]
                    p_adj = dfrow.get("p_value_adj",None)
                    stats_msg=(
                        f"Groups {A} and {B} differ but share letter(s) {conflict}. "
                        f"Diff={diff_val}, p={p_val}, p_adj={p_adj}"
                    )
                    raise RuntimeError(
                        "Inconsistent letter assignment after forced assignment. " + stats_msg
                    )

    # 7) Build final => sorted letters => 'a'
    rows=[]
    for c in CLD_ORDER:
        if c in cats:
            sorted_letters=sorted(cat_letters[c])
            auto_str=''.join(sorted_letters)
            pm=mean_map[c]
            rows.append((c, pm, "", auto_str))
        else:
            rows.append((c, None, "", ""))
...
def pairwise_and_cld(df, sentiment, measure, struct, scale_name):
    ...
    2) Build pairwise => BH => find reject_H0
    3) build_iterative_cld => final check => raise error if conflict
    """
    from statsmodels.genmod.generalized_estimating_equations import GEE
    d2=df.copy()
    if measure=="Quotation":
    ...
    # final iterative approach
    cld_df=build_iterative_cld(final_fit, pair_df)
...
def compile_results_into_multiple_workbooks(
    ...
    wb_plots=Workbook()
    # Only remove default sheet if there's more than one
...
def main():
    setup_logging()
    logging.info("Starting best QIC GEE approach with BH post-hoc & iterative CLD final approach.")
    ...
    print("Fitting best QIC approach for each sentiment–measure (with BH pairwise + iterative letter assignment).")
