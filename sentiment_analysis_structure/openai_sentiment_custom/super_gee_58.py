...
def pairwise_and_diagnostics(
    ...
    # If non-empty => compute means over axis=0
    if not boot_params_df.empty:
        param_mean_boot = boot_params_df.mean(axis=0).round(4).to_dict()
        param_std_boot  = boot_se_df.mean(axis=0).round(4).to_dict()
    else:
        param_mean_boot = {}
        param_std_boot  = {}

    if not jk_params_df.empty:
        param_mean_jk = jk_params_df.mean(axis=0).round(4).to_dict()
        param_std_jk  = jk_se_df.mean(axis=0).round(4).to_dict()
    else:
        param_mean_jk = {}
        param_std_jk  = {}
    ...
    #    (FIXED so we exclude the intercept in non-reference comparisons)
    # -------------------------------------------------------------------
    params = final_fit.params       # e.g. [Intercept, media_category[T.Lean Left], ...]
    cov    = final_fit.cov_params() # robust covariance
    ...
    # Build a map from category -> index in final_fit.params (or None if reference)
    exog_names = final_fit.model.exog_names  # e.g. ["Intercept", "media_category[T.Lean Left]", ...]
    idx_map = {}
    for c in cats:
        param_name = f"media_category[T.{c}]"
        if param_name in exog_names:
            idx_map[c] = exog_names.index(param_name)
        else:
            idx_map[c] = None  # reference cat => param=0
    ...
    for i2 in range(n_cats):
        for j2 in range(i2 + 1, n_cats):
            ...
            a_idx = idx_map[ca]  # can be None or an int
            b_idx = idx_map[cb]  # can be None or an int

            # param(A) = 0 if A is reference, else params[a_idx]
            param_a = params[a_idx] if a_idx is not None else 0.0
            # param(B) = 0 if B is reference, else params[b_idx]
            param_b = params[b_idx] if b_idx is not None else 0.0

            diff_est = param_b - param_a

            # Build contrast vector for variance
