...
**Change**: We now pass `maxiter=300` in every `model.fit(...)` call to allow more GEE iterations.
"""
...
def pseudo_likelihood_check(final_fit):
    ...
    nb_res=nb_model.fit(maxiter=300)  # <--- set 300 here as well, just in case
    nb_qic=nb_res.qic()
...
def cross_validation_gee(data_with_score, formula, group_col, family, cov_struct, n_folds=3):
    ...
    for i in range(n_folds):
        ...
        res=mod.fit(maxiter=300)  # <--- add maxiter=300
...
def coverage_simulation_gee(final_fit, n_sims=1000, alpha=0.05):
    ...
    for _ in range(n_sims):
        ...
        sim_model = GEE.from_formula(model.formula,
                                     ...
                                     data=sim_data,
                                     family=family, cov_struct=cov_struct)
        sim_res = sim_model.fit(maxiter=300)  # <--- add
...
###############################################################################
# GEE alpha search in [0..30], with maxiter=300 in .fit()
###############################################################################
        ...
        for cov_obj in cor_structs:
            ...
            for sc_opt in scale_opts:
                try:
                    ...
                    base_res=model.fit(maxiter=300)  # <--- changed here
                    ...
                    if sc_opt!="none" and dfresid>0:
                        ...
                        if val is not None and not np.isnan(val):
                            final_res = model.fit(maxiter=300, scale=val)  # <--- changed
        ...
        for sc_opt in scale_opts:
            for alpha_candidate in alpha_range:
                try:
                    ...
                    base_res=model.fit(maxiter=300)  # <--- changed
                    y=base_res.model.endog
                    mu=base_res.fittedvalues
                    n=len(y)
                    p=len(base_res.params)
...
###############################################################################
# Refit + advanced checks, with maxiter=300 in .fit()
###############################################################################
def refit_best_gee_with_scale(df, measure, fam_name, struct, scale_name, alpha_val=None):
    ...
    model=GEE.from_formula("_score_col ~ media_category",
                           ...
                           family=fam_obj,
                           cov_struct=cov_obj)
    bres=model.fit(maxiter=300, scale=None)  # <--- changed
...
def main():
    setup_logging()
    logging.info("Starting dual-run GEE approach with NB alpha grid search, coverage, cluster bootstrap, etc., maxiter=300.")
