...
def pairwise_and_diagnostics(
    ...
    # A helper to safely convert DataFrame.mean(...) to a dict, 
    # even if there is only one column (i.e. result is float).
    def safe_mean_to_dict(df_in):
        if df_in.empty:
            return {}
        mean_result = df_in.mean(axis=0)
        # If there's only one column, mean_result might be a float => convert to 1-element Series
        if isinstance(mean_result, float):
            # re-construct a Series with the same column name
            col_name = df_in.columns[0]
            mean_result = pd.Series([mean_result], index=[col_name])
        return mean_result.round(4).to_dict()

    # Instead of directly calling .mean(...).to_dict(), use the helper
