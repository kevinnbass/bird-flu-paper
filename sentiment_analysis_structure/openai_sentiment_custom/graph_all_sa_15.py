...
def fit_lmm_and_posthoc(df, sentiment, measure_type='Quotation'):
    ...
    if "Random effects covariance is singular" in summ_text:
        ...
        # ADDITION HERE: Identify outlets that may cause the issue by checking std dev
        group_std = model_df.groupby("media_outlet")[score_col].std().fillna(0)
        problematic_outlets = group_std[group_std == 0].index.tolist()
        if problematic_outlets:
            print("Outlets with zero standard deviation in scores (likely causing singularity):")
            for po in problematic_outlets:
                print(f" - {po}")
        else:
            # If no zero-std outlets found, print those with very low std
            low_std_threshold = 1e-6
            very_low_std = group_std[group_std < low_std_threshold].index.tolist()
            if very_low_std:
                print("Outlets with extremely low standard deviation in scores (likely causing singularity):")
                for vo in very_low_std:
                    print(f" - {vo}: std={group_std[vo]}")
