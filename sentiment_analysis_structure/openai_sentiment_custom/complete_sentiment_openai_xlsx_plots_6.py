...
def plot_mean_scores(stats_df, sentiment_categories, output_dir):
    """
    Generates bar plots for the mean scores of each sentiment/emotion category across media categories.
    ...
    """
    sns.set_style("whitegrid")
    palette = sns.color_palette("magma", len(MEDIA_CATEGORIES))
    ...
    for sentiment in sentiment_categories:
        ...
        plt.figure(figsize=(10, 6))
        sns.barplot(x='Media Category', y='Mean', data=subset, palette=palette)
        plt.title(f"Mean Scores for '{sentiment.capitalize()}' Across Media Categories", fontsize=16)
        plt.xlabel('Media Category', fontsize=14)
