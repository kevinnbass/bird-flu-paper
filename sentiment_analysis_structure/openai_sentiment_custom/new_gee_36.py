...
INPUT_JSONL_FILE = 'processed_all_articles_fixed.jsonl'
...
def fit_gee_model(df, formula, groups, cov_struct, family=Poisson(), time=None):
    ...
    Now with extra debug for Unstructured specifically.
    """
    if time is not None:
        logging.debug(f"Preparing to fit GEE with {cov_struct.__class__.__name__}, time array shape={time.shape}, dtype={time.dtype}.")
    else:
        logging.debug(f"Preparing to fit GEE with {cov_struct.__class__.__name__}, no time param.")

    if isinstance(cov_struct, Unstructured):
        logging.debug("Detected Unstructured covariance. Possibly checking time usage internally even if not passed.")
        if time is None:
            logging.debug("Unstructured: time is None -> This might cause an issue if statsmodels expects a time array.")
        else:
            logging.debug(f"Unstructured: time is not None, shape={time.shape}, dtype={time.dtype} (but might not be used?).")
...
def run_gee_and_qic(df, sentiment, measure_type, structures, min_date=None):
    ...
    If AR(1) is tested and date is present, create int_date as integer days from min_date.
    """
    ...
    logging.debug(f"Dataframe after dropna on {score_col}, media_category, media_outlet => {len(mdf)} rows.")
    if mdf['media_category'].nunique() < 2:
        logging.debug("Not enough categories to fit GEE.")
        return {}

    # If date is present, parse to datetime
    ...
    if min_date is not None and 'date' in mdf.columns and mdf['date'].notna().sum() > 0:
        logging.debug(f"Attempting AR(1) time array creation with min_date={min_date}.")
        mdf = mdf.sort_values(['media_outlet','date'])
        mdf['int_date'] = (mdf['date'] - min_date).dt.days + 1
        # Now strictly int
        mdf['int_date'] = mdf['int_date'].astype(np.int32, errors='ignore')
        logging.debug(f"Created int_date => shape={mdf['int_date'].shape}, dtype={mdf['int_date'].dtype}, first5={mdf['int_date'].values[:5]}")
    ...
    for struct_name, struct_obj in structures.items():
        ...
        if struct_name == 'AR1':
            if 'int_date' not in mdf.columns:
                logging.debug("AR1 chosen but int_date not in mdf => skipping AR(1).")
                continue
            # Ensure no missing int_date
            before_ar1 = len(mdf)
            mdf = mdf.dropna(subset=['int_date'])
            after_ar1 = len(mdf)
            logging.debug(f"AR(1): dropped {before_ar1-after_ar1} rows for missing int_date. {len(mdf)} remain.")
            if len(mdf) < 2:
                logging.debug("Insufficient rows left for AR(1). Skipping.")
                continue
            time_arr = mdf['int_date'].values
            logging.debug(f"AR(1): final time array shape={time_arr.shape}, dtype={time_arr.dtype}, first5={time_arr[:5] if len(time_arr)>5 else time_arr}")

        elif struct_name == 'Unstructured':
            # Additional debug for Unstructured
            logging.debug("Unstructured structure chosen. Checking time usage. 'time' param won't typically be used, but statsmodels might still look at it.")
            # Typically, time_arr remains None for Unstructured, but let's confirm
            if 'int_date' in mdf.columns:
                logging.debug("We do have an int_date column, but won't pass it to Unstructured by design.")
            else:
                logging.debug("No int_date column found. That's okay for Unstructured, but statsmodels might do internal checks.")

        logging.debug(f"Fitting structure={struct_name} for sentiment={sentiment}, measure={measure_type}. (#rows={len(mdf)})")
...
def run_gee_analyses(df):
    """
    If date present => define min_date => attempt AR1.
    Fit multiple structures (Independence, Exchangeable, Unstructured, AR1),
    pick best by QIC, do pairwise. Return all results in nested dict.
    """
