...
def compute_article_level_scores(df, sentiment_categories):
    # Compute per-article quotation mean and fulltext clipped scores
...
def aggregate_sentiment_scores(df, sentiment_categories):
    ...
    for media_category in MEDIA_CATEGORIES.keys():
        ...
        for sentiment in sentiment_categories:
            ...
            # Compute median from article-level columns
            quotation_values = subset[f"{sentiment}_quotation_mean_article"].dropna()
            if not quotation_values.empty:
                quotation_median = quotation_values.median()
            else:
                quotation_median = None

            fulltext_values = subset[f"{sentiment}_fulltext_article"].dropna()
            if not fulltext_values.empty:
                fulltext_median = fulltext_values.median()
            else:
                fulltext_median = None
            ...
            aggregation.append({
                ...
                'Quotation_Median': quotation_median,
                'Fulltext_Median': fulltext_median
            })
...
def plot_statistics(aggregated_df, output_dir):
    ...
    for sentiment in CATEGORIES:
        ...
        # Median Quotation-Based Plot (use Quotation_Median)
        plt.figure(figsize=(10, 6))
        sns.barplot(
            x='Media Category',
            y='Quotation_Median',
        ...
        plt.title(f"Median Quotation-Based '{sentiment.capitalize()}' Scores", fontsize=14)
        plt.xlabel('Media Category', fontsize=12)
        plt.ylabel('Median Quotation-Based Score', fontsize=12)
        plt.xticks(rotation=45, ha='right')
        plt.tight_layout()
        quote_plot_median_filename = f"quote_median_{sentiment}.png"
        quote_plot_median_path = os.path.join(output_dir, quote_plot_median_filename)
        try:
            plt.savefig(quote_plot_median_path)
            plt.close()
            print(f"Quotation-Based Median '{sentiment}' scores plot saved to '{quote_plot_median_path}'.")
        except Exception as e:
            print(f"Error saving quotation-based median '{sentiment}' scores plot: {e}")

        # Median Fulltext-Based Plot (use Fulltext_Median)
        plt.figure(figsize=(10, 6))
        sns.barplot(
            x='Media Category',
            y='Fulltext_Median',
        ...
        plt.title(f"Median Fulltext-Based '{sentiment.capitalize()}' Scores", fontsize=14)
        plt.xlabel('Media Category', fontsize=12)
        plt.ylabel('Median Fulltext-Based Score', fontsize=12)
        plt.xticks(rotation=45, ha='right')
        plt.tight_layout()
        fulltext_plot_median_filename = f"fulltext_median_{sentiment}.png"
        fulltext_plot_median_path = os.path.join(output_dir, fulltext_plot_median_filename)
        try:
            plt.savefig(fulltext_plot_median_path)
            plt.close()
            print(f"Fulltext-Based Median '{sentiment}' scores plot saved to '{fulltext_plot_median_path}'.")
        except Exception as e:
            print(f"Error saving fulltext-based median '{sentiment}' scores plot: {e}")
...
def compile_results_into_multiple_workbooks(aggregated_df, stats_df, raw_df, gee_results, plots_dir,
    ...
    for sentiment in CATEGORIES:
        ...
        # Median plots
        quote_plot_median_path = os.path.join(plots_dir, f"quote_median_{sentiment}.png")
        if os.path.exists(quote_plot_median_path):
            sheet_title = f"Quote_Med_{sentiment[:25]}"
            worksheet = wb.create_sheet(title=sheet_title)
            try:
                img = ExcelImage(quote_plot_median_path)
                img.anchor = 'A1'
                worksheet.add_image(img)
            except Exception as e:
                logging.error(f"Error embedding image '{quote_plot_median_path}' into Excel: {e}")

        fulltext_plot_median_path = os.path.join(plots_dir, f"fulltext_median_{sentiment}.png")
        if os.path.exists(fulltext_plot_median_path):
            sheet_title = f"Fulltext_Med_{sentiment[:22]}"
            worksheet = wb.create_sheet(title=sheet_title)
            try:
                img = ExcelImage(fulltext_plot_median_path)
                img.anchor = 'A1'
                worksheet.add_image(img)
            except Exception as e:
                logging.error(f"Error embedding image '{fulltext_plot_median_path}' into Excel: {e}")
...
def main():
    ...
    # Compute article-level scores before aggregation
