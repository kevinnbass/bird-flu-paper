#!/usr/bin/env python3
# super_gee_all_validations_dual_cats_sens_in_same_xlsx_collapsed_keyfix.py
"""
...
All output files are placed into subdirectory "analysis_output/".
Any run with "_collapsed" in its prefix is labeled "analysis_XXX_collapsed_YYY.xlsx".

This version fixes the KeyError in check_residuals_and_correlation by ensuring
that residuals are converted to a NumPy array before indexing.

**CHANGES**:
  1) In aggregate_sentiment_scores(), we now compute and store SD (standard deviation) 
     and SEM (standard error of the mean) for both Quotation and Fulltext.  
  2) In analyze_2fields_correlation(), we now write out the z-score–normalized data 
     (allc) to an XLSX file (in addition to the existing correlation summary).
"""
...
# We'll store base file names here:
...
# Will store parameter-level comparisons to check cluster imbalance
COMPARE_IMBALANCE = []
...
###############################################################################
# rename_files_for_prefix => handle "collapsed" labeling
###############################################################################
def rename_files_for_prefix(base_name, prefix):
    """
    - Takes e.g. "analysis_main.xlsx"
    - If prefix ends with "_collapsed", insert "_collapsed_" before last portion
      => "analysis_main_collapsed_Yes.xlsx" if prefix="Yes_collapsed"
    - Else => "analysis_main_Yes.xlsx"
    All placed under OUTPUT_DIR
    """
    if not prefix:
...
def analyze_2fields_correlation(df, left_field_pattern, right_field,
    ...
    barpath=os.path.join("graphs_analysis", barname.lower())
    try:
        plt.savefig(barpath)
    except:
        pass
    ...
    # Save cor_df to CSV
    out_csv = os.path.join("csv_raw_scores", f"correlation_{prefix}_{correlation_title.replace(' ','_')}.csv")
    cor_df.to_csv(out_csv, index=False)

    # If we have data, we compute z-scores & store them
    if all_combo:
        ...
        # Scatter-plot
        # (unchanged from your original code, or as previously requested, etc.)
        ...
        scatterpath=os.path.join("graphs_analysis", scattername.lower())
        plt.tight_layout()
        try:
        ...
        # Write correlation summary & z-score data to XLSX
        out_xlsx = os.path.join("csv_raw_scores", f"correlation_{prefix}_{correlation_title.replace(' ','_')}.xlsx")
...
def aggregate_sentiment_scores(df, sentiments):
    """
    For each (media_category x sentiment), collect:
      - Quotation_Sum, Quotation_Count, Quotation_SD, Quotation_SEM
      - Fulltext_Sum, Fulltext_Count, Fulltext_SD, Fulltext_SEM
    """
    recs=[]
    ...
    for cat in cats_in_df:
        ...
        for s in sentiments:
            # Quotation columns (like joy_0, joy_1, etc.)
            ...
            # Single Fulltext column (like joy_fulltext)
            fcol = f"{s}_fulltext"
...
def bootstrap_gee_md(df, formula, group_col,
    ...
    for _ in range(B):
        ...
        factor=M/(M-1.0) if M>1 else 1.0
        cov_mand=factor*base_cov
        se_mand=np.sqrt(np.diag(cov_mand))
        if len(se_mand)!=len(param_names):
            continue
...
###############################################################################
# check_residuals_and_correlation (KeyError fix)
###############################################################################
def check_residuals_and_correlation(final_fit):
    """
    Convert residuals to a NumPy array => no KeyError when indexing by i.
    """
    y=final_fit.model.endog
    mu=final_fit.fittedvalues
    pearson_res_arr = np.asarray((y - mu) / np.sqrt(mu + 1e-9))  # KEY FIX
...
def pairwise_and_diagnostics(df, sentiment, measure, fam_name, struct, scale_name,
    ...
    ########################################################################
    # >>>>>> The ONLY LINES CHANGED: force a custom order for categories <<<<<
    ########################################################################
    # 1) Force the specific order (1..6)
    desired_order = [
        "Scientific",
        "Left",
        "Lean left",
    ...
    # 2) Start with just those in 'cats' that appear in desired_order
    forced_cats = [c for c in desired_order if c in cats]
    # 3) Append any leftover categories not in forced_cats
    remainder = [c for c in cats if c not in forced_cats]
    forced_cats.extend(remainder)

    # 4) We'll assign IDs to forced_cats in that exact order => 1..N
    cat_index_map = {forced_cats[i]: i+1 for i in range(len(forced_cats))}

    # 5) We still define the first category as the reference (exog index=0).
    ref = forced_cats[0]
    idx_map={ref:0}
    # For the others, if they appear in exog_names, store their index:
    for c in forced_cats[1:]:
    ...
    pair_list=[]
    # We'll do pairwise comparisons among forced_cats in order
    for i2 in range(len(forced_cats)):
        for j2 in range(i2+1,len(forced_cats)):
            ca=forced_cats[i2]
            cb=forced_cats[j2]
            # If neither in idx_map, skip
    ...
    # Build the "diffIDs_df"
    diff_map={c:set() for c in forced_cats}
...
def compile_results_into_multiple_workbooks(
    ...
    with pd.ExcelWriter(main_excel, engine="openpyxl") as w:
        # Aggregated_Scores now includes the SD/SEM columns for Quotation/Fulltext
    ...
    # combined => aggregator + best QIC + validations
...
###############################################################################
# append_sens_to_excel => put Sens results in same files
###############################################################################
def append_sens_to_excel(
    ...
    # gee => combos, best qic index for Sens
    if os.path.exists(gee_excel):
...
###############################################################################
# 6) run_pipeline_for_df => merges small clusters => store Sens in same files
###############################################################################
def run_pipeline_for_df(
    ...
    # rename outputs according to prefix
    ...
    # If prefix indicates "Yes", we collapse categories => single cluster
    ...
    # Aggregate => now includes Quotation_SD/SEM, Fulltext_SD/SEM
    ...
    df_sens=exclude_small_clusters(df,"media_outlet_clean",min_size,merge_into="OtherSmall")
    if len(df_sens)<2:
