...
Script that:
  ...
  6) For each best QIC model:
     ...
     - Build a CLD table with 4 columns => [MediaCategory, Mean, UserCLD, AutoCLD]
       using adjacency + rpy2 => R’s multcompView::multcompLetters
         * If reject_H0=False => "not differ" => adjacency=1
         * If reject_H0=True => "differ" => adjacency=0
  7) Writes summary, pairwise, & CLD into analysis_gee.xlsx (one sheet per sentiment×measure),
     ...
     + If a conflict arises, we do not skip the tab. Instead we put an error message there
       and also create a Conflict_... sheet.
...
# ----------------------------------------------------------------------------
# 6) Build adjacency => pass to R => might raise conflict
# ----------------------------------------------------------------------------
def build_cld_with_rpy2(final_fit, pair_df):
    """
    1) adjacency=0/1 => 1 means 'not differ'
    2) pass to multcompView::multcompLetters(..., compare=">", threshold=0.5)
    3) Convert named vector => python dict => do final conflict check => raise RuntimeError if conflict
    """
    ...
    np.fill_diagonal(adjacency,1) # same group => not differ

    cat_index={cat:i for i,cat in enumerate(group_names)}
    # Build adjacency => 'reject_H0'=>True => differ =>0
    ...
    # final conflict check => if adjacency[i,j]==1 => must share >=1 letter
    for i in range(n):
        for j in range(i+1,n):
            ...
            if not_diff:
                if len(overlap)==0:
                    raise RuntimeError(f"R-based CLD conflict: {A} vs {B} are 'not differ' but share no letter!")
            else:
                if len(overlap)>0:
                    raise RuntimeError(f"R-based CLD conflict: {A} vs {B} differ but share letter(s) {overlap}!")

    # build final => 4 columns
    rows=[]
    for c in CLD_ORDER:
        if c in group_names:
            pm=mean_map[c]
            letters_str=letters_dict[c]
            rows.append((c, pm, "", letters_str))
        else:
            rows.append((c, None, "", ""))
...
def pairwise_and_cld(df, sentiment, measure, struct, scale_name):
    ...
    2) pairwise => BH => adjacency => build_cld_with_rpy2 => might raise RuntimeError if conflict
    """
    d2=df.copy()
    if measure=="Quotation":
        pat=rf"^{re.escape(sentiment)}_\d+$"
    ...
    # Build pairwise table => diff_est, pval => BH => reject_H0
    ...
    # adjacency => build CLD => might conflict => raise
    cld_df=build_cld_with_rpy2(final_fit, pair_df)
    return summary_txt, pair_df, cld_df

# ----------------------------------------------------------------------------
# 7) compile => if conflict => still create normal sheet + conflict sheet
# ----------------------------------------------------------------------------
def compile_results_into_multiple_workbooks(
    ...
    # analysis_gee => best QIC => handle conflicts
    with pd.ExcelWriter(gee_excel, engine="openpyxl") as writer:
        ...
        for i, row in df_best_qic.iterrows():
            ...
            # Start with an empty DataFrame so we have the sheet created
            base_df=pd.DataFrame({"Info":[f"{s}-{meas} tab initialization."]})
            ...
            try:
                ...
                if summary_txt is None:
                    no_data=pd.DataFrame({"Summary":[f"No valid model or not enough data: {s}-{meas}"]})
                    no_data.to_excel(writer,sh_name,index=False)
                else:
                    # fill normal sheet
                ...
                # fill the normal sheet with an error
                err_df=pd.DataFrame({"Summary":[f"Error encountered for {s}-{meas} => {str(e)}"]})
                err_df.to_excel(writer,sh_name,index=False)

                # create conflict sheet
                conflict_sheet=f"Conflict_{s[:10]}_{meas[:8]}_{i}"
                cdf=pd.DataFrame({"ConflictError":[str(e)]})
                cdf.to_excel(writer, conflict_sheet, index=False)

                logging.error(f"CLD conflict => {e}")
...
def main():
    setup_logging()
    logging.info("Starting best QIC GEE approach with BH post-hoc & rpy2-based CLD, "
                 "ensuring each tab is created + conflict sheets on errors.")
    ...
    # 5) best QIC
    print("Fitting best QIC approach for each sentiment–measure...")
