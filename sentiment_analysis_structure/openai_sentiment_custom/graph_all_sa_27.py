...
def debug_singular_matrix(model_df, formula, sentiment, measure_type):
    ...
    # Use smf.dmatrices here
    y, X = smf.dmatrices(formula, data=model_df, return_type='dataframe')
    ...
    for sentiment in CATEGORIES:
        ...
        if matched_cols:
            ...
            if model_df['media_category'].nunique() > 1 and model_df['media_outlet'].nunique() > 1:
                try:
                    md = sm.formula.mixedlm(formula, data=model_df, groups=model_df["media_outlet"])
