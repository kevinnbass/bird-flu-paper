...
Script that:
  ...
  4) For each best QIC model:
     ...
     - Pairwise => "DiffIDs"
  5) Writes summary, pairwise, & advanced checks into an Excel output
     with BestQIC_Index, All_Combos, Diagnostics, CrossValidation_Res, 
     Sensitivity_Analysis, ClusterBootstrap_Validation, and now CoverageSimulation_Res
  6) Also saves analysis_main, analysis_raw, analysis_plots, analysis_combined
     (Files renamed for clarity).
  7) Coverage simulation is integrated (parametric approach).
"""
...
# Renamed output files for better readability:
OUTPUT_EXCEL_MAIN = "analysis_readability_main.xlsx"
OUTPUT_EXCEL_RAW = "analysis_readability_raw.xlsx"
OUTPUT_EXCEL_GEE = "analysis_readability_gee.xlsx"
OUTPUT_EXCEL_PLOTS = "analysis_readability_plots.xlsx"
OUTPUT_EXCEL_COMBINED = "analysis_readability_combined.xlsx"
...
###############################################################################
# 3) Scale computations (additional scale methods added)
###############################################################################
...
def compute_ub_scale(y, mu, df_resid):
    """
    Just an illustrative scale => 1.1 * pearson
    """
...
def compute_bc_scale(y, mu, df_resid):
    """
    Another illustrative scale => 0.9 * deviance
    """
...
def bootstrap_gee(df, formula, group_col,
    ...
    """
    Cluster bootstrap with 1000 simulations by default
    for final validation (checks variability).
    """
...
def coverage_simulation_gee(final_fit, n_sims=1000, alpha=0.05):
    """
    Parametric approach to estimate coverage of the final GEE model's confidence intervals.
    We treat the fitted model parameters as 'true', simulate new data from them,
    refit, and see if the original parameters lie in the refitted CIs.

    - final_fit: the fitted GEE result object
    - n_sims: number of simulation replicates
    - alpha: significance level (0.05 for 95% CI), used for z-based intervals

    Returns a DataFrame with coverage rates for each parameter.
    NOTE: This is approximate, especially for NB or correlated data.
    """
    ...
    group_col = model.groups
    formula = model.formula

    # Original (fitted) parameters
    beta_true = final_fit.params.values

    data["_cluster_id"] = data[group_col].astype("category").cat.codes
    cluster_ids = data["_cluster_id"].unique()

    coverage_counts = np.zeros(len(beta_true))

    # We'll use the final_fit.fittedvalues as the mean for simulation
    mu = final_fit.fittedvalues
    # scale might be relevant for e.g. Gaussian
    sigma_est = np.sqrt(final_fit.scale) if hasattr(final_fit, "scale") else 1.0

    # If NB, get alpha
    alpha_nb = family.alpha if hasattr(family, "alpha") else 1.0

    # z-value for approximate CI
    z_val = abs(norm.ppf(alpha/2))

    for _ in range(n_sims):
        sim_data = data.copy()

        # Simulate outcomes based on family
        # (Ignoring correlated draws, each obs is simulated individually)
        if family.__class__.__name__ == "Poisson":
            ...
            # Approx NB param mapping:
            # mean = mu, var = mu + alpha_nb*mu^2
            # np.random.negative_binomial(r, p)
            # r = 1/alpha_nb, p = r / (r + mu[i])
            sim_draws = []
            for val in mu:
                r_ = 1.0/alpha_nb if alpha_nb != 0 else 10.0  # fallback
            ...
            # For Gamma, InverseGaussian, etc., you'd need further param considerations
            # We'll just do a fallback where we skip param-based draws and set them to mu
            # Or do an approximate Poisson approach, etc.
            sim_data[model.endog_names] = mu

        # Refit model
        sim_model = GEE.from_formula(formula, groups=group_col,
                                     data=sim_data, family=family, cov_struct=cov_struct)
        sim_res = sim_model.fit()

        # Extract new param estimates & CIs
...
###############################################################################
# 6) Multi-Family enumerations (QIC approach) - no brute force alpha anymore
###############################################################################
def try_all_families_and_scales(df, formula, group_col):
    """
    We brute-force across:
      - Families (Poisson, NB=alpha=1.0, Gaussian, Gamma, InverseGaussian)
      - CovStruct (Independence, Exchangeable)
      - Scale (none, pearson, deviance, ub, bc)
    But we no longer iterate over alpha values for NegativeBinomial (alpha=1.0 fixed).
    """
    families = [
        Poisson(),
        ...
        for cov_obj in cor_structs:
            ...
            for sc_opt in scale_opts:
                try:
                    ...
                    alpha_label = "alpha=1.0" if fam_name=="NegativeBinomial" else "alpha=None"
                    all_results.append((fam_name, cov_name, sc_opt, alpha_label, qic_val))

                    if qic_val<best_qic:
                        best_qic=qic_val
                        best_tuple=(fam_name,cov_name,sc_opt,alpha_label,qic_val)

                except Exception as e:
                    logging.warning(f"Fit fail => {fam_name}+{cov_name}+{sc_opt} => {e}")
                    alpha_label = "alpha=1.0" if fam_name=="NegativeBinomial" else "alpha=None"
                    all_results.append((fam_name,cov_name,sc_opt,alpha_label, np.nan))
...
###############################################################################
# 7) Refit + advanced checks (Pairwise, Diagnostics, CV, Sensitivity, Bootstrap, Coverage)
###############################################################################
def refit_best_gee_with_scale(df, measure, fam_name, struct, scale_name, alpha_val=None):
    ...
    if fam_name=="NegativeBinomial":
        logging.info(f"Re-fitting NB GEE with alpha=1.0 (no iteration).")
        print(f"DEBUG: Re-fitting NB GEE with alpha=1.0")
    else:
...
def pairwise_and_diagnostics(df, measure, fam_name, struct, scale_name, alpha_val=None):
    """
    Refit model with best QIC approach, then do:
      - summary (text),
      - pairwise diffs,
      - diagnostics,
      - cross-validation,
    ...
    # 1) Cluster bootstrap for final parameters:
    boot_df=bootstrap_gee(d2,
    ...
    return summary_txt, pair_df, diffIDs_df, diag, pseudo_dict, cv_val, sens_df, boot_df, coverage_df
...
def compile_results_into_multiple_workbooks(
    ...
    with pd.ExcelWriter(gee_excel, engine="openpyxl") as writer:
        ...
        for i,row in df_best_qic.iterrows():
            ...
            summary_txt, pair_df, diffIDs_df, diag_dict, pseudo_dict, cv_val, sens_df_, boot_df, coverage_df = out
        ...
        if len(boot_records)>0:
            ...
            boot_all.to_excel(writer,"ClusterBootstrap_Validation",index=False)

        # Coverage simulation
        if len(coverage_records)>0:
            coverage_all = pd.concat(coverage_records, ignore_index=True)
            coverage_all.to_excel(writer,"CoverageSimulation_Res", index=False)
...
def main():
    ...
    print("Analysis completed successfully with final cluster bootstrap, multiple scale options, no alpha brute force, plus coverage simulation.")
    logging.info("Analysis completed successfully with final cluster bootstrap, multiple scale options, no alpha brute force, plus coverage simulation.")
