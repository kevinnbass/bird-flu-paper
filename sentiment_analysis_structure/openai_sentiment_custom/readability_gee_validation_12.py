...
Script that:
  ...
  5) Writes summary, pairwise, & advanced checks into an Excel output
     ...
     Sensitivity_Analysis, ClusterBootstrap_Validation, CoverageSimulation_Res
  ...
  8) CrossValidation_Res now only shows (Measure, Family, Structure, Scale, Alpha, CV_MSE).
  9) ClusterBootstrap_Validation now has one row per parameter with BootMean & BootStd.
"""
...
def cross_validation_gee(data_with_score, formula, group_col, family, cov_struct, n_folds=3):
    """
    Perform cluster-level cross-validation in n_folds. Return average MSE across folds.
    """
...
def bootstrap_gee(df, formula, group_col,
    ...
    Returns a DataFrame of shape (B, #params) with parameter estimates for each replicate.
    """
...
def coverage_simulation_gee(final_fit, n_sims=1000, alpha=0.05):
    ...
    for _ in range(n_sims):
        ...
        if family.__class__.__name__ == "Poisson":
            ...
            sim_draws = []
            for val in mu:
                r_ = 1.0/alpha_nb if alpha_nb != 0 else 10.0
...
###############################################################################
# 6) Multi-Family enumerations (QIC approach) - no brute force alpha
###############################################################################
...
def pairwise_and_diagnostics(df, measure, fam_name, struct, scale_name, alpha_val=None):
    ...
    Refit model with best QIC approach, then do:
      ...
      - pairwise diffs
      - diagnostics
      - cross-validation => returns CV_MSE
      - sensitivity analysis
      - cluster bootstrap => returns param-level distribution
      - coverage simulation
    """
    d2, final_fit=refit_best_gee_with_scale(df, measure, fam_name, struct, scale_name, alpha_val=alpha_val)
    if final_fit is None:
        return (None,)*9  # no results
...
def compile_results_into_multiple_workbooks(
    ...
    diag_records=[]
    cv_records=[]        # Now simplified: just (Measure, Family, Structure, Scale, Alpha, CV_MSE)
    sens_records=[]
    coverage_records=[]
    idx_rows=[]

    # We'll store param-level bootstrap results in a dedicated DF
    # with columns: Measure, Family, Structure, Scale, Alpha, Parameter, BootMean, BootStd
    ...
    with pd.ExcelWriter(gee_excel, engine="openpyxl") as writer:
        ...
        for i,row in df_best_qic.iterrows():
            ...
            # 7) Cluster bootstrap => param-level record
            # each column in boot_df = one parameter
            # let's compute mean & std for each param
        ...
        # CrossValidation => now minimal columns
        ...
        # ClusterBootstrap => param-level
        if len(bootstrap_param_records)>0:
            boot_all=pd.DataFrame(bootstrap_param_records)
            # Round or keep as is
            boot_all["BootMean"] = boot_all["BootMean"].round(4)
            boot_all["BootStd"] = boot_all["BootStd"].round(4)
    ...
    wb_plots.save(OUTPUT_EXCEL_PLOTS)
...
def main():
    ...
    print("Analysis completed successfully. Now CrossValidation_Res & ClusterBootstrap_Validation are refined.")
    logging.info("Analysis completed successfully. Now CrossValidation_Res & ClusterBootstrap_Validation are refined.")
