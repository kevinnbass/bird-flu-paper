...
def run_pipeline_for_df(
    ...
    NOTE: We removed any code that collapsed categories if prefix=="Yes" or
          prefix=="Yes_collapsed", because we no longer do that. The code below
          simply runs the pipeline using the dataframe as-is.
    """
    ...
    # 6) SENS run => merges small outlets within each category into 'OtherSmall'
    #    (We keep this logic: after merging, we re-run the aggregator, GEE, etc.)
    ...
    # 7) Save aggregated => with a new suffix
    save_aggregated_scores_to_csv(agg_sens, csvout, prefix=f"{prefix}_Sens")

    # 8) Append 'SENS' results to Excel => side by side
    append_sens_to_excel(
...
def main():
    setup_logging()
    logging.info("Starting pipeline => yes => subset outlets => then all => no collapsed analysis")

    df_raw = load_jsonl(INPUT_JSONL_FILE)
    print(f"Total articles loaded => {len(df_raw)}")

    if "high_rate_2" in df_raw.columns:
        # Convert to lower/strip if needed
    ...
    # 1) Analyze the subset where high_rate_2 == "yes"
    #    => This will produce xlsx files with "_Yes" in their filenames.
    df_yes_orig = df_orig[df_orig["high_rate_2"] == "yes"].copy()
    if len(df_yes_orig) > 0:
        print("**Pipeline => Yes** (orig cats, subset of outlets with high_rate_2=yes)")
        run_pipeline_for_df(df_yes_orig, prefix="Yes")
        del df_yes_orig
        gc.collect()
    else:
        print("No rows => skip 'Yes' subset analysis (no outlets with high_rate_2=yes).")

    # 2) Analyze the full data => prefix="All"
    print("**Pipeline => All** (orig cats, full dataset)")
    run_pipeline_for_df(df_orig, prefix="All")
    del df_orig
    gc.collect()

    logging.info("All done => 'Yes' subset + 'All' => original cats => no collapsed analyses.")
