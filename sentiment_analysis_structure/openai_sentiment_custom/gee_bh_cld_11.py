...
Script that:
  ...
  6) For each best QIC model:
     ...
     - Build a CLD table with 4 columns [MediaCategory, Mean, UserCLD (blank), AutoCLD]
       using your new insert–absorb–sweep approach:
         - insert_phase => duplicate columns for significant differences
         - absorb_phase => drop duplicate columns
         - sweep_phase => remove redundancies
     - Then writes summary, pairwise, & CLD into analysis_gee.xlsx (one sheet per sentiment×measure),
       highlighting rows where reject_H0=True in red
  7) Also saves analysis_main, analysis_raw, analysis_plots, analysis_combined
"""
...
###############################################################################
# 6) The "insert–absorb–sweep" approach to CLD
###############################################################################
...
def insert_phase(data):
    ...
    if not any(data['reject H0']):
        ...
        for i, signif in enumerate(signifs['product_1']):
            ...
            df1 = df[cols_to_dup].copy()
            df2 = df[cols_to_dup].copy()
            df3 = df.drop(labels=cols_to_dup, axis=1)
...
def sweep_phase(df):
    ...
    for col in data.columns:
        ...
        for i in index:
            ...
            if len(set(check.values()))==1 and list(set(check.values()))[0]==1:
                data[col].loc[list(check.keys())[0][0]] = 0
...
def new_cld_main(data):
    df = insert_phase(data)
    dups = absorb_phase(df)
    if len(dups)>0:
...
###############################################################################
# pairwise_and_cld => uses new snippet
###############################################################################
...
def pairwise_and_cld(df, sentiment, measure, struct, scale_name):
    ...
    # Convert to snippet's format => product_1, product_2, 'reject H0'
    snippet_df = pd.DataFrame({
        "product_1": pair_df["CategoryA"],
        "product_2": pair_df["CategoryB"],
        "reject H0": pair_df["reject_H0"]
    })

    # run your new snippet => get a matrix of columns
    final_mat = new_cld_main(snippet_df)

    # Build final => [MediaCategory, Mean, UserCLD, AutoCLD]
    ...
    rows=[]
    for c in CLD_ORDER:
        if c in final_mat.index:
            active_cols = [col for col in final_mat.columns if final_mat.loc[c, col]==1]
            auto_str = "".join(active_cols)
...
def compile_results_into_multiple_workbooks(
    ...
    # analysis_gee => best QIC w/ pairwise + new snippet
...
def main():
    setup_logging()
    logging.info("Starting best QIC GEE approach with BH post-hoc & the new insert–absorb–sweep approach for CLD.")
    ...
    # 5) best QIC approach
    print("Fitting best QIC approach for each sentiment–measure (with BH pairwise + new snippet-based CLD).")
