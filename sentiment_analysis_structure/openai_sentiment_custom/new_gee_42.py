...
##############################
#   Optional "Fixes" or Debug
##############################

def remove_zero_var_groups(df, score_col):
    """Remove zero-variance groups to avoid numeric trouble."""
...
# (1) Optional fix: remove single-obs groups
def remove_single_obs_groups(df, score_col):
    """Remove groups that only have 1 observation, as AR(1) won't do much with them."""
...
# (2) More targeted debugging: 
# Dump each group's mean, var, size to CSV for deeper inspection
def dump_group_stats_csv(df, score_col, out_file='group_stats_debug.csv'):
    ...
    stats_df.sort_values(by='n', inplace=True)
    stats_df.to_csv(out_file)
    logging.debug(f"Wrote group-level stats to {out_file} for further inspection.")


def examine_ar1_data(df, score_col):
    """Existing AR(1) debug: logs group sizes and zero-variance info."""
    group_counts= df.groupby('media_outlet').size()
    group_variances= df.groupby('media_outlet')[score_col].var()
    logging.debug(
        f"AR(1) debug => group size stats: min={group_counts.min()}, max={group_counts.max()}"
    )
    logging.debug(
        f"AR(1) debug => # groups total={len(group_counts)}. # with 1 obs => {(group_counts==1).sum()}"
    )
    logging.debug(
        f"AR(1) debug => # zero-variance groups => {(group_variances==0).sum()}"
    )
...
def fit_gee_model(df, formula, groups, cov_struct, family=Poisson(), time=None):
    ...
    try:
        model= GEE.from_formula(
            ...
            family=family, time=time, scale=1  # forcibly set scale=1
        )
    except Exception as e:
    ...
    try:
        ...
        # If AR(1) => correlation param bracketing mention
...
def run_gee_and_qic(df, sentiment, measure_type, structures, min_date=None):
    ...
    # remove zero var groups always
    mdf= remove_zero_var_groups(mdf, score_col)

    # (1) If you'd like: remove single-obs groups
    # UNCOMMENT these lines if you want that fix:
    # mdf= remove_single_obs_groups(mdf, score_col)

    if len(mdf)<2:
        return {}

    # (2) If you'd like deeper group stats => UNCOMMENT:
    ...
    for nm, stobj in structures.items():
        ...
        if res is None:
            ...
            continue
        # QIC might be a tuple => get first
        try:
            qic_ = res.qic()
