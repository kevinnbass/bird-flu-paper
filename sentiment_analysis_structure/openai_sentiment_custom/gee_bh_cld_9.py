...
Script that:
  ...
  6) For each best QIC model:
     ...
     - Build a CLD table with 4 columns:
         [MediaCategory, Mean, UserCLD (blank), AutoCLD (classic stepwise assignment)]
...
def pairwise_and_cld(df, sentiment, measure, struct, scale_name):
    ...
    4) Build CLD => 4 columns with stepwise letter assignment:
    ...
    # Build final 4-col table
    cld_df=build_cld_4_columns_stepwise(final_fit, pair_df)

    return summary_txt, pair_df, cld_df


# ------------------------------
# 7) Stepwise CLD assignment
# ------------------------------
def build_cld_4_columns_stepwise(final_fit, pair_df):
    """
    Returns a 4-column DataFrame:
      [MediaCategory, Mean, UserCLD (blank), AutoCLD]
    The letters in AutoCLD use the classical stepwise approach:
      1) sort cats by ascending mean
      2) first cat => 'a'
      3) each subsequent cat => check existing letters in order:
         - if cat is NOT different from all groups that have letter X,
           cat can share X
         - if conflict => skip that letter
         - if no letter is possible => new letter
    """
    mdf=final_fit.model.data.frame
    cats=mdf["media_category"].cat.categories
    ...
    # 3) differ dict => differ[A][B] = True if they differ
    #    if reject_H0 == True => differ => can't share letter
    #    if reject_H0 == False => not differ => can share letter
    ...
    # 4) Stepwise letter assignment
    letters_used=[]
    cat_letters={c:[] for c in cats}

    def can_share_letter(group, letter):
        """
        Return True if 'group' does NOT differ from ANY group that already has 'letter'.
        If differ => conflict => return False
        """
        # find all groups that have letter
        conflicts= [g for g in cats if letter in cat_letters[g] and differ[group][g]==True]
        return (len(conflicts)==0)

    # first cat
    if sorted_cats:
        cat_letters[sorted_cats[0]]=["a"]
        letters_used=["a"]

    # the rest
    for group in sorted_cats[1:]:
        assigned=False
        for letter in letters_used:
            if can_share_letter(group,letter):
                cat_letters[group].append(letter)
                assigned=True
                break
        if not assigned:
            # create a new letter
            ...
            cat_letters[group].append(new_letter)

    # build table
    rows=[]
    for c in CLD_ORDER:
        if c in cats:
            auto_str="".join(sorted(cat_letters[c]))
...
def compile_results_into_multiple_workbooks(
    ...
    # 3) analysis_gee => best QIC w/ pairwise + 4-col CLD (stepwise assignment)
...
def main():
    setup_logging()
    logging.info("Starting best QIC GEE approach with BH post-hoc + 4-col CLD (stepwise letter assignment).")
    ...
    # 5) best QIC approach
    print("Fitting best QIC approach for each sentiment–measure (BH pairwise + stepwise CLD).")
