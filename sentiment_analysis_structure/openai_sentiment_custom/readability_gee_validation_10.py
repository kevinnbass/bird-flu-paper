...
Additional modifications based on user requests:
  - Output files have been renamed for better readability.
  - The iterative alpha brute force for NegativeBinomial has been removed.
    (We now use alpha=1.0 only for NB.)
  - A cluster bootstrap validation method with 1000 simulations has been added.
  - We now consider additional scaling methods beyond 'none' and 'pearson':
    also 'deviance', 'ub', and 'bc'.
...
###############################################################################
# 5) Multi-Family enumerations (QIC approach) - no brute force alpha anymore
###############################################################################
def try_all_families_and_scales(df, formula, group_col):
    ...
    But we no longer iterate over alpha values for NegativeBinomial.
    """
    families = [
        Poisson(),
    ...
    for fam in families:
        ...
        for cov_obj in cor_structs:
            ...
            for sc_opt in scale_opts:
                # Fit once with alpha=1.0 if NB; skip alpha search
                try:
                    ...
                    # Apply scaling if requested and feasible
...
def run_gee_for_measure_best_qic(df, measure):
    ...
    for row in combos:
        ...
        recs.append({
            ...
            "CovStruct": cov_,
            "Scale": sc_,
            "Alpha": alpha_str,  # e.g. 'alpha=1.0' or 'alpha=None'
            "QIC": q_
        })
    ...
    return {
        ...
        "Best_Scale": scOpt,
        "Best_Alpha": alphaVal,  # store numeric alpha or None
...
def refit_best_gee_with_scale(df, measure, fam_name, struct, scale_name, alpha_val=None):
    """
    Now uses alpha_val if NB (defaults to 1.0).
    """
...
def pairwise_and_diagnostics(df, measure, fam_name, struct, scale_name, alpha_val=None):
    """
    Refit model with best QIC approach, then do diagnostics, cross-validation,
    sensitivity analysis, and cluster bootstrap (1000 sims).
    """
    ...
    # Now 1000-sim cluster bootstrap for final parameters:
...
def compile_results_into_multiple_workbooks(
    ...
    with pd.ExcelWriter(gee_excel, engine="openpyxl") as writer:
        ...
        # Put cluster bootstrap results on a separate tab:
...
def main():
    ...
    print("Analysis completed successfully with final cluster bootstrap, multiple scale options, and no alpha brute force.")
    logging.info("Analysis completed successfully with final cluster bootstrap, multiple scale options, and no alpha brute force.")
