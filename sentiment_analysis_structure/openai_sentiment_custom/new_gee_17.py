...
def fit_gee_and_pairwise(df, sentiment, measure_type='Quotation', cov_struct_type=Exchangeable):
    ...
    if 'date' not in temp_df.columns:
        # Cannot do AR(1) properly without date
        pass
    ...
    if isinstance(cov_struct, Autoregressive):
        # Sort by group and date for AR(1)
    ...
    for sentiment in CATEGORIES:
        ...
        for measure_type in ['Quotation', 'Fulltext']:
            ...
            if len(results_list) == 0:
                continue
            # Ensure QIC are numeric
            # If QIC are nan or not numeric, handle gracefully
            results_list = [(sn, (qic if np.issubdtype(type(qic), np.number) else np.nan), r) for (sn, qic, r) in results_list]
            # If all are nan, sorting still works but won't be meaningful
...
def main():
    ...
    # No predictor columns, just run and plot
    # Plot statistics
