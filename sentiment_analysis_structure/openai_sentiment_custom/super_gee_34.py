...
def pairwise_and_diagnostics(df, sentiment, measure, fam_name, struct, scale_name,
    ...
    ############################################################################
    # >>>>> CHANGE HERE: enforce custom order for categories and IDs <<<<<
    ############################################################################
    # The desired row order and IDs for the final 'DiffIDs' table:
    #   1) Scientific
    #   2) Left
    #   3) Lean left
    #   4) Center
    ...
    #   6) Right
    #
    # We will force that order whenever these categories exist in the data.
    custom_order = [
        "Scientific",
        "Left",
        "Lean left",
    ...
    # Filter out any that aren't present in 'cats':
    cat_list = [c for c in custom_order if c in cats]
    # If there are additional categories (e.g. "Other"), append them at the end:
    remainder = [c for c in cats if c not in cat_list]
    cat_list.extend(remainder)
    # Now assign 1..N in exactly that order:
    cat_index_map = {cat_list[i]: i+1 for i in range(len(cat_list))}
    ############################################################################

    ref=cat_list[0]  # reference is the first in the forced order
    idx_map={ref:0}
    for c in cat_list[1:]:
    ...
    pair_list=[]
    for i2 in range(len(cat_list)):
        for j2 in range(i2+1,len(cat_list)):
            ca,cb=cat_list[i2], cat_list[j2]
            # If the categories do not exist in exog, skip
    ...
    # Build a dict-of-sets that tells which categories each cat differs from
