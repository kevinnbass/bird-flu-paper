...
Script that:
  ...
  6) For each best QIC model:
     ...
     - Build a CLD table with 4 columns:
       ...
       ensuring every pairwise difference is respected
...
# ------------------------------------------------------
# 6) Pairwise + CLD (classic stepwise)
# ------------------------------------------------------
...
def pairwise_and_cld(df, sentiment, measure, struct, scale_name):
    ...
    4) Build stepwise CLD => 4 columns:
       [MediaCategory, Mean, UserCLD, AutoCLD]
       ensuring every pairwise difference is fully enforced
    """
    ...
    # Build final table with stepwise approach
...
def build_cld_4_columns_stepwise(final_fit, pair_df):
    """
    4 columns: [MediaCategory, Mean, UserCLD, AutoCLD]
    Stepwise approach that re-checks each new group in ascending mean order,
    ensuring we honor all pairwise differences fully.
    """
    mdf=final_fit.model.data.frame
    cats=mdf["media_category"].cat.categories
    ...
    # 3) differ => differ[A][B] = True if they differ (reject_H0 = True)
    ...
    def can_share_letter(group, letter):
        # group can share letter only if it does NOT differ with any group that has letter
        for g in cats:
            if letter in cat_letters[g]:
                if differ[group][g] == True:
                    return False
        return True
    ...
    for group in sorted_cats[1:]:
        assigned=False
        for let in letters_used:
            if can_share_letter(group, let):
                cat_letters[group].append(let)
                assigned=True
                break
        if not assigned:
            # new letter
    ...
    for c in CLD_ORDER:
        if c in cats:
            auto=''.join(sorted(cat_letters[c]))
            rows.append((c, mean_map[c], "", auto))
        else:
            rows.append((c, None, "", ""))
...
def compile_results_into_multiple_workbooks(
    ...
    # 3) analysis_gee => best QIC w/ pairwise + 4-col stepwise CLD
...
def main():
    setup_logging()
    logging.info("Starting best QIC GEE approach with BH post-hoc & refined classical stepwise CLD (4 columns).")
    ...
    # 5) best QIC approach
    print("Fitting best QIC approach for each sentiment–measure (with BH pairwise + stepwise letter assignment).")
