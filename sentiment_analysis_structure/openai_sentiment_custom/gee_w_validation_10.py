#!/usr/bin/env python3
# gee_w_validation_9_fixed.py
"""
Script that:
  1) Loads data from JSONL
  2) Performs correlation analyses
  3) Aggregates stats
  4) Finds best QIC combination (multi-family)
  5) For each best QIC model:
     - Refit & produce GEE summary
     - Advanced checks => diagnostics, cross-validation, etc.
     - Pairwise => "DiffIDs"
  6) Writes summary, pairwise, & advanced check results into analysis_gee.xlsx
...
Fix for `NameError: '_score_col' is not defined`:
 - We make `refit_best_gee_with_scale` return both (d2, final_fit).
 - Then cross_validation_gee(...) uses that d2, ensuring `_score_col` is present.
"""
...
def check_residuals_and_correlation(final_fit):
    ...
    avg_corr=np.mean(wcorr) if len(wcorr)>0 else 0
...
def cross_validation_gee(data_with_score, formula, group_col, family, cov_struct, n_folds=3):
    """
    We pass data_with_score => ensures `_score_col` is defined for formula.
    """
...
def refit_best_gee_with_scale(df, sentiment, measure, fam_name, struct, scale_name):
    """
    Return => (d2, final_res)
    so we can pass d2 to cross_validation_gee => ensuring `_score_col` is recognized
    """
...
def pairwise_and_diagnostics(df, sentiment, measure, fam_name, struct, scale_name):
    """
    We get => (d2, final_fit)
    Then do advanced checks => cross_validation_gee(d2,...)
    """
    ...
    # CROSS-VALIDATION => we must pass d2 (which has _score_col)
...
def main():
    setup_logging()
    logging.info("Starting best QIC GEE approach + advanced checks + fix for `_score_col` CV error")
    ...
    print("Fitting best QIC approach for each sentiment–measure (with BH pairwise + advanced checks).")
    ...
    print("Analysis completed successfully with `_score_col` cross-validation fix.")
