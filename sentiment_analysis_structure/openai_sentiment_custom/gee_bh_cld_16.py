...
Script that:
  ...
  6) For each best QIC model:
     ...
     - Build a CLD table with 4 columns => [MediaCategory, Mean, UserCLD (blank), AutoCLD]
       using a compactletterdisplay-style algorithm
...
###############################################################################
# 6) Insert-and-absorb style function => build_iterative_cld
###############################################################################
def build_iterative_cld(final_fit, pair_df):
    """
    Replaces old code with a compactletterdisplay-like approach
    but is named "build_iterative_cld" so references remain valid.
    We'll do the insert-and-absorb approach with safe index checks.
    """
    # 1) Get categories
    ...
    # 3) build significant pairs => a list of (i, j)
    ...
    # 4) We'll store columns as a list of lists => each column has len(num_groups)
    num_groups=len(group_names)
    columns=[ ['a' for _ in range(num_groups)] ]  # start with one column of 'a'

    def get_next_unused_letter(columns_list):
        used_letters=set(letter for col in columns_list for letter in col if letter!='')
        for letter in 'abcdefghijklmnopqrstuvwxyz':
            if letter not in used_letters:
                return letter
        return None

    def absorb_columns(columns_list):
        absorbed=True
        while absorbed:
            absorbed=False
            for i, col1 in enumerate(columns_list):
                for j, col2 in enumerate(columns_list):
                    if i!=j:
                        indices1={idx for idx, ch in enumerate(col1) if ch!=''}
                        indices2={idx for idx, ch in enumerate(col2) if ch!=''}
    ...
    # 5) For each significant pair => if they share => split
    for (i_idx, j_idx) in sig_pairs:
        connected=False
        for col_idx, column in enumerate(columns):
            # check index range
            if i_idx>=len(column) or j_idx>=len(column):
                # skip
                continue
            if column[i_idx]==column[j_idx] and column[i_idx]!='':
                connected=True
                ...
                new_col=column.copy()
                # split
                new_col[i_idx]=new_letter if column[i_idx]!='' else ''
                new_col[j_idx]=''
                column[j_idx]=''
    ...
    # 6) final => for each group => gather letters
    result=[]
    for n in range(num_groups):
        letset=[]
        for col in columns:
            if n<len(col) and col[n]!='':
                letset.append(col[n])
        result.append(''.join(letset))

    # 7) build final DataFrame in CLD_ORDER
    rows=[]
    for c in CLD_ORDER:
        if c in group_names:
            idx=group_names.index(c)
            auto_str=result[idx]
            rows.append((c, mean_map[c], "", auto_str))
        else:
            rows.append((c, None, "", ""))

    return pd.DataFrame(rows, columns=["MediaCategory","Mean","UserCLD","AutoCLD"])

###############################################################################
def pairwise_and_cld(df, sentiment, measure, struct, scale_name):
    """
    1) Refit best GEE => final_fit
    2) Build pair_df => BH => reject_H0
    3) build_iterative_cld => return final cld_df
    """
    from statsmodels.genmod.generalized_estimating_equations import GEE
    d2=df.copy()
    if measure=="Quotation":
    ...
    # build final with build_iterative_cld
...
def compile_results_into_multiple_workbooks(
    ...
    # fix "At least one sheet must be visible"
    wb_plots=Workbook()
...
def main():
    setup_logging()
    logging.info("Starting best QIC GEE approach with BH post-hoc & iterative CLD, final code with safe checks + visible sheet fix.")
