...
def try_all_families_and_scales(df, formula, group_col):
    ...
    We brute-force across:
      - Families (Poisson, NB, Gaussian, Gamma, InverseGaussian)
      - CovStruct (Independence, Exchangeable)
      - Scale (none, pearson)

    BUT if Family == NegativeBinomial, we also do an inner loop over alpha in [0,3.0,step=0.1].

    Returns:
      best_tuple -> (fam_name, cov_name, sc_opt, best_qic)
      all_results -> list of (Family, CovStruct, Scale, QIC) or now
                     list of (Family, CovStruct, Scale, alpha, QIC) if NB
    """
    families = [
        Poisson(),
    ...
    for fam in families:
        ...
        for cov_obj in cor_structs:
            ...
            for sc_opt in scale_opts:
                # If it's not NegativeBinomial, do the old approach
                if fam_name != "NegativeBinomial":
                    ...
                    # If NegativeBinomial, do a brute force alpha from 0 to 3.0
                    alpha_vals = np.arange(0.0, 3.1, 0.1)
                    for alpha_candidate in alpha_vals:
                        # Rebuild the NB family with this alpha
                        nb_fam = NegativeBinomial(alpha=alpha_candidate)
                        try:
                            model=GEE.from_formula(formula, groups=group_col,
                            ...
                            # Store alpha also
                            all_results.append((fam_name, cov_name, sc_opt,
                                                f"alpha={alpha_candidate:.1f}", qic_val))

                            if qic_val<best_qic:
                                best_qic=qic_val
                                # We'll store alpha in best_tuple "slot" 3 for debugging
                                # or we could store alpha as well
                                best_tuple=(fam_name,cov_name,sc_opt,qic_val)
                        except Exception as e:
                            logging.warning(f"NB Fit fail => alpha={alpha_candidate}"
                                            f"+{fam_name}+{cov_name}+{sc_opt} => {e}")
                            all_results.append((fam_name, cov_name, sc_opt,
                                                f"alpha={alpha_candidate:.1f}", np.nan))
...
def run_gee_for_measure_best_qic(df, measure):
    ...
    # We don't know if combos contain alpha or not, so let's unify them:
    # If it's NB, combos might have length 5 per row, else 4. We'll handle that gracefully.
    # We'll unify them by pushing alpha info into a single string in the Scale column, if present.
    # Or we'll just store them as they are, but keep the last item as the QIC.
    # For clarity, let's ensure each row is (Family, CovStruct, Scale, Extra, QIC)
    # We'll rename them, so columns might differ in length. We'll do a small post-process below.

    # We'll convert combos to a list of lists first, then a DataFrame with consistent columns
    recs=[]
    for row in combos:
        if len(row)==4:
            # e.g. (Family, CovStruct, Scale, QIC)
            fam_, cov_, sc_, q_ = row
            recs.append({
                "Family": fam_,
                "CovStruct": cov_,
                "Scale": sc_,
            ...
            # e.g. (Family, CovStruct, Scale, alphaString, QIC)
            fam_, cov_, sc_, alpha_str, q_ = row
            recs.append({
                "Family": fam_,
                "CovStruct": cov_,
                "Scale": sc_,
                "Alpha": alpha_str,  # e.g. 'alpha=1.2'
                "QIC": q_
            })
...
def refit_best_gee_with_scale(df, measure, fam_name, struct, scale_name):
    ...
    if fam_name=="Poisson":
        ...
        # We'll keep alpha=1.0 here; the user picks the final alpha from combos if desired
