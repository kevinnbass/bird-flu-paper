...
def run_pipeline_for_df(
    ...
    NOTE: We removed the code that collapsed categories if prefix=="Yes"
          or "Yes_collapsed" because we no longer do those runs.
    """
    ...
    # 2) Removed the entire "if prefix=='Yes'..." block that collapsed categories
...
def main():
    setup_logging()
    logging.info("Starting pipeline => only all => original cats, then all => collapsed cats (no 'Yes')")

    df_raw = load_jsonl(INPUT_JSONL_FILE)
    print(f"Total articles loaded => {len(df_raw)}")

    if "high_rate_2" in df_raw.columns:
        # Convert to lower/strip if needed, but we won't subset on "yes" anymore
    ...
    # -- Remove the entire block that looked for "yes" subset
    #    i.e., the df_yes_orig code is gone.

    # Now we only do the "All" run
    ...
    # -- Remove the entire block that looked for "yes_collapsed" subset
    #    i.e., the df_yes_coll code is gone.

    # Now we only do the "All_collapsed" run
    print("**Pipeline => All_collapsed**")
    run_pipeline_for_df(df_coll, prefix="All_collapsed")
    del df_coll
    gc.collect()

    logging.info("All done => only 'All' and 'All_collapsed' runs. No 'Yes' subsets.")
