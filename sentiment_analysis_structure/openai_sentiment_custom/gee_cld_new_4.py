...
Script that:
  ...
  6) For each best QIC model:
     ...
     - Build a CLD table with 4 columns => [MediaCategory, Mean, UserCLD, AutoCLD]
       using rpy2 to call R’s multcompView::multcompLetters on an adjacency matrix
...
# We'll assume you've already installed R + "multcompView" in R
...
def run_gee_for_sentiment_measure_best_qic(df, sentiment, measure):
    ...
    if measure=="Quotation":
        ...
        if not matched:
            return None
        # fix the "params[0]" => let's just do ".iloc[0]" later
...
###############################################################################
# 6) Build adjacency => pass to R => multcompLetters
###############################################################################
def build_cld_with_rpy2(final_fit, pair_df):
    """
    Create adjacency matrix => 1 if not differ, 0 if differ.
    Then pass it to R => set row/col names inside R,
    then call multcompView::multcompLetters(...).
    We'll fix the "letters_py" indexing by converting the result to a dict.
    Also fix final_fit.params indexing by using .iloc[0].
    """

    mdf=final_fit.model.data.frame
    cats=mdf["media_category"].cat.categories

    # Instead of final_fit.params[0], do this:
    intercept_val=final_fit.params.iloc[0]  # ensures we index by position
    cat_effect={cats[0]: 0.0}

    for c in cats[1:]:
        nm=f"media_category[T.{c}]"
        # final_fit.params.get(nm, 0.0) is fine
    ...
    np.fill_diagonal(adjacency,1)  # each group "not differ" from itself

    cat_index={cat:i for i,cat in enumerate(group_names)}
    for i, row in pair_df.iterrows():
        A=row["CategoryA"]
        B=row["CategoryB"]
        differ=row["reject_H0"]  # True => differ => adjacency=0, else 1
        iA=cat_index[A]
        iB=cat_index[B]
        if differ:
            adjacency[iA,iB]=0
    ...
    # By default, "letters_r" might be an R Factor or named Vector => we convert to a Python dict
    with localconverter(default_converter + np2ri.converter):
        # e.g. letters_py might be something like ["a", "ab", ...], with an attached index
        letters_py = ro.conversion.rpy2py(letters_r)
        # letters_py might be a pandas Series or an array
        # We want a dict => name -> letter_str
        # If letters_r has 'names', we can do:
        r_names = letters_r.names
        # Convert to python list
        py_names = list(r_names)
        # Now zip them
        # letters_py might be e.g. an array of letter strings
        # We'll build a dict => { "Center": "abc", "Left": "a", ... }
        letters_dict = {}
        for i, nm in enumerate(py_names):
            letters_dict[nm] = letters_py[i]

    # optional check => adjacency
    for i in range(n):
        for j in range(i+1,n):
            not_diff=(adjacency[i,j]==1)
            A=group_names[i]
            B=group_names[j]
            # Access from our dictionary:
            A_letters = letters_dict[A]
            B_letters = letters_dict[B]
    ...
    # Build final => we read from letters_dict
    rows=[]
    for c in CLD_ORDER:
        if c in group_names:
            meanv=mean_map[c]
            letter_str=letters_dict[c]
            rows.append((c, meanv, "", letter_str))
        else:
            rows.append((c, None, "", ""))
...
def pairwise_and_cld(df, sentiment, measure, struct, scale_name):
    ...
    2) Pairwise => BH => adjacency => pass to R => retrieve letters
    3) Build final table
    """
    from statsmodels.genmod.generalized_estimating_equations import GEE
    d2=df.copy()
    ...
    # final => build adjacency => pass to R
...
def compile_results_into_multiple_workbooks(
    ...
    # Make sure the "plots" workbook has at least one sheet
    wb_plots=Workbook()
    ...
    if not any_sheets:
        # create a dummy sheet so openpyxl doesn't complain
...
def main():
    setup_logging()
    logging.info("Starting best QIC GEE approach (final) with BH post-hoc & rpy2-based CLD.\n"
                 "Fixes:\n"
                 " - .iloc for final_fit.params\n"
                 " - convert letters_py => dict\n"
                 " - ensure we always have at least one workbook sheet.")
