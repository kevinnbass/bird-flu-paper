#!/usr/bin/env python3
# gee_cld_with_conflicts_no_error.py
"""
Script that:
  ...
  6) For each best QIC model:
     ...
     - Always produce a normal summary + pairwise table in the same sheet
     - Attempt to build a CLD table with 4 columns => [MediaCategory, Mean, UserCLD, AutoCLD].
       * If conflicts arise ("Center vs Lean Left are 'not differ' but share no letter!"),
         we do NOT raise an exception. Instead we produce a minimal 'CLD conflict' table
         or an error message in place of the normal CLD.
  7) Writes summary, pairwise, & "CLD" (or conflict) into analysis_gee.xlsx (one sheet per sentiment×measure),
     highlighting rows where reject_H0=True in red
  8) Also saves analysis_main, analysis_raw, analysis_plots, analysis_combined

Why the repeated conflict error?
--------------------------------
Typically, "Center vs Lean Left are 'not differ' but share no letter!" means your adjacency matrix
says these two categories do NOT differ (p≥alpha => 'reject_H0'=False => adjacency=1),
but R's letter assignment can't find a shared letter for them, usually due to other constraints
(“triangular” or contradictory significance patterns).
Possible debugging steps:
  - Print or log the adjacency matrix for each pairwise table. Does it match your expectations?
  - Check for contradictory patterns: e.g., Center ~ LeanLeft not differ, LeanLeft ~ X differ, etc.
  - If it's truly contradictory, you need a different post-hoc approach or accept that no single
    letter assignment can fully respect these constraints.

In this code:
  - We do NOT skip any sheet if there's a conflict. We still show GEE summary + pairwise results.
  - The CLD portion shows either normal letter groupings or a minimal conflict table, so you have
    the best info for each sentiment–measure pair.
"""
...
# rpy2 (assuming installed + configured)
...
# Must have "multcompView" installed in R
multcompview = importr('multcompView')
...
# ----------------------------------------------------------------------------
# 6) Build adjacency => pass to R => produce minimal conflict in the CLD portion
# ----------------------------------------------------------------------------
def build_cld_with_rpy2(final_fit, pair_df):
    """
    1) adjacency => if reject_H0=False => adjacency=1, else=0
    2) pass M_r => multcompView.multcompLetters(..., compare=">", threshold=0.5)
    3) If conflict => instead of raising an error, we store them & produce a minimal table
    """
    conflicts=[]
    mdf=final_fit.model.data.frame
    cats=mdf["media_category"].cat.categories

    intercept_val=final_fit.params.iloc[0]
    ...
    mean_map={}
    for c in cats:
        val=intercept_val + cat_effect[c]
        mean_map[c]=math.exp(val)
    ...
    cat_index={cat:i for i,cat in enumerate(group_names)}
    for i,row in pair_df.iterrows():
        A=row["CategoryA"]
        B=row["CategoryB"]
        differ=row["reject_H0"]
        iA=cat_index[A]
        iB=cat_index[B]
        if differ:
            adjacency[iA,iB]=0
    ...
    # debugging: you might log or print adjacency
    # logging.debug(f"Adjacency matrix for {final_fit} =>\n{adjacency}")

    # pass to R
    with localconverter(default_converter + np2ri.converter):
        M_r=ro.conversion.py2rpy(adjacency)
    ro.r.assign("M_r",M_r)
    ro.r.assign("cat_names",ro.StrVector(group_names))
    ro.r("colnames(M_r) <- cat_names")
    ro.r("rownames(M_r) <- cat_names")

    try:
        res=multcompview.multcompLetters(ro.r("M_r"),compare=">",threshold=0.5)
        letters_r=res.rx2("Letters")
        with localconverter(default_converter + np2ri.converter):
            letters_py=ro.conversion.rpy2py(letters_r)
            r_names=letters_r.names
            py_names=list(r_names)
            letters_dict={}
            for i,nm in enumerate(py_names):
                letters_dict[nm]=letters_py[i]

        # final check => no exception => store conflict(s)
        for i in range(n):
            for j in range(i+1,n):
                A=group_names[i]
                B=group_names[j]
                not_diff=(adjacency[i,j]==1)
                overlap=set(letters_dict[A]).intersection(set(letters_dict[B]))
                if not_diff:
                    if len(overlap)==0:
                        # record conflict => no letter shared
                        conflicts.append(f"{A} vs {B}")
                else:
                    if len(overlap)>0:
                        conflicts.append(f"{A} vs {B}")
    except Exception as e:
        # major break => fill conflicts with entire adjacency or something
        # e.g. "some big error"
        conflicts.append(f"multcompLetters call error => {str(e)}")
        letters_dict=None

    # if conflicts => produce minimal CLD with conflict message
    # else => produce normal
    rows=[]
    if not conflicts and letters_dict is not None:
        # normal
        for c in CLD_ORDER:
            if c in group_names:
                pm=mean_map[c]
                let_str=letters_dict[c]
                rows.append((c,pm,"",let_str))
            else:
                rows.append((c,None,"",""))
    else:
        # conflict => build a table that shows 'CLD conflict => ...'
        conf_str=", ".join(conflicts)
        # We'll produce a single row or multiple rows, your choice
        # Let's do one row indicating conflict + maybe store adjacency
        rows.append(("CLD Conflict =>", conf_str, "", "No valid letters."))
        # You could also log adjacency or partial letters
...
def pairwise_and_cld(df, sentiment, measure, struct, scale_name):
    ...
    2) build pairwise => adjacency => build_cld_with_rpy2 => produce conflict in CLD if needed
    """
    d2=df.copy()
    if measure=="Quotation":
        pat=rf"^{re.escape(sentiment)}_\d+$"
    ...
    model=GEE.from_formula("_score_col ~ media_category", groups="media_outlet_clean",
                           data=d2, family=Poisson(), cov_struct=cov_obj)
    ...
    # build CLD => if conflict => put conflict info in table
    cld_df=build_cld_with_rpy2(final_fit, pair_df)
    return summary_txt, pair_df, cld_df


# ----------------------------------------------------------------------------
# 7) compile => do not skip => if conflict => minimal conflict in CLD portion
# ----------------------------------------------------------------------------
def compile_results_into_multiple_workbooks(
    ...
    # gee => each tab => summary + pairwise + CLD or conflict
    with pd.ExcelWriter(gee_excel, engine="openpyxl") as writer:
        ...
        for i,row in df_best_qic.iterrows():
            ...
            base_df=pd.DataFrame({"Info":[f"{s}-{meas} initial."]})
            base_df.to_excel(writer,sh_name,index=False)

            summary_txt, pair_df, cld_df = pairwise_and_cld(df_full, s, meas, st, sc)
            if summary_txt is None:
                # no valid model
                err_df=pd.DataFrame({"Summary":[f"No valid model or not enough data for {s}-{meas}."]})
                err_df.to_excel(writer,sh_name,index=False)
            else:
                # fill normal
    ...
    # plots => ensure at least one sheet
    wb_plots=Workbook()
    if "Sheet" in wb_plots.sheetnames and len(wb_plots.sheetnames)>1:
    ...
    # combined => ensure at least one sheet
    raw_clean=raw_df.copy()
    raw_clean=raw_clean.applymap(lambda x:", ".join(x) if isinstance(x,list) else x)
    wb_comb=Workbook()
    if "Sheet" in wb_comb.sheetnames and len(wb_comb.sheetnames)>1:
    ...
    if len(wb_comb.sheetnames)==0:
        wb_comb.create_sheet("DummySheet")
...
def main():
    setup_logging()
    logging.info("Starting best QIC GEE approach with BH post-hoc & rpy2-based CLD conflict approach => no skipping tabs.")
    ...
    # 5) best QIC
    print("Fitting best QIC approach for each sentiment–measure (with BH pairwise + adjacency-based CLD).")
    df_best, df_allcombos = run_gee_analyses_best_qic(df)
    print("Best QIC approach completed.\n")

    # 6) compile => produce each tab => if conflict => minimal conflict row in the CLD portion
    ...
    print("Analysis completed successfully. Any conflicts appear in the CLD table as a minimal row.")
