...
def pairwise_and_diagnostics(df, sentiment, measure, fam_name, struct, scale_name,
                             prefix="", isSens=False):
    """
    OLD version that calculates DiffIDs in the “old way” and includes Trans_DiffIDs.
    Mapping: 1->3, 2->1, 3->5, 4->2, 5->6, 6->4
    Then reorders them from lowest to highest.
    """

    from statsmodels.stats.multitest import multipletests
    global COMPARE_IMBALANCE

    # 1) Fit the model with best scale
    d2, final_fit = refit_best_gee_with_scale(df, sentiment, measure, fam_name, struct, scale_name)
    if final_fit is None:
        return (None, None, None, None, None, None, None, None)

    # 2) Summaries & diagnostics
    ...
    # 3) Store parameter-level info in COMPARE_IMBALANCE
    ...
    # 4) Pairwise differences across categories => old approach
    ...
    # 6) Build a dictionary of param stats from bootstrap
    ...
    return (
        summary_txt,    # final_fit.summary().as_text()
        pair_df,        # differences table
        diffIDs_df,     # old-style DiffIDs plus Trans_DiffIDs
        diag_dict,      # from check_residuals_and_correlation
        pseudo_dict,    # from pseudo_likelihood_check
        cv_val,         # cross_validation_gee results
        sens_df,        # sensitivity_analysis_correlation
        boot_info       # param means/std from bootstrap
    )
