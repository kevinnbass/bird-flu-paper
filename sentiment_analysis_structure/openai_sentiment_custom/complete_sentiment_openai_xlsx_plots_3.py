...
MEDIA_CATEGORIES = {
    ...
    'Left': ['TheAtlantic', 'The Daily Beast', 'The Intercept', 'Mother Jones', 'MSNBC', 'Slate', 'Vox'],
...
def save_raw_scores_to_csv(df, categories, sentiment_categories, csv_output_dir):
    ...
    for sentiment in sentiment_categories:
        data = {}
        for category in categories:
            subset = df[df['media_category'] == category][sentiment]
            data[category] = subset.tolist()
...
def main():
    ...
    if 'media_outlet' not in df.columns:
        print("Error: 'media_outlet' field not found in the data. Ensure the JSONL file contains 'media_outlet'. Exiting.")
        logging.error("Missing 'media_outlet' field in data. Exiting.")
        return
    ...
    sentiment_categories = CATEGORIES.copy()  # List of ten categories
    ...
    # Assuming the first script logs total_input_tokens and total_output_tokens
    # If not, you may need to adjust accordingly
    # For now, we'll skip cost calculation
