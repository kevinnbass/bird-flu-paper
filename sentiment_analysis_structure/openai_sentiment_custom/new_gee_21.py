...
def fit_gee_and_pairwise(df, sentiment, measure_type='Quotation', cov_struct_type=Exchangeable):
    ...
    # For Autoregressive structure, we need integer time indices
    if cov_struct_type == Autoregressive:
        cov_struct = Autoregressive()  # no grid parameter for Autoregressive
        # Sort by outlet and date
        model_df = model_df.sort_values(['media_outlet', 'date'])
        # Assign integer time indices per outlet
        model_df['time_index'] = model_df.groupby('media_outlet').cumcount().astype(int)
        time = model_df['time_index'].values  # ensure it's numpy array of int
    else:
    ...
    # Note: scale cannot be directly passed to GEE __init__ in older statsmodels
    # The warning unknown kwargs ['scale'] occurs. QIC requires appropriate scale.
    # Since QIC does not accept a user-defined scale directly here, we must accept that QIC
    # may not be comparable across models. We'll just rely on QIC as given.
    # If we want a known scale, we must set results.scale manually after fitting or choose a method.
    # For now, we will omit scale to avoid errors.

    model = GEE(y, X, groups=model_df["media_outlet"], time=time, family=family, cov_struct=cov_struct)
...
def main():
    ...
    if 'date' not in df.columns:
        ...
        # Convert YYYY-MM-DD to datetime64
