...
INPUT_JSONL_FILE = 'processed_all_articles_fixed_2.jsonl'  # unchanged
...
OUTPUT_EXCEL_QIC = 'analysis_gee_qic.xlsx'  # if you want to store QIC results in a separate file
...
def fit_gee_model(df, formula, groups, cov_struct, family=Poisson(), time=None):
    ...
    if time is not None:
        logging.debug(f"Preparing to fit GEE with {cov_struct.__class__.__name__} and integer time array.")
        logging.debug(f"time array shape={time.shape}, dtype={time.dtype}, first5={time[:5] if len(time)>5 else time}")
    else:
...
def run_gee_and_qic(df, sentiment, measure_type, structures, min_date=None):
    ...
    for struct_name, struct_obj in structures.items():
        ...
        logging.debug(f"Fitting structure={struct_name} for sentiment={sentiment}, measure={measure_type}.")
