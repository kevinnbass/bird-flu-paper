...
###############################################################################
# JACKKNIFE & CLUSTER BOOTSTRAP
###############################################################################
def jackknife_gee(df, formula, group_col, family, cov_struct):
    cluster_ids = df[group_col].unique()
    param_list = []
    for cid in cluster_ids:
        sub_df = df[df[group_col] != cid].copy()
        if len(sub_df) < 10:
            continue
        model = GEE.from_formula(formula, groups=group_col,
                                 data=sub_df, family=family,
                                 cov_struct=cov_struct)
        try:
            res = model.fit(maxiter=300)
    ...
    std_series = param_df.std(axis=0)
    return std_series.mean()
        ...
        for cov_obj in cor_structs:
            ...
            for sc_opt in scale_opts:
                try:
                    ...
                    # cluster bootstrap
                    cb_std = cluster_bootstrap_gee(df, formula, group_col,
                                                   B=50,
                                                   family=fam, cov_struct=cov_obj)
                    # jackknife
                    jk_std = jackknife_gee(df, formula, group_col, fam, cov_obj)

                    all_results.append((fam_name, cov_name, sc_opt, qic_val, cb_std, jk_std))
                    ...
                    all_results.append((fam_name, cov_name, sc_opt, np.nan, np.nan, np.nan))
...
def run_gee_for_sentiment_measure_best_qic(df, sentiment, measure):
    ...
    # combos => store cluster bootstrap & jackknife in columns
    combos_df=pd.DataFrame(
        combos,
        columns=["Family","CovStruct","Scale","QIC","ClusterBoot_MeanStd","Jackknife_MeanStd"]
    )
...
###############################################################################
# MAPPING FOR TRANS_DIFFIDs
###############################################################################
TRANS_MAP = {1:3, 2:1, 3:5, 4:2, 5:6, 6:4}

def transform_and_sort_diffIDs(diff_str):
    """
    - Parse the comma-separated string of digits.
    - Map each digit according to TRANS_MAP.
    - Sort the mapped values from low to high.
    - Return as comma-separated string.
    """
    if not diff_str.strip():
        return ""
    nums = [n.strip() for n in diff_str.split(",") if n.strip().isdigit()]
    mapped = []
    for n_str in nums:
        old_val = int(n_str)
        new_val = TRANS_MAP.get(old_val, old_val)  # default if missing
...
def pairwise_and_diagnostics(df, sentiment, measure, fam_name, struct, scale_name,
    ...
    # We skip the M&D correction entirely now.

    for i, p in enumerate(param_names):
        sr_se=robust_se[i]
        sr_val=param_values[i]
        # cluster boot std is not used here, so store np.nan
        ratio=np.nan
    ...
    # =============== NEW PART: CREATE "Trans_DiffIDs" ===============
    if not diffIDs_df.empty:
        diffIDs_df["Trans_DiffIDs"] = diffIDs_df["DiffIDs"].apply(transform_and_sort_diffIDs)
    # ================================================================
...
def compile_results_into_multiple_workbooks(
    ...
    with pd.ExcelWriter(gee_excel, engine="openpyxl") as writer:
        ...
        for i,row in df_best_qic.iterrows():
            ...
            boot_records.append({
                ...
                "Structure": st,
                "Scale": sc,
                "ClusterBootParamMean": pm_str,
                "ClusterBootParamStd":  ps_str
            })
    ...
    ws_val.cell(row=lr2,column=1,value=(
        "Above table for all checks. Look for warnings or potential issues."
    ))
