...
def aggregate_sentiment_scores(df, sentiment_categories):
    ...
    for media_category in MEDIA_CATEGORIES.keys():
        ...
        for sentiment in sentiment_categories:
            ...
            if matched_cols:
                # Clip negative quotation-based scores to zero
...
def fit_lmm_and_posthoc(df, sentiment, measure_type='Quotation'):
    ...
    try:
        md = smf.mixedlm(f"{score_col} ~ media_category", data=model_df, groups=model_df["media_outlet"])
...
def main():
    ...
    # ------------------ NEW SECTION FOR FILTERING OUTLETS ------------------ #
    print("Filtering out outlets with fewer than 2 observations before LMM fitting...")
