#!/usr/bin/env python3
# analysis_subset_then_full_intensity.py
"""
Script that:
  - Loads data from JSONL
  - Normalizes 'high_rate_2'
  - First analyzes the subset (high_rate_2=='yes'), producing _Yes outputs
  - Wipes data (del + gc.collect())
  - Reloads data, analyzes full set, producing _All outputs

Includes:
  - 5 measure modes for GEE: Quotation, Fulltext, Fulltext_Intensity, Title_Intensity, Quotation_Intensity
  - Cross-validation fix => unify category levels
  - Debug logic for 'high_rate_2'
  - Prefix-based naming => no overwriting
"""
...
# Output directories & base filenames
...
# Mapping media outlets => categories
MEDIA_CATEGORIES = {
...
###############################################################################
# 1) Utility to load data, map outlets
###############################################################################
...
def print_basic_stats(df, prefix=""):
    ...
    print(f"\nSummary Statistics for prefix={prefix or '(NoPrefix)'}:")
...
###############################################################################
# 3) Quotation vs Fulltext correlation
###############################################################################
def analyze_quotation_fulltext_correlation(
    ...
    """
    Original correlation analysis between averaged Quotation_(^sentiment_\\d+$)
    vs. <sentiment>_fulltext
    """
    ...
    if all_combo:
        ...
        # Write correlation + Z to Excel
        out_qftc=output_excel_qftc.replace(".xlsx", f"{suffix}.xlsx")
        with pd.ExcelWriter(out_qftc, engine="openpyxl") as writer:
    ...
    logging.info(f"Quotation-Fulltext correlation => {out_csv}")
...
def save_aggregated_scores_to_csv(agg_df, out_dir, prefix=""):
    ...
    print(f"Aggregated sentiment/emotion => {fn}")
    logging.info(f"Aggregated => {fn}")

def plot_statistics(agg_df, out_dir, prefix=""):
    logging.info("Plotting bar charts for Quotation/Fulltext across categories => prefix={prefix}")
...
def cross_validation_gee(data_with_score, formula, group_col, family, cov_struct, n_folds=3):
    # cross-validation fix => unify cat levels
...
def bootstrap_gee(df, formula, group_col, B=3, family=Poisson(), cov_struct=Independence()):
    ...
    for _ in range(B):
        ...
        mod=GEE.from_formula(formula, groups=group_col, data=boot_df, family=family, cov_struct=cov_struct)
        res=mod.fit()
        param_records.append(res.params)
    return pd.DataFrame(param_records)

###############################################################################
# 7) Multi-Family enumerations + 5 measure modes
###############################################################################
...
def refit_best_gee_with_scale(df, sentiment, measure, fam_name, struct, scale_name):
    """
    Refit with 5 measure modes => Quotation, Fulltext, Fulltext_Intensity, Title_Intensity, Quotation_Intensity
    """
...
def pairwise_and_diagnostics(df, sentiment, measure, fam_name, struct, scale_name):
    ...
    if final_fit is None: return (None,)*8
...
def run_gee_for_sentiment_measure_best_qic(df, sentiment, measure):
    """
    We do a quick test => measure columns exist,
    then run the multi-family approach => best QIC
    """
    # Quick test for columns
    # (similar logic to refit but simpler)
...
def run_gee_analyses_best_qic(df):
    logging.info("Running best QIC approach multi-families for each sentiment–measure => 5 measure modes.")
    measure_list=[
        "Quotation",
        "Fulltext",
        "Fulltext_Intensity",
...
def compile_results_into_multiple_workbooks(
    ...
    with pd.ExcelWriter(gee_excel, engine="openpyxl") as writer:
        ...
        for i,row in df_best_qic.iterrows():
            ...
            summary_txt, pair_df, diffIDs_df, diag_dict, pseudo_dict, cv_val, sens_df_df, boot_df=pairwise_and_diagnostics(df_full, s, meas, fam, st, sc)
            if summary_txt is None:
                tmp_df=pd.DataFrame({"Summary":[f"No valid model => {s}-{meas} or not enough data."]})
...
def run_pipeline_for_df(
    ...
    This covers:
      ...
      - aggregator
      - run_gee_analyses_best_qic => 5 measure modes
      - compile results
    """
    # Suffix => avoid overwriting
    if prefix:
...
###############################################################################
# main => subset first, wipe, then full
###############################################################################
def main():
    setup_logging()
    logging.info("Starting pipeline => SUBSET first, then wipe, then FULL => with intensity measures, cross-validation fix, etc.")

    # 1) Load data
    df=load_jsonl(INPUT_JSONL_FILE)
    print(f"Total articles loaded initially => {len(df)}")

    # 2) Normalize high_rate_2 => 'yes'/'no'
    if "high_rate_2" in df.columns:
        df["high_rate_2"] = df["high_rate_2"].astype(str).str.strip().str.lower()

    # 3) Map outlets => category
    df=map_media_outlet_to_category(df)

    # 4) Subset => df_yes
    if "high_rate_2" in df.columns:
        df_yes = df[df["high_rate_2"]=="yes"].copy()
        print(f"Subset df_yes => {len(df_yes)} rows with high_rate_2 == 'yes'")
        if len(df_yes)>0:
            print("Running pipeline on SUBSET => 'Yes'")
            run_pipeline_for_df(
                df_yes,
                prefix="Yes"
            )
        else:
            print("Subset is empty => no subset analysis.")
    else:
        print("No 'high_rate_2' column => cannot do subset analysis at all.")

    # 5) Wipe data => to avoid potential merges/overlaps
    del df
    if "df_yes" in locals():
        del df_yes
    gc.collect()

    logging.info("Memory wiped. Reloading data => full set...")

    # 6) Reload data => run full
    df_full=load_jsonl(INPUT_JSONL_FILE)
    print(f"Total articles loaded for full => {len(df_full)}")
    ...
    print("**Now running pipeline on FULL => 'All'**")
    run_pipeline_for_df(
        df_full,
        prefix="All"
    )

    logging.info("All analyses done. Exiting main().")
