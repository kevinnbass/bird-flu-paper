...
def build_cld(fit_result, pair_df):
    """
    Build letter groupings based on:
      - Sort by ascending mean
      - If group doesn't differ from all groups with a letter, share letter
      - Otherwise create a new letter
      - 'differ' is determined by 'reject_H0'=True => they do differ.

    pair_df must have columns ["CategoryA","CategoryB","Difference","SE","Z","p_value","p_value_adj","reject_H0"].
    """
    ...
    # 2) Sort groups by ascending mean
    sorted_cats = sorted(cats, key=lambda x: mean_map[x])

    # 3) Build differ dictionary
    #    differ[a][b] = True if they differ significantly (reject_H0=True)
    differ = {c: {} for c in cats}
    for row in pair_df.itertuples(index=False):
        ca, cb, diffest, se, z, p_val, p_adj, reject = row
        differ[ca][cb] = reject
        differ[cb][ca] = reject
    ...
    for group in sorted_cats:
        assigned = False
        for letter in letters_used:
            # Find existing groups that share this letter
            groups_with_letter = [g for g in cats if letter in cat_letters[g]]

            # If it "differs" from ANY group that has this letter => conflict
            conflict = any(differ[group].get(g, False) for g in groups_with_letter)
