...
def exclude_small_clusters_within_category(
    ...
    and re-casts the group_col as a categorical so that the changes persist.
    """
    df2 = df.copy()

    total_merged = 0  # just a debug counter

    # For each category, identify outlets with fewer than min_size articles
    categories = df2[cat_col].dropna().unique()
    for cat in categories:
        # Subset only this category
        mask_cat = (df2[cat_col] == cat)
        sub = df2[mask_cat]

        # Count how many times each outlet appears (in this category)
        counts = sub[group_col].value_counts(dropna=False)

        # Identify the "small" outlets
        small_outlets = counts[counts < min_size].index
        if len(small_outlets) > 0:
            # Mark them as "OtherSmall" but only within this category
            mask_small = (df2[group_col].isin(small_outlets)) & mask_cat
            df2.loc[mask_small, group_col] = merge_into
            total_merged += mask_small.sum()

    # Convert to categorical so that "OtherSmall" is recognized as a valid group
    df2[group_col] = df2[group_col].astype("category")

    # Debug prints
    print(f"[DEBUG] exclude_small_clusters_within_category => merged {total_merged} rows "
          f"into '{merge_into}' for outlets with < {min_size} articles in their category.")
    print(f"[DEBUG] After merging => #unique outlets: {df2[group_col].nunique()}")
    return df2
...
def append_sens_to_excel(
    ...
    """
    Extends existing Excel files with 'SENS' results, 
    including pairwise results for each best-QIC outcome.
    """
    ...
    ws_agg = wb_main.create_sheet("Aggregated_Scores_Sens")
    for r in dataframe_to_rows(aggregated_df_sens, index=False, header=True):
        ws_agg.append(r)

    ws_stats = wb_main.create_sheet("Mean_Median_Statistics_Sens")
    for r in dataframe_to_rows(stats_df_sens, index=False, header=True):
        ws_stats.append(r)

    wb_main.save(main_excel)

    # 2) raw => raw_excel => put in "Raw_Data_Sens"
    if os.path.exists(raw_excel):
        wb_raw = load_workbook(raw_excel)
    else:
        wb_raw = Workbook()
        if "Sheet" in wb_raw.sheetnames:
            wb_raw.remove(wb_raw["Sheet"])

    ws_raw = wb_raw.create_sheet("Raw_Data_Sens")
    ...
    wb_raw.save(raw_excel)

    # 3) GEE => combos, best QIC index for Sens, *plus* produce pairwise results
    if os.path.exists(gee_excel):
        wb_gee = load_workbook(gee_excel)
    else:
        wb_gee = Workbook()
        if "Sheet" in wb_gee.sheetnames:
            wb_gee.remove(wb_gee["Sheet"])

    # Insert all combos
    if not df_all_combos_sens.empty:
        ws_ac = wb_gee.create_sheet("All_Combos_Sens")
        combos_rows = dataframe_to_rows(df_all_combos_sens, index=False, header=True)
        for r in combos_rows:
            ws_ac.append(r)

    # Insert best QIC index
    ws_idx = wb_gee.create_sheet("BestQIC_Index_Sens")
    ...
    # --- NEW STEP: for each best-QIC row, produce a pairwise sheet ---
    for i, row in df_best_qic_sens.iterrows():
        ...
        sheet_name = f"BestQIC_Sens_{s[:8]}_{meas[:12]}"
        wss = wb_gee.create_sheet(sheet_name)

        # Run the diagnostics on the SENS dataframe
        out = pairwise_and_diagnostics(
            df_full_sens, s, meas, fam, st, sc,
            prefix="SENS", isSens=True
        )
        summary_txt, pair_df, diffIDs_df, diag_dict, pseudo_dict, cv_val, sens_df_df, boot_info = out

        if summary_txt is None:
            tmp_df = pd.DataFrame({"Summary": [f"No valid SENS model => {s}-{meas}"]})
            rows_ = dataframe_to_rows(tmp_df, index=False, header=True)
            for rr in rows_:
                wss.append(rr)
            continue
        ...
        # 2) Pairwise table
        if not pair_df.empty:
            p_rows = dataframe_to_rows(pair_df, index=False, header=True)
            for rr in p_rows:
                wss.append(rr)
            wss.append([])
        ...
        # 3) diffIDs_df
        if diffIDs_df is not None and not diffIDs_df.empty:
            d_rows = dataframe_to_rows(diffIDs_df, index=False, header=True)
            for rr in d_rows:
                wss.append(rr)
            wss.append([])

        # 4) Some textual checks
        wss.append([
            f"SENS Overdisp={diag_dict['overdisp_ratio']}, "
            f"Corr={diag_dict['avg_within_corr']}",
            diag_dict["assessment"]
        ])
        wss.append([
            f"SENS NB_QIC={pseudo_dict['NB_QIC']}, "
            f"Poisson_QIC={pseudo_dict['Poisson_QIC']}, "
            f"diff={pseudo_dict['diff_QIC']}, "
            f"conclusion={pseudo_dict['conclusion']}"
        ])
        wss.append([f"SENS CrossVal MSE={cv_val}"])
        wss.append([])

    wb_gee.save(gee_excel)

    # 4) combined => aggregator + stats => SENS
    if os.path.exists(combined_excel):
        wb_comb = load_workbook(combined_excel)
    else:
        wb_comb = Workbook()
        if "Sheet" in wb_comb.sheetnames:
            wb_comb.remove(wb_comb["Sheet"])

    ws_agg2 = wb_comb.create_sheet("Aggregated_Scores_Sens")
    for r in dataframe_to_rows(aggregated_df_sens, index=False, header=True):
        ws_agg2.append(r)

    ws_stats2 = wb_comb.create_sheet("Mean_Median_Statistics_Sens")
    for r in dataframe_to_rows(stats_df_sens, index=False, header=True):
        ws_stats2.append(r)
...
def run_pipeline_for_df(
    ...
    """
    Standard pipeline that DOES merge small outlets in the SENS step.
    1) Basic checks & logging
    2) Chunk + basic stats
    3) Correlations, aggregator, stats, plots
    4) GEE => best combos
    5) Compile main results => Excel
    6) SENS => merges small outlets (< min_size) per category => "OtherSmall"
    """
    # 1) Rename files for 'prefix'
    ...
    print(f"[{prefix}] Best QIC combos =>")
    print(df_best)

    # 6) Compile into multiple Excel files
    ...
    # 7) SENS run => merges small outlets < min_size => "OtherSmall"
    from copy import deepcopy
    print(f"[SENS merging step] => prefix={prefix}, min_size={min_size}")
    ...
    # Check how many unique clusters remain:
    unique_outlets_after = df_sens["media_outlet_clean"].nunique()
    print(f"[DEBUG] SENS => after merging => #unique outlets: {unique_outlets_after}")

    if len(df_sens) < 2:
        print(f"[Sensitivity => {prefix}] => Not enough data after merges => skip.")
        return
    ...
    print(f"[SENS => {prefix}] Best QIC combos =>")
    print(df_best_sens)

    # Save aggregated => with suffix
    save_aggregated_scores_to_csv(agg_sens, csvout, prefix=f"{prefix}_Sens")

    # Finally append SENS to Excel
    append_sens_to_excel(
...
def run_pipeline_for_df_no_collapse(
    df,
    prefix="",
    main_excel=BASE_MAIN_FILE,
    raw_excel=BASE_RAW_FILE,
    ...
    """
    A variant that does NOT merge small outlets at all.
    Produces a single analysis (no SENS step).
    """

    # 1) Rename output files for this prefix
    main_excel    = rename_files_for_prefix(main_excel,    prefix)
    raw_excel     = rename_files_for_prefix(raw_excel,     prefix)
    gee_excel     = rename_files_for_prefix(gee_excel,     prefix)
    plots_excel   = rename_files_for_prefix(plots_excel,   prefix)
    ...
    # 7) We do NOT do a SENS run here.
    print(f"[NoCollapse] => Completed pipeline for prefix={prefix} with NO merging step.")
...
def main():
    setup_logging()
    logging.info("Starting pipeline => 1)Yes => 2)All => 3)All_NoCollapse")
    ...
    # A) Map categories => using original categories
    df_orig = map_media_outlet_to_category(df_raw, MEDIA_CATEGORIES_ORIG)

    # 1) Subset where high_rate_2 == yes => run prefix="Yes"
    df_yes = df_orig[df_orig["high_rate_2"] == "yes"].copy()
    if len(df_yes) > 0:
        print("**Pipeline => YES** subset (with SENS merging).")
        run_pipeline_for_df(df_yes, prefix="Yes")
        del df_yes
        gc.collect()
    else:
        print("No rows => skip 'Yes' subset analysis.")

    # 2) Full data => run prefix="All" => includes SENS merging
    print("**Pipeline => ALL** (with SENS merging).")
    run_pipeline_for_df(df_orig, prefix="All")

    gc.collect()

    # 3) Full data again => NO collapsing => run_pipeline_for_df_no_collapse
    print("**Pipeline => ALL_NoCollapse** (no SENS merging).")
    run_pipeline_for_df_no_collapse(
        df_orig,
        prefix="All_NoCollapse"
    )

    del df_orig
    gc.collect()

    logging.info("All done => (1)Yes, (2)All, (3)All_NoCollapse => original cats only.")
