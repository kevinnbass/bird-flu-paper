...
Script that:
  1) Loads data, does correlation analyses
     - Quotation vs Fulltext
     - Quotation_Intensity vs Fulltext_Intensity
     - Fulltext_Intensity vs Fulltext
  2) Aggregates stats
  3) Finds best QIC combination (multi-family, GEE) for 5 measure modes:
  ...
  7) Replicates the entire pipeline for "high_rate_2 == yes" subset,
     but collapses that subset's clusters to 6 categories:
       df_yes["media_outlet_clean"] = df_yes["media_category"]
  8) Then we do the normal approach for the "All" set
     => prefix-based naming so we don't overwrite.
"""
...
# Original correlation file => Quotation vs Fulltext
OUTPUT_EXCEL_QFTC = "quotation_fulltext_correlation.xlsx"
# Additional correlation => Quotation_Intensity vs Fulltext_Intensity
OUTPUT_EXCEL_QIvFI = "quotation_intensity_fulltext_intensity_correlation.xlsx"
# Additional correlation => Fulltext_Intensity vs Fulltext
...
def analyze_2fields_correlation(
    ...
    """Generic correlation analysis between an averaged pattern vs. a single column."""
    from scipy.stats import pearsonr
    suffix = f"_{prefix}" if prefix else ""
    logging.info(f"Analyzing correlation => {correlation_title}, prefix={prefix}")
    ...
    plt.title(f"{correlation_title} Correlation - {prefix}")
    ...
    logging.info(f"{correlation_title} correlation => {out_csv}")


def analyze_all_custom_correlations(df, prefix=""):
    """
    1) Quotation vs Fulltext  => left='^<sent>_\\d+$', right='<sent>_fulltext'
    2) Quotation_Intensity vs Fulltext_Intensity => left='^<sent>_\\d+_intensity$', right='<sent>_fulltext_intensity'
    3) Fulltext_Intensity vs Fulltext => left='<sent>_fulltext_intensity', right='<sent>_fulltext'
    """
    # 1) Quotation vs Fulltext
    ...
    # 2) Quotation_Intensity vs Fulltext_Intensity
    analyze_2fields_correlation(
        df=df,
    ...
    # 3) Fulltext_Intensity vs Fulltext
    analyze_2fields_correlation(
        df=df,
...
def aggregate_sentiment_scores(df, sentiments):
    logging.info("Aggregating sentiment/emotion by category + sentiment.")
    recs=[]
...
def plot_statistics(agg_df, out_dir, prefix=""):
    logging.info(f"Plotting bar charts => prefix={prefix}")
    ...
    for s in CATEGORIES:
        ...
        plt.title(f"Mean Quotation '{s}' Scores {prefix}")
        plt.xticks(rotation=45, ha="right")
        plt.tight_layout()
        ...
        plt.title(f"Mean Fulltext '{s}' Scores {prefix}")
        plt.xticks(rotation=45, ha="right")
        plt.tight_layout()
...
###############################################################################
# 6) Additional checks & Validation
###############################################################################
...
###############################################################################
# 7) Multi-Family enumerations + measure modes
###############################################################################
...
def refit_best_gee_with_scale(df, sentiment, measure, fam_name, struct, scale_name):
    """
    5 measure modes => Quotation, Fulltext, Fulltext_Intensity, Title_Intensity, Quotation_Intensity
    """
    if measure=="Quotation":
        ...
        if fcol not in df.columns:return None,None
        d2=df.copy()
...
def pairwise_and_diagnostics(df, sentiment, measure, fam_name, struct, scale_name):
    ...
    for i in range(len(cats)):
        for j in range(i+1,len(cats)):
            ...
            if not np.isnan(diff_se) and diff_se>0:
                z=diff_est/diff_se
...
def run_gee_for_sentiment_measure_best_qic(df, sentiment, measure):
    ...
    if measure=="Quotation":
        ...
        fcol=f"{sentiment}_fulltext"
        if fcol not in d2.columns:return None
        d2["_score_col"]=d2[fcol].clip(lower=0)
    elif measure=="Fulltext_Intensity":
        fcol=f"{sentiment}_fulltext_intensity"
        if fcol not in d2.columns:return None
    ...
    if best_tuple is None:return None
...
def run_gee_analyses_best_qic(df):
    logging.info("Running best QIC approach multi-families => 5 measure modes.")
    measure_list=[
        "Quotation",
        "Fulltext",
        "Fulltext_Intensity",
...
def compile_results_into_multiple_workbooks(
    ...
    cbar=os.path.join(plots_dir,"correlation__quotation_vs_fulltext_bar.png")
...
###############################################################################
# 9) run_pipeline_for_df => single place to run
###############################################################################
def run_pipeline_for_df(
    ...
    3) Aggregation => stats
    4) GEE => 5 measure modes
    5) compile => Excel
    """
    # Add prefix => avoid overwriting
    if prefix:
    ...
    print(f"Best QIC approach => prefix={prefix}")
    print(df_best)
...
###############################################################################
# main => subset first, then full
###############################################################################
def main():
    setup_logging()
    logging.info("Starting pipeline => subset first => collapsed clusters => then wipe => then full => normal clusters.")

    # 1) Load
    df=load_jsonl(INPUT_JSONL_FILE)
    print(f"Total articles => {len(df)}")

    # 2) Normalize 'high_rate_2'
    ...
    if "high_rate_2" in df.columns:
        ...
        print(f"df_yes => {len(df_yes)} rows => 'yes' subset")

        if len(df_yes)>0:
            # *** Key step: collapse clusters => each category => single cluster
            # So we do:
            df_yes["media_outlet_clean"] = df_yes["media_category"]
            # i.e. 6 categories => 6 clusters

            print("**Running pipeline on SUBSET => 'Yes' => collapsed 6 categories**")
            run_pipeline_for_df(
                df_yes,
                prefix="Yes"
            )
        else:
            print("Subset => empty => skip subset analysis.")
    else:
        print("No 'high_rate_2' => skip subset analysis entirely.")
    ...
    if "df_yes" in locals():
        del df_yes
    gc.collect()
    logging.info("Memory wiped => now reloading for the FULL run...")

    # 5) reload => full => normal approach
    df_full=load_jsonl(INPUT_JSONL_FILE)
    print(f"df_full => {len(df_full)} rows => FULL set")
    ...
    # *** We do NOT overwrite 'media_outlet_clean' => we keep normal approach
    print("**Now running pipeline on FULL => 'All' => normal approach**")
    run_pipeline_for_df(
        df_full,
        prefix="All"
    )

    logging.info("All done => subset first (collapsed clusters) then full => normal clusters. Exiting main().")
