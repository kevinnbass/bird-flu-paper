...
# Convert all outlets to lowercase to match the `.lower().strip()` logic
MEDIA_CATEGORIES = {
...
def setup_logging(log_file='analysis.log'):
    logging.basicConfig(
        filename=log_file,
        level=logging.DEBUG,  # Set to DEBUG to capture more detailed info
...
def map_media_outlet_to_category(df, media_categories):
    ...
    for category, outlets in media_categories.items():
        for outlet in outlets:
            # Ensure mapping dictionary also uses lowercase and stripped values
...
def fit_lmm_and_posthoc(df, sentiment, measure_type='Quotation'):
    ...
    if row_count < 2:
        logger.debug("Not enough data points to fit the model (less than 2 rows). Skipping.")
        return None
    if category_count < 2:
        logger.debug("Less than 2 categories in data. Cannot estimate category effects. Skipping.")
        return None
    ...
    if outlet_counts.min() < 2:
        logger.debug("Some media_outlets have fewer than 2 observations. This may cause singular covariance.")
    ...
    if cat_means['media_category'].nunique() < 2:
        logger.debug("Only one category after grouping for post-hoc. Skipping post-hoc tests.")
        return {
...
def compile_statistics_to_excel(aggregated_df, stats_df, plots_dir, output_excel_file, raw_df, lmm_results):
    with pd.ExcelWriter(output_excel_file, engine='openpyxl') as writer:
        ...
        # Write raw scores for each sentiment
        for sentiment in CATEGORIES:
