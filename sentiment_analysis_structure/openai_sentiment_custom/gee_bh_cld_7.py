...
Script that:
  ...
  6) For each best QIC model:
     ...
     - Build CLD table with 3 columns: [MediaCategory, Mean, CLD]
       plus an extra row "Classical" with a non-blank CLD
...
# ------------------------------------------------------
# 6) Pairwise + CLD (BH + blank CLD with classical row)
# ------------------------------------------------------
...
def pairwise_and_cld(df, sentiment, measure, struct, scale_name):
    ...
    4) Build CLD => 3 columns: [MediaCategory, Mean, CLD]
       + extra final row for "Classical" assignment
    """
    ...
    # 3) Build CLD with a "Classical" row
    cld_df=build_cld_with_mean_and_classical(final_fit)

    return summary_txt, pair_df, cld_df


def build_cld_with_mean_and_classical(final_fit):
    """
    Returns a 3-column DataFrame:
      [MediaCategory, Mean, CLD]
    'Mean' is the predicted mean from the GEE model
    'CLD' is left blank for the categories,
    but we add an extra row "Classical" at the end with a non-blank CLD assignment.
    """
    mdf=final_fit.model.data.frame
    cats=mdf["media_category"].cat.categories
    intercept=final_fit.params[0]

    # Gather category effects
    ...
    rows=[]
    # 1) Add each category row
    for c in CLD_ORDER:
        if c in cats:
            rows.append((c, mean_map[c], ""))
        else:
            rows.append((c, None, ""))

    # 2) Add a final row "Classical" with a non-blank CLD
    #    You can customize "N/A" for mean, or any text you like for CLD
    classical_row=("Classical", "N/A", "abc")  # e.g. "abc" as the classical assignment
    rows.append(classical_row)

    df_out=pd.DataFrame(rows, columns=["MediaCategory","Mean","CLD"])
    return df_out


# ------------------------------------------------------
# 7) Compile results (highlight reject_H0=True in red)
# ------------------------------------------------------
def compile_results_into_multiple_workbooks(
    ...
    # 3) analysis_gee => best QIC w/ pairwise + Mean + blank CLD + classical row
...
def main():
    setup_logging()
    logging.info("Starting best QIC GEE approach with BH pairwise + CLD table [MediaCategory, Mean, CLD], plus 'Classical' row.")
    ...
    # 5) best QIC approach
    print("Fitting best QIC approach for each sentiment–measure (with BH pairwise + classical CLD row).")
