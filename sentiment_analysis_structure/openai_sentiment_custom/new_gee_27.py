...
def fit_gee_and_pairwise(df, sentiment, measure_type='Quotation', cov_struct_type=Exchangeable,
    ...
    """
    AR(1) Steps:
      1) Convert date -> integer day index from earliest date => (date - min_date).dt.days + 1
      2) filter to clusters with >=2 obs
      3) sort by (media_outlet, int_date)
      4) pass int_date as an int32 array to time=...
    """
    ...
    # Must have at least 2 categories for a valid model
    if model_df['media_category'].nunique() < 2:
        return None

    # Convert date -> integer days from earliest date
    model_df['int_date'] = (model_df['date'] - global_min_date).dt.days + 1
    # int_date is guaranteed integer. Make it int32
    model_df['int_date'] = model_df['int_date'].astype(np.int32)

    if len(model_df) < 2:
        return None

    # Make sure categories are typed
    ...
    if cov_struct_type == Autoregressive:
        # Only keep clusters with >=2 obs
        model_df = filter_clusters_for_ar(model_df, 'media_outlet')
        if len(model_df) < 2:
            return None
        # Sort by media_outlet, int_date
        ...
        # time is just int_date
        time = model_df['int_date'].values  # already int32
...
def try_all_cov_structures(df, sentiment, measure_type, global_min_date):
    # We'll try all 4 structures, skipping invalid ones
    ...
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        for sentiment in sentiments:
            ...
            for idx, row in df.iterrows():
                ...
                row_values = {c: np.nan for c in category_order}
                if row['media_category'] in category_order:
                    row_values[row['media_category']] = val
                for c in category_order:
                    cat_data_quotation[c].append(row_values[c])
            ...
            for idx, row in df.iterrows():
                ...
                if row['media_category'] in category_order:
                    row_values[row['media_category']] = val
                for c in category_order:
                    cat_data_fulltext[c].append(row_values[c])
...
def main():
    setup_logging()
    print("Single run with GEE and pairwise comparisons (Poisson family), testing multiple covariance structures including AR(1) using 'date' as integer day index.")
    df = load_jsonl(INPUT_JSONL_FILE)
    print(f"Total articles loaded: {len(df)}")
    df = map_media_outlet_to_category(df, MEDIA_CATEGORIES)

    if 'date' not in df.columns:
        print("Warning: 'date' field not found. Skipping AR(1) approach.")
    else:
    ...
    for i in range(0, len(df), chunk_size):
        ...
        outpath = os.path.join(CSV_OUTPUT_DIR, f"raw_data_part_{i//chunk_size+1}.csv")
        chunk.to_csv(outpath, index=False)
        print(f"Saved chunk {i//chunk_size+1} to {outpath}")
    ...
    for sentiment in CATEGORIES:
        sentiment_data = aggregated_df.loc[aggregated_df['Sentiment/Emotion'] == sentiment].copy()
        sentiment_data = sentiment_data.loc[sentiment_data['Media Category'].isin(MEDIA_CATEGORIES.keys())]
        sentiment_data.dropna(subset=['Quotation_Average', 'Fulltext_Average'], inplace=True)

        if len(sentiment_data) > 1:
            corr_val, _ = pearsonr(sentiment_data['Quotation_Average'], sentiment_data['Fulltext_Average'])
        else:
            corr_val = np.nan

        correlation_results.append({'Sentiment/Emotion': sentiment, 'Correlation': corr_val})
        ...
        plt.tight_layout()
        scatter_out = os.path.join(OUTPUT_DIR,f"scatter_{sentiment}.png")
        plt.savefig(scatter_out)
        plt.close()
        print(f"Scatter plot for {sentiment} saved to '{scatter_out}'.")
    ...
    plt.ylim(-1,1)
    plt.tight_layout()
    corrplot_out = os.path.join(OUTPUT_DIR,"correlation_quotation_fulltext.png")
    plt.savefig(corrplot_out)
    plt.close()
    print(f"Correlation plot saved to '{corrplot_out}'.")
    ...
    with pd.ExcelWriter(OUTPUT_EXCEL_CORRELATION, engine='openpyxl') as writer:
        ...
        for sentiment, sdat in correlation_scatter_data.items():
            sdat.to_excel(writer, sheet_name=f"{sentiment}_Data", index=False)

    # Combine for all-sentiment scatter
    combined_list = []
    for sentiment, sdata in correlation_scatter_data.items():
        dtemp = sdata.copy()
        dtemp['Sentiment/Emotion'] = sentiment
        combined_list.append(dtemp)
    ...
    if len(combined_nonan)>1:
        r_val, _ = pearsonr(combined_nonan['Quotation_Z'], combined_nonan['Fulltext_Z'])
    else:
        r_val = np.nan

    plt.figure(figsize=(10,8))
    sns.regplot(x='Quotation_Z',y='Fulltext_Z', data=combined_nonan,
    ...
    plt.tight_layout()
    comb_scatter_out = os.path.join(OUTPUT_DIR,"combined_normalized_scatter.png")
    plt.savefig(comb_scatter_out)
    plt.close()
    print(f"Combined normalized scatter plot saved to '{comb_scatter_out}'.")

    print("No predictor columns specified for collinearity check.")
    print("Fitting GEE models + pairwise comparisons with multiple covariance structures (including AR(1) with integer day index).")
    gee_results, qic_df = run_gee_analyses(df)
    print("GEE analysis + pairwise tests completed with sensitivity analysis + QIC comparison.\n")
