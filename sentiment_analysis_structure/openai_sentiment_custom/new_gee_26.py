...
def fit_gee_and_pairwise(df, sentiment, measure_type='Quotation', cov_struct_type=Exchangeable, global_min_date=None):
    ...
    # Convert date to global integer days since min_date
    model_df['int_date'] = (model_df['date'] - global_min_date).dt.days + 1
    # int_date is now an integer representing the day count since earliest date
    # No need to dropna here since we ensured date is valid
    ...
    if cov_struct_type == Autoregressive:
        # Filter clusters so all have at least 2 obs
        model_df = filter_clusters_for_ar(model_df)
        if len(model_df) < 2 or model_df['media_outlet'].nunique() < 1:
            return None

        # Sort by media_outlet and int_date
        model_df = model_df.sort_values(['media_outlet', 'int_date'])
        # time is just int_date now (already integer)
        time = model_df['int_date'].astype(np.int32).values

        # Check again if every cluster has at least 2 points
        ...
        # For other structures no special filtering needed
        # Sorting not strictly required, but let's keep data stable
        time = None
...
def try_all_cov_structures(df, sentiment, measure_type, global_min_date):
    ...
    for struct in structures:
        res = fit_gee_and_pairwise(df, sentiment, measure_type=measure_type, cov_struct_type=struct, global_min_date=global_min_date)
        if res is not None:
            qic_val = res['QIC']
...
def run_gee_analyses(df):
    # We'll find min_date here and pass to try_all_cov_structures
    ...
    for sentiment in CATEGORIES:
        ...
        for measure_type in ['Quotation', 'Fulltext']:
            results_list = try_all_cov_structures(df, sentiment, measure_type, global_min_date=min_date)
