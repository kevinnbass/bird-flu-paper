...
def run_gee_for_measure_best_qic(df, measure):
    ...
    """
    d2 = df.copy()
    # Check that measure is present
    if measure not in d2.columns:
        return None

    # Clean up the measure => numeric
    ...
    # Ensure "media_category" is truly categorical
    d2["media_category"] = d2["media_category"].astype("category")

    # Clip measure < 0 if desired
...
def refit_best_gee_with_scale(df, measure, fam_name, struct, scale_name):
    ...
    d2 = df.copy()
    # Convert measure to numeric, drop NAs
    if measure not in d2.columns:
        return None, None
    d2[measure] = pd.to_numeric(d2[measure], errors="coerce")
    d2 = d2.dropna(subset=["media_outlet_clean","media_category", measure])
    ...
    # Here, we do no clipping or you can do it if you want
...
def pairwise_and_diagnostics(df, measure, fam_name, struct, scale_name):
    ...
    # Additional lines => ensure category dtype in final_fit's data
    ...
    # Now that we forced category above:
...
def compile_results_into_multiple_workbooks(
    ...
    with pd.ExcelWriter(raw_excel, engine="openpyxl") as w:
        ...
        # As a convenience, also create sheets for each measure:
        for m in MEASURES:
