...
def pairwise_and_diagnostics(
    ...
    def safe_mean_to_dict(df_in):
        ...
        If the DataFrame has only one column, .mean(axis=0) becomes a scalar,
        so we re-wrap it in a dict with that column name.
        """
        if df_in.empty:
            return {}

        # mean_result can be a Series or a scalar
        mean_result = df_in.mean(axis=0)  # compute along columns

        # If mean_result is a scalar, that means df_in had 1 column => re-wrap
        if np.isscalar(mean_result):
            # df_in has exactly 1 column => get that column name
            col_name = df_in.columns[0]
            return {col_name: round(float(mean_result), 4)}

        # Otherwise mean_result is a Series => just .round().to_dict()
