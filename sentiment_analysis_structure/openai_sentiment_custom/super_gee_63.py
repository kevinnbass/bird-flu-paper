...
def append_sens_to_excel(
    ...
    """
    Extends existing Excel files with 'SENS' results, writes aggregated
    + stats into main & combined, plus raw data. Also loops over the final
    best combos (df_best_qic_sens), runs pairwise_and_diagnostics,
    and produces "BestQIC_Sens_{sent}_{meas}" sheets in the same GEE file.
    """
    ...
    # -- 3) GEE => combos, best QIC index for Sens, *plus* pairwise tables
    if os.path.exists(gee_excel):
    ...
    if not df_all_combos_sens.empty:
        ws_comb = wb_gee.create_sheet("All_Combos_Sens")
    ...
    # -- Now the new step:
    #    For each row in df_best_qic_sens, run pairwise_and_diagnostics
    #    and create a new sheet "BestQIC_Sens_{s}_{m}" with the GEE summary & pairwise tables.
    from . import pairwise_and_diagnostics  # or if it's local in the same file, remove import
    for i, row in df_best_qic_sens.iterrows():
        ...
        # We'll create a sheet name like "BestQIC_Sens_anger_Fulltext"
        ...
        if out[0] is None:
            ...
            # Let's write that directly to a new sheet:
            wss = wb_gee.create_sheet(sheet_name)
            for r_ in dataframe_to_rows(tmp_df, index=False, header=True):
                wss.append(r_)
            continue

        summary_txt, pair_df, diffIDs_df, diag_dict, pseudo_dict, cv_val, sens_df_df, boot_info = out

        # -- Create the new sheet and place the results
        wss = wb_gee.create_sheet(sheet_name)

        # 1) GEE summary
        lines = summary_txt.split("\n")
        row_ = 1
        wss.cell(row=row_, column=1, value="GEE_Summary")
        row_ += 1
        for ln in lines:
            wss.cell(row=row_, column=1, value=ln)
            row_ += 1
        row_ += 1
        ...
        if not pair_df.empty:
            ...
            for rr in p_rows:
                for cc, val_ in enumerate(rr, start=1):
                    wss.cell(row=row_, column=cc, value=val_)
                row_ += 1
            row_ += 2
        ...
        # Optionally store diag_dict, pseudo_dict, etc.
        wss.cell(row=row_, column=1,
                 value=f"SENS Overdisp={diag_dict['overdisp_ratio']} "
                       f"Corr={diag_dict['avg_within_corr']} => {diag_dict['assessment']}")
        row_ += 2

        wss.cell(row=row_, column=1,
                 value=f"SENS NB_QIC={pseudo_dict['NB_QIC']}, "
                       f"Poisson_QIC={pseudo_dict['Poisson_QIC']}, diff={pseudo_dict['diff_QIC']}, "
                       f"conclusion={pseudo_dict['conclusion']}")
        row_ += 2

        wss.cell(row=row_, column=1, value=f"SENS CrossVal MSE={cv_val}")
        row_ += 2

        # You can also store the bootstrap info as text if desired
        row_ += 1

    # End for each best combo in df_best_qic_sens

    wb_gee.save(gee_excel)

    # -- 5) combined => aggregator + stats => SENS
