...
Script that:
  ...
  6) For each best QIC model:
     ...
     - Compute pairwise comparisons (**Hommel correction**)
     - Build LSD-like CLD based on predicted means
...
def pairwise_and_cld(df, sentiment, measure, struct, scale_name):
    ...
    2) GEE summary
    3) Pairwise (using Hommel correction)
    4) LSD-based CLD
    """
    ...
    # Hommel correction
    # -------------------
    rej, p_adj, _, _ = multipletests(pair_df["p_value"], method="hommel")
...
def build_lsd_cld(fit_result, pair_df):
    ...
    2) sort ascending
    3) LSD-based letter assignment => differ => no share letter
    4) reorder final in CLD_ORDER
    """
    mdf=fit_result.model.data.frame
    cats=mdf["media_category"].cat.categories
    # get intercept & category effect
    intercept=fit_result.params[0]
    cat_effect={}
    # "media_category[T.x]" => effect
    for c in cats[1:]:
        nm=f"media_category[T.{c}]"
    ...
    # Build differ map => if reject => can't share letter
    differ={}
    ...
    for c in sorted_cats:
        ...
        for let in letters:
            # check conflict
            group_cats=[cx for cx in cats if let in cat_letters[cx]]
            conflict=False
            for gx in group_cats:
                # if differ[c][gx] => True => can't share letter
                if differ[c].get(gx,False) is True:
                    conflict=True
                    break
            if not conflict:
...
def main():
    setup_logging()
    logging.info("Starting best QIC GEE approach with LSD-based CLD (final).")
    ...
    # 5) best QIC approach
    print("Fitting best QIC approach for each sentiment–measure (with pairwise + LSD-based CLD).")
