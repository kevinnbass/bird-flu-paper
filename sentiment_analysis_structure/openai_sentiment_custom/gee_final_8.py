#!/usr/bin/env python3
# gee_final_best_qic_unstructured_time.py
"""
Script that:
  1) Loads data from JSONL
  2) Parses 'date' column -> integer days from earliest date => 'time_int'
  3) Chunks & saves CSV
  ...
  6) Finds best QIC combination (Ind/Exch/Unstructured × scale=[none,pearson,deviance,ub,bc])
     with 'time=time_int' only for Unstructured
...
def run_gee_for_sentiment_measure_best_qic(df, sentiment, measure):
    ...
    Each => we handle time= if Unstructured => pass time_int
    """
    # Subset data: build _score_col
    d2=df.copy()
    ...
    # For "Independence"/"Exchangeable", we can do from_formula
    # For "Unstructured", we must pass time= d2["time_int"]
    for cov_obj in structures:
        st_name=cov_obj.__class__.__name__
        try:
            if st_name=="Unstructured":
                # we must build endog, exog manually
                endog = d2["_score_col"].values
                exog = sm.add_constant(pd.get_dummies(d2["media_category"], drop_first=False))
...
def refit_best_gee_with_scale(df, sentiment, measure, struct, scale_name):
    ...
    For Unstructured => pass endog, exog, groups, time
    For others => from_formula
    """
    d2=df.copy()
    if measure=="Quotation":
        pat=rf"^{re.escape(sentiment)}_\d+$"
...
def build_lsd_cld(fit_result, pair_df):
    ...
    for c in sorted_cats:
        ...
        if not sets_to_merge:
            ...
            # we might store a "representative" name => sorted of all letters in letters_from_merged
...
def main():
    setup_logging()
    logging.info("Starting best QIC GEE approach with Unstructured requiring integer time from 'date'.")
