...
def setup_logging(log_file='analysis.log'):
    """Sets up file+console logging and suppresses font-manager logs."""
...
def map_media_outlet_to_category(df, media_categories):
    ...
    unmap= df.loc[df['media_category']=='Other','media_outlet'].unique()
    if len(unmap)>0:
        logging.warning(f"Unmapped => 'Other': {unmap}")
        print(f"Warning: Some outlets mapped to 'Other': {unmap}")
    return df
...
def examine_ar1_data(df, score_col):
    """
    Dump some stats about the group sizes and variances to see
    if there's a pattern that might break AR(1).
    """
    group_counts = df.groupby('media_outlet').size().sort_values()
    group_variances = df.groupby('media_outlet')[score_col].var()
    logging.debug(f"AR(1) debug => group size stats: min={group_counts.min()}, max={group_counts.max()}")
    logging.debug(f"AR(1) debug => # groups total={len(group_counts)}. # with 1 obs => {(group_counts==1).sum()}")
    logging.debug(f"AR(1) debug => # zero-variance groups => {(group_variances==0).sum()}")
    # You could also dump group_counts and group_variances to a CSV if needed for deeper inspection:
    # group_counts.to_csv('group_counts_debug.csv')
    # group_variances.to_csv('group_variances_debug.csv')

def remove_zero_var_groups(df, score_col):
    gvar= df.groupby('media_outlet')[score_col].var()
    zero_var= gvar[gvar==0].index
    if len(zero_var)>0:
        logging.debug(f"Removing {len(zero_var)} zero-variance groups: {list(zero_var)}")
...
def fit_gee_model(df, formula, groups, cov_struct, family=Poisson(), time=None):
    """
    If cov_struct is Autoregressive => do an extra AR(1)-specific debug check
    (group size, group var).
    """
    logging.debug(f"fit_gee_model => structure={cov_struct.__class__.__name__}, time={'YES' if time is not None else 'NO'}")

    # For demonstration, let's do extra debug if AR(1)
    if isinstance(cov_struct, Autoregressive):
        # We'll guess the outcome is the LHS of the formula
    ...
    try:
        model= GEE.from_formula(formula, groups=groups, data=df, cov_struct=cov_struct,
                                family=family, time=time, scale=1)  # scale=1 as example
    except Exception as e:
    ...
    try:
        ...
        # More explicit mention of AR(1) bracketing:
...
def run_gee_and_qic(df, sentiment, measure_type, structures, min_date=None):
    ...
    if measure_type=='Quotation':
        ...
        matched= [c for c in df.columns if re.match(pat,c)]
        if not matched:
            logging.debug("No matched cols => returning {}")
            return {}
        tmp= df.copy()
    ...
    if len(mdf)<2:
        logging.debug("All data removed => {}")
        return {}
...
def fit_and_pick_best_structure(df, sentiment, measure_type, min_date=None):
    ...
    logging.debug(f"fit_and_pick_best_structure => s={sentiment}, m={measure_type}")
    ...
    summ_txt= best_res.summary().as_text()
...
def main():
    setup_logging()
    print("Single run with GEE + QIC, plus extra AR(1) debugging for bracketing errors.")
    ...
    print("Fitting GEE models (Ind, Exch, Unstr, AR1) with more debugging for AR(1) bracketing errors...")
