...
# Plot bars with the specified color and outline
bar_color = '#7daec9'  # Desired bar color
outline_color = 'black'  # Outline color for the bars
plt.bar(months, counts, color=bar_color, edgecolor=outline_color, linewidth=1.5)

# Remove the title by not setting it
# plt.title('Total number of articles published each month (Jan 2020 - Sept 2024)', fontsize=36)  # Removed

# Remove x-axis label by not setting it
# plt.xlabel('Month', fontsize=36)  # Removed

# Update the y-axis label with further reduced font size
plt.ylabel('Number of Articles', fontsize=16)  # Reduced by an additional 33%
...
# Shrink y-axis tick labels by 33%
plt.yticks(fontsize=19)  # Reduced from 28 to ~19
...
output_png = 'total_articles_per_month_custom_color.png'  # Output filename remains the same
