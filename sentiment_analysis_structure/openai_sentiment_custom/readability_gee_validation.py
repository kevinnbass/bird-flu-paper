...
def aggregate_measures(df, measures):
    ...
    for cat in MEDIA_CATEGORIES.keys():
        ...
        for m in measures:
            if m in sub.columns:
                # Some rows might be missing or NaN, so drop them
                valid_vals = sub[m].dropna()
                the_sum = valid_vals.sum()
                the_count = valid_vals.count()
            else:
...
def run_gee_for_measure_best_qic(df, measure):
    ...
    # Put measure into a column that we treat as the endog: _score_col
    d2["_score_col"] = d2[measure].clip(lower=0)  # or no clip if you prefer raw
