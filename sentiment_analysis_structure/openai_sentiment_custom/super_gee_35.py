...
def pairwise_and_diagnostics(df, sentiment, measure, fam_name, struct, scale_name,
    ...
    # Build the "diffIDs_df" as before
    ...
    ########################################################################
    # >>>>>>>>> ONLY CHANGE: reorder the rows of diffIDs_df <<<<<<<<<<
    # We do NOT change 'pair_df' or p-values. We only reorder the final table.
    # The desired row order:
    #   1) Scientific, 2) Left, 3) Lean left, 4) Center, 5) Lean right, 6) Right
    # Extra categories go at the end in alphabetical order.
    custom_order = [
        "Scientific",
        "Left",
        "Lean left",
    ...
    # We'll see which categories actually appear in diffIDs_df
    final_cats = diffIDs_df["Category"].tolist()
    # Put them in desired order, extras at the end
    ordered_cats = [c for c in custom_order if c in final_cats]
    extras = sorted([c for c in final_cats if c not in ordered_cats])
    ordered_cats.extend(extras)
    # create a map
    order_map = {c: i for i, c in enumerate(ordered_cats)}
    diffIDs_df["__temp__"] = diffIDs_df["Category"].map(order_map)
    diffIDs_df = diffIDs_df.sort_values("__temp__").drop(columns="__temp__")
    ########################################################################
