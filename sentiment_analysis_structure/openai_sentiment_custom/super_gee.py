#!/usr/bin/env python3
# analysis_subset_then_full_intensity_collapsed.py
"""
Script that:
  1) Loads data from JSONL
  2) Normalizes high_rate_2 => 'yes'/'no'
  3) For the 'Yes' subset:
     - Collapses each media_category into a single cluster
     - Then runs 3 correlation analyses:
        (a) Quotation vs Fulltext
        (b) Quotation_Intensity vs Fulltext_Intensity
        (c) Fulltext_Intensity vs Fulltext
       => bar charts + scatter + XLSX data
     - Then runs multi-family GEE for 5 measure modes
       => Quotation, Fulltext, Fulltext_Intensity, Title_Intensity, Quotation_Intensity
       => includes validation functions (cross-validation, pseudo-likelihood, etc.)
       => no red highlight in pairwise. If SE=0 => Z=NaN
     - Saves outputs => analysis_main_Yes.xlsx, etc.
  4) Wipes data from memory
  5) Reloads data => Full set => NO collapse (original cluster=media_outlet_clean)
     => repeats correlation + GEE => analysis_main_All.xlsx, etc.

NEW CHECKS ADDED:
  - Check number of clusters (warn if < some threshold)
  - Check cluster-size balance (warn if some clusters have very large differences in size)
  - Attempt a small-sample correction (Mancl & DeRouen approach) for the robust sandwich variance
  - Provide a coverage simulation function for parametric checking of coverage
"""
...
def analyze_2fields_correlation(
    ...
    if all_combo:
        ...
        with pd.ExcelWriter(out_xlsx, engine="openpyxl") as writer:
            cor_df.to_excel(writer,"CorrelationSummary",index=False)
            allc.to_excel(writer,"CombinedZScatterData",index=False)
...
def compile_results_into_multiple_workbooks(
    ...
    # now we produce analysis_plots => embed PNG
    wb_plots=Workbook()
    ...
    # finally, combined => aggregated, stats, raw, bestQIC => single workbook
...
###############################################################################
# 6) run_pipeline_for_df => subset or full
###############################################################################
def run_pipeline_for_df(
    ...
    Then do the 3 correlation analyses + GEE
    Now also includes:
      - check_number_of_clusters
      - check_cluster_balance
    """
    # 1) Check # of clusters and cluster balance
    check_number_of_clusters(df, "media_outlet_clean", threshold=20)  # example threshold
    check_cluster_balance(df, "media_outlet_clean", imbalance_ratio=5.0)  # e.g. 5x difference
...
def check_number_of_clusters(df, cluster_col, threshold=20):
    ...
    Example usage: check_number_of_clusters(df, 'media_outlet_clean', 20)
    """
    n_clusters = df[cluster_col].nunique()
    if n_clusters < threshold:
        logging.warning(f"Number of clusters={n_clusters} < {threshold}. "
                        "Small-sample GEE standard errors may be unreliable.")
        print(f"WARNING: Only {n_clusters} clusters. "
              "Robust SE may be biased => consider small-sample corrections.")
    else:
        print(f"Number of clusters={n_clusters} >= {threshold} => likely safe for GEE asymptotics.")


def check_cluster_balance(df, cluster_col, imbalance_ratio=5.0):
    """
    Warn if some clusters have sizes much larger (or smaller) than others.
    E.g. imbalance_ratio=5.0 means if the biggest cluster is >5x the size of the smallest cluster, warn.
    """
    ...
    if ratio > imbalance_ratio:
        logging.warning(f"Cluster size imbalance: largest cluster has {max_size} rows, "
                        f"smallest has {min_size} => ratio={ratio:.1f} > {imbalance_ratio}")
        print(f"WARNING: Cluster size imbalance => ratio={ratio:.1f} > {imbalance_ratio}. "
              "Large imbalance can inflate or deflate SEs.")
    else:
        print(f"Cluster size ratio={ratio:.1f} <= {imbalance_ratio} => no major imbalance issues.")


def mancl_derouen_correction(gee_result):
    """
    **Simplified** demonstration of a possible Mancl & DeRouen small-sample correction.
    Real-world usage requires cluster-level derivatives, etc.
    This function is purely illustrative. Statsmodels does not provide a direct M&D interface.
    
    Basic idea:
    - M clusters
    - Standard robust sandwich V = A * B * A
    - M&D propose factoring out (M/(M-1)) or similar correction factor to reduce small-sample bias.
    
    Returns a corrected covariance matrix you might then substitute into gee_result.cov_params_default.
    """
    ...
    # naive approach: multiply by (M/(M-1)) – or use some other factor from the literature
...
def coverage_simulation(
    ...
    """
    Parametric coverage simulation to see if standard errors are too big/small.
    
    Creates synthetic data with a known "true_beta" for a single binary covariate (X).
    We have 'n_clusters' clusters, each with 'cluster_size' observations.
    Then fits GEE and checks coverage of the 95% CI for that coefficient.
    
    Return: coverage (float), the proportion of times the 95% CI includes true_beta.
    """
    ...
    for sim_i in range(n_sims):
        ...
        # True linear predictor: intercept=0, slope=true_beta
        lin_pred = 0 + true_beta * X

        # Link depends on family. For Poisson, mu=exp(lin_pred)
        # We'll assume Poisson for demonstration
        mu = np.exp(lin_pred)
    ...
    print(f"Coverage Simulation => coverage={coverage_rate:.2f} over n={n_sims} sims. (True beta={true_beta})")
...
def main():
    setup_logging()
    logging.info("Starting pipeline => yes-subset => single cluster/cat => correlations+GEE => wipe => all => original cluster")
    ...
    logging.info("All done => subset + full => correlation scatter + GEE, plus new checks and warnings.")
