...
Script that:
  ...
  2) Aggregates stats
  3) Finds best QIC combination (multi-family, GEE)
...
Additional features:
  - Cross-validation fix => unify category levels so "Scientific" doesn't break.
  - Prefix-based file naming => no overwriting between full & subset runs.
  - Debug checks => prints unique "high_rate_2" values & normalizes them.
"""
...
INPUT_JSONL_FILE = "processed_all_articles_fixed_4.jsonl"  # or your file name
...
# We do "Quotation vs Fulltext" for these sentiments
CATEGORIES = [
...
def run_gee_analyses_best_qic(df):
    """
    We run best QIC approach multi-families for each sentiment–measure,
    focusing on "Quotation" & "Fulltext" here.
    """
...
def run_pipeline_for_df(
    ...
    # Suffix files if prefix => avoid overwriting
    if prefix:
...
def main():
    setup_logging()
    logging.info("Starting best QIC GEE approach + advanced checks, with subset for high_rate_2=yes + debug prints + cross-validation category fix.")

    df=load_jsonl(INPUT_JSONL_FILE)
    print(f"Total articles loaded: {len(df)}")

    # Debug: see raw unique values
    print("\n-- Debug: Checking raw 'high_rate_2' values before normalization --")
    if "high_rate_2" in df.columns:
        print("Unique 'high_rate_2':", df["high_rate_2"].unique())
        print(df["high_rate_2"].value_counts(dropna=False))
    else:
        print("'high_rate_2' not in df columns at all")

    # Normalize 'high_rate_2' => trim spaces + lower
    if "high_rate_2" in df.columns:
    ...
    run_pipeline_for_df(
        df,
        prefix="All"  # ensures separate files: e.g. analysis_main_All.xlsx
    )

    # Now subset for "high_rate_2" == "yes"
    print("\n=== Subsetting for high_rate_2 == 'yes' ===")
    if "high_rate_2" not in df.columns:
        print("No 'high_rate_2' column => subset impossible => skipping.")
        return

    df_yes = df[df["high_rate_2"] == "yes"].copy()
    print(f"Subset df_yes => {len(df_yes)} rows total.")
    if len(df_yes) < 1:
        print("No entries found with high_rate_2 == yes => skipping subset analysis. (Hence identical or no results.)")
    else:
        run_pipeline_for_df(
            df_yes,
            prefix="Yes"  # ensures separate outputs: analysis_main_Yes.xlsx etc.
        )

    logging.info("All done. Exiting main().")
