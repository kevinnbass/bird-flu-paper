...
def analyze_2fields_correlation(df, left_field_pattern, right_field,
    ...
    for s in CATEGORIES:
        ...
        if len(sub)>1:
            cval,pval=pearsonr(sub["Left_Average"], sub["Right_Average"])
        else:
            cval,pval=(np.nan, np.nan)
        cor_results.append({"Sentiment":s,"Correlation":cval,"P_Value":pval})
        if not sub.empty:
            all_combo.append(sub.copy())

    cor_df=pd.DataFrame(cor_results)

    # Plot bar of correlation per sentiment
    ...
    # If we have data, compute z-scores & produce scatter
    if all_combo:
        ...
        # ----------------------------------------------------------------------
        # NEW scatterplot code: no gridlines, black points, no legend, with trendline + R/p
        # ----------------------------------------------------------------------
        # 1) Compute overall correlation for the entire allc (Left_Average vs Right_Average)
        #    so we can display R= and p= for the plot.
        r_all, p_all = pearsonr(allc["Left_Average"], allc["Right_Average"])

        # 2) Set style => remove grid lines
        sns.set_style("white")

        # 3) We use sns.regplot for a regression trendline
        ...
        # 4) No legend => (we're not coloring by category, so no legend needed)
        # 5) Add the R= value and p= value in the top-left corner
        plt.text(
            0.05, 0.95,
            f"R={r_all:.3f}, p={p_all:.3g}",
