...
def fit_lmm_and_posthoc(df, sentiment, measure_type='Quotation'):
    ...
    # Check if any outlet too sparse or no variation
    # Variation check (just an example: standard deviation of score per outlet)
...
def compile_into_single_combined_workbook(aggregated_df, stats_df, raw_df, lmm_results, plots_dir):
    # Combine all into one single xlsx
    # This is a simplified version embedding main sheets and some key results.
    # For demonstration, we'll put aggregated, stats, raw (limited), LMM summaries, and one sheet for each plot.
    wb = Workbook()
    if 'Sheet' in wb.sheetnames:
        wb.remove(wb['Sheet'])
    ...
    # Raw Data (just a sample due to size, or full if you want)
...
def check_collinearity(df, predictor_columns):
    # A basic placeholder collinearity check:
    # If no predictor columns, just print a message.
    ...
    # If you have predictors:
    # For demonstration: Check if any predictor has zero variance:
    ...
    # You can also compute correlations:
...
def main():
    ...
    # Optional predictor columns (adjust as needed):
    predictor_columns = []  # Add your predictor columns here if any.
