...
Additionally:
  - In analyze_quotation_fulltext_correlation(), we produce a second tab
    "CombinedZScatterData" in quotation_fulltext_correlation.xlsx,
    containing the Z-score adjusted data used for the Combined Z-scatter.
"""
...
#    => now also produce CombinedZScatterData tab
###############################################################################
def analyze_quotation_fulltext_correlation(df):
    ...
    cor_results=[]
    # We'll also store raw data for z-scatter
    all_combo=[]
    ...
    for s in CATEGORIES:
        ...
        # Also store sub so we can do z-scoring later
        if not sub.empty:
            cpy=sub.copy()
            cpy["Sentiment"]=s
    ...
    # Build the combined z-scatter data
    if all_combo:
        allc=pd.concat(all_combo,ignore_index=True)
        # group by Sentiment => compute mean, std of Quotation_Average, Fulltext_Average
        ...
        # We'll store 'allc' in a second tab of 'quotation_fulltext_correlation.xlsx'
        # We'll also store the correlation summary in the first tab
...
def main():
    setup_logging()
    logging.info("Starting best QIC GEE approach + advanced checks + fix for index KeyError in residuals + Z-scatter export")
    ...
    print("Analysis completed successfully with KeyError fix + new CombinedZScatterData tab.")
