...
A complete Python script that:
  ...
  3) For the 'Yes' subset => collapses each media_category into a single cluster
     => correlations, GEE, validations
  4) Then for the Full set => original clusters => correlations, GEE, validations
  5) Outputs multiple Excel files with:
     - Aggregations, raw data, GEE results, embedded plots
     - A single "ValidationSummary" tab unifying old & new checks:
        * # clusters, cluster imbalance, coverage simulation
        * old GEE diagnostics (residual check, pseudo-likelihood, CV MSE)
  6) Appends a brief conclusion at the bottom of that summary tab

NOW INCLUDES:
  - A Mancl & DeRouen (M&D) correction to robust covariance.
"""
...
OUTPUT_EXCEL_QFTC = "quotation_fulltext_correlation.xlsx"      # Quotation vs Fulltext
...
def analyze_2fields_correlation(
    ...
    """
    For each category & sentiment => average left & right => correlation
    => bar chart, scatter, XLSX
    """
    ...
    # correlation by sentiment => cor_df
    cor_results=[]
    all_combo=[]
    for s in CATEGORIES:
...
###############################################################################
# 5) GEE code => multi-family & validation
###############################################################################

################# NEW M&D CORRECTION DEMO ##################
def mancl_derouen_correction(gee_result):
    """
    Returns a Mancl & DeRouen-corrected version of the robust covariance matrix.
    The factor is M/(M-1), where M=#clusters.
    """
    ...
    V_robust = gee_result.cov_params()  # robust covariance
    correction_factor = M/(M-1.0)
    V_corrected = correction_factor * V_robust
    return V_corrected
############################################################
...
def check_residuals_and_correlation(final_fit):
    """
    Returns a dictionary with residual-based diagnostics, including an assessment string.
    """
...
def bootstrap_gee(df, formula, group_col,
    ...
    """
    NOTE: B=200 is for demonstration. Increase to 50+ for stable bootstrap.
    """
...
def pairwise_and_diagnostics(df, sentiment, measure, fam_name, struct, scale_name):
    """
    Returns (summary_txt, pair_df, diffIDs_df, diag_dict, pseudo_dict, cv_val, sens_df, boot_df)
    which we can use for final output.
    We'll also add M&D-corrected covariance as a demonstration.
    """
    ...
    # NEW: Mancl & DeRouen Correction => store corrected cov in final_fit
    V_mand = mancl_derouen_correction(final_fit)
    # e.g., store it as an attribute so we can see it later if we want.
    final_fit.cov_params_mand = V_mand
    # We can also compute M&D-corrected standard errors for reference:
    mand_se = np.sqrt(np.diag(V_mand))

    # Now do pairwise contrasts using the original robust cov (or we could do the M&D cov if we want to interpret pairwise differently)
    params=final_fit.params
    cov=final_fit.cov_params()  # original robust cov, not M&D-corrected by default
...
###############################################################################
# NEW Validations: cluster checks, coverage, plus old GEE diagnostics
###############################################################################
def check_number_of_clusters(df, cluster_col, threshold=20):
    """
    Warn if the number of unique clusters is below a threshold.
    Returns a dict for summary tab.
    """
...
def check_cluster_balance(df, cluster_col, imbalance_ratio=5.0):
    """
    Check if largest cluster is more than 'imbalance_ratio' times
    bigger than the smallest cluster. Return dict for summary tab.
    """
...
def coverage_simulation(
    ...
    """
    Quick parametric coverage simulation. Return coverage dict for summary tab.
    """
    ...
    # Decide if coverage is close to 95%
...
###############################################################################
# compile_results_into_multiple_workbooks => single ValidationSummary
###############################################################################
def compile_results_into_multiple_workbooks(
    ...
    Also merges "old" GEE diagnostics into 'validation_records' so we can unify
    everything in a single ValidationSummary sheet.
    """
    ...
    # We'll also store old GEE validations inside "validation_records" so that 
    # everything merges into ONE final ValidationSummary tab
    ...
    with pd.ExcelWriter(gee_excel, engine="openpyxl") as writer:
        ...
        for i,row in df_best_qic.iterrows():
            ...
            # OLD validation in dictionary form => appended to validation_records
            ...
            # sensitivity analysis -> separate tab
            sens_records.append(sens_df_df)

            # bootstrap -> separate tab
        ...
        if len(boot_records)>0:
            ...
            boot_all.to_excel(writer, sheet_name="Bootstrap_Res", index=False)

        # Write out the final validation summary, merging old + new
        val_df=pd.DataFrame(validation_records)
        val_df.to_excel(writer, sheet_name="ValidationSummary", index=False)
        last_row=len(val_df)+2
        ...
        conclusion_row.to_excel(writer, sheet_name="ValidationSummary", index=False, startrow=last_row)
    ...
    # COMBINED => aggregated, stats, raw, bestQIC, plus the same ValidationSummary
    ...
    for row_ in dataframe_to_rows(val_df2, index=False, header=True):
        ws_val.append(row_)
    last_r2=len(val_df2)+2
    ws_val.cell(row=last_r2, column=1, value=(
...
###############################################################################
# 6) run_pipeline_for_df => gather validation records
###############################################################################
def run_pipeline_for_df(
    ...
    """
    If prefix=="Yes", collapse each media_category => single cluster
    If prefix=="All", keep original cluster => media_outlet_clean
    Then do correlation analyses + GEE
    Also runs global validations:
      - # of clusters
      - cluster balance
      - coverage simulation
    """
    ...
    # 3) Coverage simulation (parametric) - example usage, can adjust n_sims etc.
    c_cov = coverage_simulation(
        n_sims=1000,
        n_clusters=10,
        cluster_size=10,
        true_beta=0.5
