...
monthly_counts['month_dt'] = pd.to_datetime(monthly_counts['month'], format='%b %Y')
monthly_counts = monthly_counts.sort_values('month_dt')

# For plotting, convert 'month' back to string after sorting
monthly_counts['month'] = monthly_counts['month_dt'].dt.strftime('%b %Y')

# Debugging: Print the first few rows to verify
print("\nAggregated Data (First 5 Months):")
print(monthly_counts.head())

# Verify that monthly_counts has non-zero counts
print("\nTotal Articles Per Month (First 5 Months):")
print(monthly_counts['counts'].head())

# Check for any months with zero total articles
zero_article_months = monthly_counts['counts'] == 0
if zero_article_months.any():
    print("\nMonths with zero articles:")
    print(monthly_counts[zero_article_months]['month'].tolist())

# Define the list of months
months = monthly_counts['month'].tolist()
counts = monthly_counts['counts'].tolist()

# Plotting the bar graph
plt.figure(figsize=(25, 12))  # Adjust figure size as needed

# Plot bars in blue
plt.bar(months, counts, color='blue', edgecolor='white')

# Update the font sizes for the title
plt.title('Total number of articles published each month (Jan 2020 - Sept 2024)', fontsize=48)  # Adjusted title
...
output_png = 'total_articles_per_month.png'
