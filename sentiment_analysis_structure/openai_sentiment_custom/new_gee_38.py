...
INPUT_JSONL_FILE = 'processed_all_articles_fixed_2.jsonl'  # Using the new file per your request
...
def fit_gee_model(df, formula, groups, cov_struct, family=Poisson(), time=None):
    ...
    Additional debugging: we log the shape/dtype of time if present, and parse QIC carefully.
    """
    logging.debug(f"fit_gee_model => structure={cov_struct.__class__.__name__}, time is {'NOT None' if time is not None else 'None'}")
    if time is not None:
        logging.debug(f"Time array dtype={time.dtype}, shape={time.shape}, first5={time[:5] if len(time)>5 else time}")
...
def run_gee_and_qic(df, sentiment, measure_type, structures, min_date=None):
    """
    Attempt GEE with each correlation structure, compute QIC, return dict of {struct_name: (res, qic_float)}.

    We fix the TypeError by extracting only the first element of the QIC result if it's a tuple.
    """
    ...
    if measure_type == 'Quotation':
        ...
        if not matched:
            logging.debug("No matched quotation columns found => returning {}.")
            return {}
        temp = df.copy()
        ...
        if fcol not in df.columns:
            logging.debug("No fulltext column found => returning {}.")
            return {}
        temp = df.copy()
    ...
    if mdf['media_category'].nunique() < 2:
        logging.debug("Not enough categories => returning {}.")
        return {}
    ...
    if 'date' in mdf.columns:
        ...
        logging.debug(f"Dropped {b4-len(mdf)} rows with invalid date => {len(mdf)} remain.")
    mdf['media_category'] = mdf['media_category'].astype('category')

    # create int_date (time) for all structures
    time_arr = None
    ...
    # 3) Fit each structure
    formula = f"{score_col} ~ media_category"
    groups = "media_outlet"
    result_map = {}
    for sname, sobj in structures.items():
        actual_time = None
        if time_arr is not None and len(mdf)>1:
            actual_time = time_arr
        logging.debug(f"Fitting structure={sname}, with {'a time array' if actual_time is not None else 'no time array'} (#rows={len(mdf)}).")

        res = fit_gee_model(mdf, formula, groups=groups, cov_struct=sobj, time=actual_time)
        if res is None:
            logging.debug(f"Fit returned None => skipping {sname}.")
            continue
        # 4) get QIC => might be a tuple => pick first item if so
        try:
            qic_ = res.qic()
            # if it's a tuple => pick the first
            ...
            qic_val = np.nan
        logging.debug(f"Structure={sname}, QIC={qic_val}")
        result_map[sname] = (res, qic_val)
...
def fit_and_pick_best_structure(df, sentiment, measure_type, min_date=None):
    ...
    for nm, (res_obj, qic_val) in res_map.items():
        ...
        # Compare if we have a real float
        if (qic_val is not None) and np.isscalar(qic_val) and (qic_val < best_qic):
            best_qic = qic_val
