...
OUTPUT_EXCEL_GRAPHING = 'analysis_for_graphing.xlsx'  # Modified to create per-sentiment sheets
...
def compute_article_level_scores(df, sentiment_categories):
    # For each sentiment:
    # _quotation_mean_article is the mean of all quotation-based columns for that sentiment, clipped at 0.
    # _fulltext_article is the clipped fulltext score for that sentiment.
...
def create_analysis_for_graphing_file(df, sentiments, category_order, output_file):
    # For each sentiment, create two sheets:
    # 1. "<sentiment>_Quotation" sheet: Contains columns for each category in category_order.
    #    Each row represents one article. For an article in category c, put its 
    #    sentiment_quotation_mean_article value in column c, and leave other columns blank.
    #
    # 2. "<sentiment>_Fulltext" sheet: Same structure, but use sentiment_fulltext_article values.
    #
    # We only consider articles whose category is in category_order (ignore 'Other').
...
def main():
    ...
    # Create separate raw xlsx for graphing. Now a tab for each sentiment and measure.
