...
# gee_cld_debug.py
"""
This script runs GEE analyses for each sentiment–measure combination,
and attempts to build a Compact Letter Display (CLD) via adjacency matrix + rpy2.

Repeated Conflict Errors
------------------------
If you see repeated messages like:
  "R-based CLD conflict: Center vs Lean Left are 'not differ' but share no letter!"
that indicates your adjacency matrix says "Center" and "Lean Left" must share at 
least one letter (meaning they are 'not differ'), but the letter assignment logic 
can't find a consistent letter for them—likely because of other conflicting constraints 
(“triangular” patterns). 

To debug:
  - We log the adjacency matrix for each tab.
  - We log each conflict in detail, so you can see which pairs produce contradictions.
  - We do NOT raise an exception. Instead, if conflicts appear, the CLD table will 
    contain a minimal row describing them, rather than the normal letter grouping.
"""
...
# Make sure "multcompView" is installed in R
...
def analyze_quotation_fulltext_correlation(df):
    ...
    plt.ylim(-1,1)
    plt.tight_layout()
    outpath_bar=os.path.join(OUTPUT_DIR,"correlation_quotation_fulltext_bar.png")
    try:
    ...
    if combo:
        ...
        plt.tight_layout()
        csc_path=os.path.join(OUTPUT_DIR,"combined_normalized_scatter.png")
        try:
            plt.savefig(csc_path)
        except:
            pass
        plt.close()
...
def compute_deviance_scale(y,mu,df_resid):
    ...
    for i in range(len(y)):
        if y[i]>0 and mu[i]>0:
            arr[i]=y[i]*math.log(y[i]/mu[i])-(y[i]-mu[i])
        elif y[i]==0:
            arr[i]=-(y[i]-mu[i])
        else:
            arr[i]=np.nan
...
def run_gee_for_sentiment_measure_best_qic(df, sentiment, measure):
    ...
    for cov_obj in structures:
        ...
        for sm,(qm,qa,sc_val) in sc_map.items():
            ...
            if (qm<best_qic_main) and (not math.isnan(qm)):
                best_qic_main=qm
...
###############################################################################
# 6) Build adjacency => pass to R => log adjacency + conflicts
###############################################################################
def build_cld_with_rpy2(final_fit, pair_df):
    """
    Attempt to build CLD from adjacency matrix using rpy2 + multcompView.
    We log the adjacency matrix for debugging, and if conflicts are found,
    we embed them in the final table rather than raising an error.
    """
    conflicts=[]
    ...
    # debugging => log adjacency
    logging.debug(f"Adjacency matrix for categories {group_names} =>\n{adjacency}")
    ...
    try:
        ...
        for i in range(n):
            for j in range(i+1,n):
                ...
                A=group_names[i]
                B=group_names[j]
                Aletters=letters_dict[A]
                Bletters=letters_dict[B]
                overlap=set(Aletters).intersection(set(Bletters))
                if not_diff:
                    if len(overlap)==0:
                        conflict_msg=f"{A} vs {B} => not_diff=1, but share no letter"
                        logging.debug(conflict_msg)
                        conflicts.append(conflict_msg)
                else:
                    if len(overlap)>0:
                        conflict_msg=f"{A} vs {B} => differ=1, but share letter {overlap}"
                        logging.debug(conflict_msg)
                        conflicts.append(conflict_msg)

    except Exception as e:
        logging.error(f"multcompLetters call error => {e}")
        letters_dict=None
        conflicts.append(f"multcompLetters error => {str(e)}")

    # if conflicts => produce minimal conflict table
    rows=[]
    if not conflicts and letters_dict is not None:
        ...
        # conflicts => produce minimal row(s)
        if letters_dict is None:
            # big error
            rows.append(("CLD conflict =>", "multcompLetters call error", "", "No valid letters."))
        else:
            conf_join=", ".join(conflicts)
            rows.append(("CLD conflict =>", conf_join, "", "No stable letter assignment."))
...
def pairwise_and_cld(df, sentiment, measure, struct, scale_name):
    ...
    2) Build pairwise => adjacency => pass to build_cld_with_rpy2 => logs adjacency, merges conflicts if needed
    3) Return summary_txt, pair_df, cld_df
    """
    from statsmodels.genmod.generalized_estimating_equations import GEE
    d2=df.copy()
    if measure=="Quotation":
    ...
    # build CLD => logs adjacency & conflicts
    cld_df=build_cld_with_rpy2(final_fit, pair_df)
    return summary_txt, pair_df, cld_df

###############################################################################
# 7) Compile => produce each tab => conflict => minimal row
###############################################################################
def compile_results_into_multiple_workbooks(
    ...
    # analysis_gee => produce summary, pairwise, cld (with conflict row if needed)
    with pd.ExcelWriter(gee_excel,engine="openpyxl") as writer:
        ...
        for i, row in df_best_qic.iterrows():
            ...
            # create that sheet with placeholder
            placeholder_df=pd.DataFrame({"Info":[f"{s}-{meas} tab init"]})
            placeholder_df.to_excel(writer,sh_name,index=False)

            summary_txt, pair_df, cld_df=pairwise_and_cld(df_full,s,meas,st,sc)
            if summary_txt is None:
                # no valid model => fill that sheet
                no_data_df=pd.DataFrame({"Summary":[f"No valid model or not enough data => {s}-{meas}"]})
                no_data_df.to_excel(writer,sh_name,index=False)
            else:
                # place summary
    ...
    # analysis_plots => ensure at least one sheet
    wb_plots=Workbook()
    ...
    # analysis_combined => ensure at least one sheet
...
def main():
    setup_logging()
    logging.info("Starting best QIC GEE approach with adjacency-based CLD => conflict is embedded in CLD table, no skipping tabs.")
    ...
    # 5) best QIC approach
    print("Fitting best QIC approach for each sentiment–measure (with BH pairwise).")
    df_best, df_allcombos = run_gee_analyses_best_qic(df)
    print("Best QIC approach completed.\n")

    # 6) compile => no skipping => conflict embedded in CLD
    ...
    print("Analysis completed successfully. Any repeated conflicts are shown in the CLD portion.")
