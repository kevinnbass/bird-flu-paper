...
def pairwise_and_diagnostics(df, sentiment, measure, fam_name, struct, scale_name,
    ...
    # (Optional) cluster bootstrap on final model
    means_boot, sds_boot = cluster_bootstrap_gee(
        df=d2,
        formula=final_fit.model.formula,
    ...
    # For now, let's do no final bootstrap or keep it if you want:
    means_boot, sds_boot = (pd.Series(dtype=float), pd.Series(dtype=float))

    # Fill COMPARE_IMBALANCE
    for i, p in enumerate(param_names):
        sr_se  = robust_se[i]
        sr_val = param_values[i]
        bs_sd  = sds_boot.get(p, np.nan) if not sds_boot.empty else np.nan
        ratio  = np.nan
        if sr_se>0 and not np.isnan(bs_sd):
            ratio = bs_sd/sr_se
    ...
    # Return partial "boot_info" if needed
    boot_info = {
        "ParamMean": means_boot.to_dict(),
        "ParamStd":  sds_boot.to_dict()
    }
