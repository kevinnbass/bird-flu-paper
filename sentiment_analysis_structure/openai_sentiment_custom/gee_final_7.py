#!/usr/bin/env python3
# gee_final_best_qic_improved_cld.py
"""
Script that:
  ...
  6) For each best QIC model:
     ...
     - Build LSD-like CLD with an improved approach that merges letter sets 
       if categories are “equal” to multiple different letter groups
...
The key improvement is in build_lsd_cld(), ensuring that if a new category does not differ from multiple groups of categories, it inherits all letters from all those groups, merging them.
"""
...
def run_gee_for_sentiment_measure_best_qic(df, sentiment, measure):
    ...
    Each => fit_and_compute_scales => compare QIC => best
    """
    d2=df.copy()
...
def refit_best_gee_with_scale(df, sentiment, measure, struct, scale_name):
    ...
    try:
        ...
        logging.warning(f"Refit base => {struct} => fail => {e}")
        return None
...
#
# == Improved LSD-based CLD (Merges letter sets) ==
#
def build_lsd_cld(fit_result, pair_df):
    """
    1) predicted means => exp(intercept+cat_effect)
    2) sort ascending
    3) LSD-based letter assignment => 'reject_H0'=True => can't share letter
       BUT if a new category is 'not different' from multiple letter groups, we merge them
    4) reorder final in CLD_ORDER
    """
    ...
    # differ => if reject_H0 => True => they differ => can't share letter
    differ={}
    ...
    # We'll store letter sets in a dictionary, plus maintain a "representative" approach
    # Each time we add a new cat, we find the sets of letter groups from cats that are "not differ" => we union them
    # This ensures if a cat is "equal" to multiple letter sets, we unify them all
    cat_letters={c:set() for c in cats}  # each cat -> set of letters
    # We'll keep a list of letter sets
    letter_groups=[]  # each group is a set of cats that share that letter

    # We'll also keep an array of letter names
    letter_names=[]

    import string
    all_letters=list(string.ascii_lowercase)
    # We'll create letters dynamically
    letter_counter=0

    for c in sorted_cats:
        # find all existing letter sets that c is "not differ" from
        # i.e. for each cat x in that letter set => differ[c][x]=False => no difference
        # We'll gather a union of all those sets
        sets_to_merge=[]
        for group_ix, catset in enumerate(letter_groups):
            # check if c is 'equal' to *any* cat in catset => or must it be 'equal' to all cats in catset?
            # LSD approach => c must be 'not differ' from ALL cats in that group in order to share that letter
            # so let's check all cats in catset => if ANY differ => conflict
            conflict=False
            for x in catset:
                if differ[c].get(x,False) is True:
                    ...
                    break
            if not conflict:
                # we can share this group's letter
                sets_to_merge.append(group_ix)

        if not sets_to_merge:
            # c needs a new letter
            ...
            # unify all sets in sets_to_merge into one big set
            merged_cats=set()
            merged_letters=[]
            # we'll gather the first letter name from sets_to_merge[0], or we can unify letter names?
            # LSD typically merges letter sets => they share letter. 
            # But if we unify multiple groups, we effectively combine letters. We'll do that => cat gets union of letters
            # Step 1: union all cat sets
            for gix in sets_to_merge:
                merged_cats = merged_cats.union(letter_groups[gix])
            # step 2: gather letter(s)
            # all those cat sets presumably had one letter each? 
            # but LSD can produce a scenario where each group is a single letter
            # We'll unify them => they become a multi-letter group if needed
            # Actually typical LSD approach merges them into a single letter. But for "Right=ab" scenario => that means c is in multiple letters if c is bridging two sets that can't be merged into a single letter. We'll keep them separate if there's conflict? 
            # The simplest fix => each group_ix has exactly 1 letter => so we unify them all => that cat has union of letters
            # But LSD "standard" approach merges them into a single letter name. We'll produce multi-letter if needed.
            # So let's find the letter each group had used => "the first cat"?
            # We'll do a map from group_ix -> letter? Let's store letter_of_group in parallel
            # We'll keep a separate structure: group_letter_name. We'll do that:
            pass

            # For simplicity, let's maintain group_ix => letter_name
            # but we do need that in parallel
            # We'll store in letter_names
            # letter_names[gix] => the single letter for that group
            # then we unify them => c gets the union
            # Also all cats in those groups get that union => because we must reflect that c connects them
            # Actually typical LSD merges them into a single letter. That merges all sets. So we produce a new letter if they differ?
            # The scenario for "Right=ab" arises if "Right" is bridging two letter sets that do differ among themselves. LSD approach merges them => they'd all share the same letter => losing distinction. 
            # But we do want "Right"=ab if it's bridging two distinct letter sets that are not bridging each other. 
            # => We implement "multi-letter" LSD approach. 
            
            # We'll gather all the letters from each group_ix in sets_to_merge
            letters_from_merged=set()
            for gix in sets_to_merge:
                # gather cats in letter_groups[gix]
                for cc in letter_groups[gix]:
                    letters_from_merged = letters_from_merged.union(cat_letters[cc])
            
            # c inherits that entire set of letters
            cat_letters[c] = letters_from_merged.copy()

            # c might add even a new letter if none are assigned yet
            if not cat_letters[c]:
                new_letter=all_letters[letter_counter]
                letter_counter+=1
                cat_letters[c].add(new_letter)

            # now unify all sets into one big set that includes c
            unified_set = merged_cats.union({c})

            # all cats in that unified set => their letters = union_of(letters_from_merged)
            for cc in unified_set:
                cat_letters[cc] = cat_letters[cc].union(cat_letters[c])

            # we must replace old sets in letter_groups with the single new unified set, removing duplicates
            # simplest approach => create a new list of letter_groups with these sets removed, then add the new set
            new_letter_groups=[]
            new_letter_names=[]
            merged_out=set(sets_to_merge)

            # We'll unify all the cat sets from sets_to_merge into one
            for gix, catset in enumerate(letter_groups):
                if gix not in sets_to_merge:
                    new_letter_groups.append(catset)
                    new_letter_names.append(letter_names[gix])
            # now add the new unified set as a new group => but it might represent multiple letters? We'll store a placeholder
            new_letter_groups.append(unified_set)
            # letter for that new group => we do not have a single letter, but let's pick a "lowest" letter as the "representative"? 
            # We'll store them as something like "multi" or the min letter?
            # We'll store the concatenation sorted
            merged_letters_sorted=''.join(sorted(cat_letters[c]))
            new_letter_names.append(merged_letters_sorted)

            letter_groups=new_letter_groups
            letter_names=new_letter_names

    # Now we've assigned sets. We just produce the final letter strings from cat_letters
    rows=[]
    for c in CLD_ORDER:
        if c in cats:
...
def main():
    setup_logging()
    logging.info("Starting best QIC GEE approach with LSD-based CLD that merges letter sets for multi-letter categories if needed.")
    ...
    # 5) best QIC approach (Ind, Exch, Unstructured)
    print("Fitting best QIC approach for each sentiment–measure (Ind/Exch/Unstructured) + pairwise + improved LSD-based CLD.")
