...
Script that:
  ...
  6) For each best QIC model:
     ...
     - Build CLD table with 3 columns: [MediaCategory, Mean, CLD],
       where 'CLD' is blank & 'Mean' is the predicted mean from the model
...
# ------------------------------------------------------
# 6) Pairwise + CLD (BH + blank CLD with mean)
# ------------------------------------------------------
...
def pairwise_and_cld(df, sentiment, measure, struct, scale_name):
    ...
    4) Build CLD => 3 columns: [MediaCategory, Mean, CLD (blank)]
    """
    ...
    # 3) Build CLD with a "Mean" column in the middle
    cld_df=build_cld_with_mean(final_fit)

    return summary_txt, pair_df, cld_df

def build_cld_with_mean(final_fit):
    """
    Returns a 3-column DataFrame:
      [MediaCategory, Mean, CLD]
    'Mean' is the predicted mean from the GEE model
    'CLD' is left blank for manual assignment.
    """
    mdf=final_fit.model.data.frame
    cats=mdf["media_category"].cat.categories
    ...
    # Build the table: [MediaCategory, Mean, CLD]
    rows=[]
    for c in CLD_ORDER:
        if c in cats:
            rows.append((c, mean_map[c], ""))
        else:
            # If this category isn't in the data at all, put None for mean
            rows.append((c, None, ""))
...
def compile_results_into_multiple_workbooks(
    ...
    # 3) analysis_gee => best QIC w/ pairwise + (Mean col + blank CLD)
...
def main():
    setup_logging()
    logging.info("Starting best QIC GEE approach with BH pairwise + CLD table [MediaCategory, Mean, CLD (blank)].")
    ...
    # 5) best QIC approach
    print("Fitting best QIC approach for each sentiment–measure (with BH pairwise + Mean + blank CLD).")
