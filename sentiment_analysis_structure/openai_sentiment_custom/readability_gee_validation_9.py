...
def pairwise_and_diagnostics(df, measure, fam_name, struct, scale_name, alpha_val=None):
    ...
    # We'll add a line to GEE_Summary about alpha usage if NB
...
def compile_results_into_multiple_workbooks(
    ...
    with pd.ExcelWriter(gee_excel, engine="openpyxl") as writer:
        ...
        for i,row in df_best_qic.iterrows():
            ...
            # We pass alpha_val so the final model truly uses the best alpha
            ...
            diag_records.append({
                ...
                "Structure": st,
                "Scale": sc,
                "Alpha": alpha_val,  # store alpha in Diagnostics
...
def main():
    ...
    print("Analysis completed successfully with best alpha integrated + debug checks.")
    logging.info("Analysis completed successfully with best alpha integrated + debug checks.")
