...
"""
main.py
High-level script that orchestrates article processing using the old multi-phase pipeline
(ArticleProcessor.process_article).
"""
...
def load_articles(input_file: str, id_range: Dict[str, int]) -> List[Dict[str, Any]]:
    """
    Read articles from JSONL input, filter by ID range, and return as a list.
    """
    articles = []
    try:
        with open(input_file, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Filter by ID range if valid integer
                    try:
...
def main():
    ...
    # Initialize processor with API client, validator, and prompts
    try:
    ...
    try:
        ...
        # Process each article via the old multi-phase pipeline
            ...
            for article in articles:
                ...
                try:
                    ...
                    if final_data is not None and "error" not in final_data:
                        ...
                        # Track discarded statements if any
                        if discard_info:
        ...
        # If there are discarded statements, write them out
        if discard_list:
