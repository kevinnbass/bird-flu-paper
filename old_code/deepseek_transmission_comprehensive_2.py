...
def process_article_six_prompts(article_id, fulltext):
    ...
    You are an extremely thorough analytical assistant that never misses shades of nuance in meaning and always analyzes and understands the subtleties of precisely different words mean, following instructions precisely by matching the meaning of what is asked for with what is in the text, yet without risking the possibility that you might leave something out that is important that matches the criteria.
    ...
    affirm_transmissible_statements = affirm_transmissible_result.get("affirm_transmissible_statement", {})
    affirm_why_statements = affirm_why_result.get("affirm_why_statement", {})
    ...
    affirm_keywords_statements = affirm_keywords_result.get("affirm_keywords_statement", {})
    ...
    deny_transmissible_statements = deny_transmissible_result.get("deny_transmissible_statement", {})
    deny_why_statements = deny_why_result.get("deny_why_statement", {})
    ...
    deny_keywords_statements = deny_keywords_result.get("deny_keywords_statement", {})
